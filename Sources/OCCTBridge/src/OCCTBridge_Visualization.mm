...
void OCCTCameraSetProjectionType(OCCTCameraRef cam, int type)
{
  if (!cam)
    return;
  cam->camera->SetProjectionType(type == 1 ? Graphic3d_Camera::Projection_Orthographic
                         ...
                         int32_t         maxResults)
{
  ...
  try
  {
    ...
    sel->selector->PickPoint(pixelX, pixelY, pickCam, (int)viewW, (int)viewH);
                             ...
                             int32_t         maxResults)
{
  ...
  try
  {
    ...
    sel->selector->PickBox(xMin, yMin, xMax, yMax, pickCam, (int)viewW, (int)viewH);
...
int32_t OCCTSelectorPickPoly(OCCTSelectorRef sel,
                             ...
                             int32_t         maxResults)
{
  if (!sel || !cam || !out || !polyXY || pointCount < 3 || maxResults <= 0)
    return 0;
  try
  {
    Handle(Graphic3d_Camera) pickCam = new Graphic3d_Camera(*cam->camera);
...
// MARK: - v0.81-v0.82: Quantity_Color/RGBA, Graphic3d Material+PBR, Quantity_Period/Date,
// Font_FontMgr, Image_AlienPixMap MARK: - v0.81.0: Visualization — Quantity_Color,
// Quantity_ColorRGBA, Graphic3d_MaterialAspect, Graphic3d_PBRMaterial

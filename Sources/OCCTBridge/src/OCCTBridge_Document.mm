...
// MARK: - v0.83: XDE Attributes
// (XCAFDoc_Location/GraphNode/Color/Material/Note*/NotesTool/ClippingPlaneTool/ShapeMapTool/AssemblyGraph/AssemblyItemId/VisMaterial/View/NoteObject/PrsStyle)
...
// MARK: - v0.84: TDataStd Directory/Variable/Expression, TDocStd_XLink, XCAFDimTolObjects_Tool,
// TPrsStd_DriverTable, TObj_Application
...
// MARK: - TNaming Extensions + TDataStd_IntPackedMap/NoteBook/UAttribute/ChildNodeIterator +
// TDF_Transaction/Delta/ComparisonTool + TDocStd_XLinkTool + TFunction_IFunction/Scope +
// TDF_AttributeIterator/DataSet (v0.88-v0.89) MARK: - TNaming Extensions (v0.88.0)
...
// MARK: - v0.90: TDF_ChildIDIterator + TFunction_DriverTable + TNaming_Scope/Translator +
// TDataXtd_Placement/Presentation + XCAFDoc_AssemblyIterator/DimTol (TDocStd_PathParser was wrapped
// here too until #499 folded it into the OSD_Path family in OCCTBridge_IO.mm) MARK: -
// TDF_ChildIDIterator (v0.90.0)
...
// Helper: build a path string from an item's path list (entries separated by '/')
static TCollection_AsciiString itemPathString(const XCAFDoc_AssemblyItemId& itemId)
{
  TCollection_AsciiString                          path;
  const NCollection_List<TCollection_AsciiString>& pathList = itemId.GetPath();
  for (NCollection_List<TCollection_AsciiString>::Iterator it(pathList); it.More(); it.Next())
  {
    if (path.Length() > 0)
      path += "/";
    path += it.Value();
...
int32_t OCCTDocumentAssemblyItemCount(OCCTDocumentRef doc, int32_t maxDepth)
{
  ...
  try
  {
    ...
    int                      count = 0;
    // Track visited item paths to detect cycles (issue #964).
    NCollection_DataMap<TCollection_AsciiString, Standard_Boolean> visited;
    while (iter.More())
    {
      XCAFDoc_AssemblyItemId  itemId = iter.Current();
      TCollection_AsciiString path   = itemPathString(itemId);
      if (visited.IsBound(path))
      {
        // Cycle detected - abort to avoid infinite loop.
        // Return -1 to indicate cycle detected.
        return -1;
      }
      visited.Bind(path, Standard_True);
      count++;
      iter.Next();
    }
    return count;

...
/// synchronization, same as every other builder wrapper in this package).
///
...
public final class ThruSectionsBuilder: @unchecked Sendable {
    ...
    ///
    /// Invalidates `shape`/``generatedFace(from:)`` until the next
    ...
    ///
    /// Invalidates `shape`/``generatedFace(from:)`` until the
    ...
    ///
    /// BRepOffsetAPI_ThruSections accepts every value without failing.
    ///
    /// This used to read only.
    /// 0 and 1, mapping everything else to C2 (#490).
    ///
    /// Invalidates `shape`/``generatedFace(from:)``
    ...
    /// ``setParType(_:)``, ``setCriteriumWeight(w1:w2:w3:)``).
    ///
...
extension ThruSectionsBuilder {
    ...
    /// doc comment.
    ///
    /// Returns true if all weights are non-negative; returns false and does not.
    ...
    ///   sequence, if the edge\'s bound face is a leftover from an *earlier* successful build that.

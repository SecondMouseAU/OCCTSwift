...
/// face, closed into a loop where it can be.
///
/// The analysis runs once, and every query below is.
/// answered from that one result, which is the difference between this and the Shape\'s.
...
/// so a lone face reports no free bounds.
///
/// In practice the sewing pass closes essentially every.
...
///   any tolerance; see `Shape.freeBounds(sewingTolerance:)` for why, on both branches.
///
///   This type's.
...
///   not only on the sewing-branch reasoning.
///
///   See #655.
public final class FreeBoundsProperties: @unchecked Sendable {
    ...
    ///
    /// Calling this is optional: every query below runs it on demand.
    ///
    /// It is also idempotent, so a.

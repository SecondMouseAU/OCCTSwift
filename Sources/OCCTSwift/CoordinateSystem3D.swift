...
public struct CoordinateSystem3D: Sendable {
    ...
    public func angle(to other: CoordinateSystem3D) -> Double {
        OCCTAx3Angle(
            origin.x, origin.y, origin.z, direction.x, direction.y, direction.z, xDirection.x,
            xDirection.y, xDirection.z,
            other.origin.x, other.origin.y, other.origin.z, other.direction.x, other.direction.y,
            other.direction.z,
    ...
    public func isCoplanar(
        with other: CoordinateSystem3D, linearTolerance: Double = 1e-6,
    ...
    public func mirrored(about point: SIMD3<Double>) -> CoordinateSystem3D {
        ...
        return CoordinateSystem3D(
            origin: SIMD3(rpx, rpy, rpz), direction: SIMD3(rnx, rny, rnz),
            xDirection: SIMD3(rxDx, rxDy, rxDz))
    }

    /// Rotate about an axis.
    public func rotated(
        about axisOrigin: SIMD3<Double>, axisDirection: SIMD3<Double>, angle: Double
    ) -> CoordinateSystem3D {
        var rpx = 0.0
        var rpy = 0.0
        var rpz = 0.0
        ...
        OCCTAx3Rotate(
            origin.x, origin.y, origin.z, direction.x, direction.y, direction.z,
            xDirection.x, xDirection.y, xDirection.z,
            axisOrigin.x, axisOrigin.y, axisOrigin.z, axisDirection.x, axisDirection.y,
            axisDirection.z, angle,
            &rpx, &rpy, &rpz, &rnx, &rny, &rnz, &rxDx, &rxDy, &rxDz)
        return CoordinateSystem3D(
            origin: SIMD3(rpx, rpy, rpz), direction: SIMD3(rnx, rny, rnz),
            xDirection: SIMD3(rxDx, rxDy, rxDz))
    ...
    public func translated(by vector: SIMD3<Double>) -> CoordinateSystem3D {
        ...
        return CoordinateSystem3D(
            origin: SIMD3(rpx, rpy, rpz), direction: direction, xDirection: xDirection)
    }
}

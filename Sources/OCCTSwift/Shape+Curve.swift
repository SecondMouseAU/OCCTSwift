...
extension Shape {
    ...
    /// with their Bezier equivalents.
    ///
    /// Converts 2D/3D curves, surfaces, lines, circles,.
    ...
    /// reads only `.Parameter()` before discarding it.
    ///
    /// If you need the point or face struck, use.
    ...
    ///
    /// the CPnts_UniformDeflection\'s point count is driven by chordal deflection against curve.
    /// curvature, with no documented ceiling either (the same *class* of exposure as).
    /// `uniformAbscissa(distance:)` above, which has one now.
    ///
    /// Unlike that pair, the buffers here.
    ...
    /// this is an unambiguous failure sentinel, matching ``Curve3D/totalArcLength``.
    ///
    /// It used to.
    ...
    /// 1e-9 relative.
    ///
    /// An elliptical edge measured 1.485% long before that (#603); a straight or.
    ...
    /// closed spline) covers a whole period and so measures the whole range, winding (#600).
    ///
    /// The.
...
extension Shape {
    ...
    /// and the GCPnts_UniformAbscissa\'s count constructor returns exactly the requested count, so.
    /// the check here is a no-op for those two.
    ///
    /// The `distance:` pair has no caller-supplied count
    ...
    ///   already performs internally (once inside the sizeAndFillUniformAbscissa\'s size-only call,.
    ///   again inside its fill call) (two sufficed before this guard existed).
    ///
    ///   Collapsing size+fill.
    ///   into one bridge call would need a pre-allocated buffer; sizing it at.
    ///   ``Sampling/maximumSampleCount`` (10,000,000 Doubles, ~80MB) to stay safe against the.
    ...
    ///
    ///   O(spans) not O(points)) call.
    ///
    ///   Left unchanged rather than trading a simple, already-tested.

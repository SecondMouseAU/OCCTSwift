...
extension Shape {
    ...
    /// answer derived from nothing.
    ///
    /// To measure an open shell, close it first (for example with.
    /// ``sew(shapes:tolerance:)``); which closure you want is your choice to make, not one this
    /// method should guess.
    ///
    /// Use `surfaceArea` or `surfaceInertia` for an area measure and.
    ...
    /// rather than the number the BRepGProp\'s divergence integral produces over a surface that.
    ...
    ///
    /// Closedness is topological, so geometry that merely looks watertight is not enough.
    ///
    /// Six.
    ...
    /// value.
    ///
    /// Use this to detect orientation problems; use ``orientedForward()``.
    /// to fix them.
    ///
    /// **This is an orientation signal, not a measurement.** It is the divergence integral over the.
    /// shape's faces, so its magnitude is a volume only when the surface is closed.
    ///
    /// An open shell.
    ...
    /// negates the flux.
    ///
    /// That is why ``sweep(profile:along:)`` can still normalise a pipe whose
    /// faces point inward even though the pipe it builds is an open shell (#170, #609).
    ///
    /// Returns 0 on an internal error.
    ///
    /// It used to return `-1` there, which ``orientedForward()``.
    /// read as "reverse me".
    ///
    /// ```swift.
    ...
    /// three agree with each other exactly.
    ///
    /// Unlike ``ShapeMeasurements/totalFaceArea`` (N.
    ...
    /// reach for (#885).
    ///
    /// This is the canonical explanation of that divergence; the other.
    ...
    /// built from parts of unequal size sits near the heavy one.
    ///
    /// It is nil for anything that.
    /// encloses no volume, which is a face, wire, edge, vertex or **open shell**.
    ///
    /// Close an open.
    ...
    /// ``linearProperties()`` (length).
    ///
    /// For a vertex's position use ``vertices()``.
    ///
    /// ```swift.
    ...
    ///   unchanged).
    ///
    ///   Do not pass it from a UI/main thread without accepting that stall; the.
    ...
    ///     thread variant: see "Why `timeout:`, not `hardTimeout:`" below).
    ///
    ///     There is no way to.
    ...
    ///     the cost of the rest of the scan combined (a few ms vs 30s at a 30s timeout).
    ///
    ///     On.
    ///     ordinary shapes, including a real 662-face mesh-sewn import, the measured overhead was.
    ///     1x-3x, cheap enough to opt into whenever a caller actually wants the answer.
    ///
    ///     See.
    ...
    /// this shape.
    ///
    /// The shape should be a solid for reliable results.
    ///
    ...
    ///   ``Shape/PointState`` instead of `PointClassification`.
    ///
    ...
    ///
    /// - Parameter optimal: If true, compute a tighter OBB (slower).
    ///
    /// Default is false.
    ...
    ///
    /// - Parameter optimal: If true, compute a tighter OBB.
    ///
    /// Default is false.
    ...
    ///
    /// Nil for any shape with no closed volume: a face, wire, edge, vertex or open shell.
    ///
    /// Every.
    /// field of the result is meaningless there, not merely zero.
    ///
    /// The centre of mass is the.
    /// shape's location origin, the principal axes are the identity basis that math_Jacobi.
    /// returns for a zero matrix, and both symmetry flags read true (#609).
    ///
    /// Use.
    ...
    /// well defined over any set of faces.
    ///
    /// It is nil for a shape with no faces at all (a wire,.
    ...
    ///
    /// Use with `allDistanceSolutions(to:)` to get
    /// the solution index.
    ...
    /// or dispersed in 3D space.
    ///
    /// Useful for determining degeneracy of point sets.
    ...
    /// (#609).
    ///
    /// Also reports the two symmetry flags ``surfaceInertiaProperties()`` has always had.
    ...
    ///     print(hit.distance)       // 7.81, to the end at (5, 0, 0).
    ///
    ///     Was 11, the far side.
    ...
    ///     print(hit.distance)       // 92.
    ///
    ///     Was nil: no extremum exists past the end.
    ///     print(hit.solutionCount)  // 0 (no perpendicular foot, not a failure).
    /// }.
    /// ```.
    ///
    /// Before #580 this reported the smallest of the BRepExtrema_ExtPC\'s extrema, which excludes the.
    ...
    public struct FreeBoundInfo: Sendable {
        ...
        /// it sits precisely on the boundary between the two branches.
        ///
        /// So 0 means "not solvable.
    ...
    ///
    /// Both shapes must be meshed (have triangulation).
    ///
    /// This is faster than exact.
    ...
    /// was asked for.
    ///
    /// Both used to report (0,0,0), which is indistinguishable from a real centroid.
    /// at the origin (#609).
    ///
    /// `errorReached` is `BRepGProp_VinertGK::GetErrorReached()`.
    ///
    /// It used to be hardcoded to `0.0`.
    /// on every call. `GetErrorReached()` is defined inline in the OCCT header, which is why it has.
    /// no linkable symbol, not evidence it is unusable (#732).
    ///
    /// There is no paired absolute-error.
    ...
    /// small.
    ///
    /// Driving a genuinely curved face's mass toward zero, using a half-cylinder lateral face.
    ...
    ///
    /// The only realistic way to force literal-zero mass is a face whose.
    ...
    /// shrinks.
    ///
    /// It grows, staying finite and non-negative, rather than quietly staying small or.
    /// misreporting high confidence right where a caller most needs a reliable number.
    ///
    /// Treat a large.
    ...
    ///
    /// Nil for a face, wire, edge, vertex or open shell.
    ///
    /// It used to return the shape's **location.
    ...
    /// could recognise it (#609).
    ///
    /// For a sheet body use `surfaceInertia`; for a wire or edge use.
...
extension Shape {
    ...
    ///   - deflection: Linear mesh deflection (mm) for the detection triangulation.
    ///
    ///   Default `0.1`.
...
extension Shape {
    ...
    ///   be evaluated, or the tangent is undefined there.
    ///
    ///   That last case used to be 0, which is.
    ///   also a straight edge's real curvature (#595).
    ///
    ///   It is not exotic: a sphere carries a
    ...
    /// no normal, and neither does a cusp.
    ///
    /// Before #529 both cases returned `(0, 0, 0)`, which is.
    /// not a direction.
    ///
    /// ```swift.
    /// let arc = Curve3D.circle(center: .zero, normal: SIMD3(0, 0, 1), radius: 4)!
...
extension Shape {
    ...
    /// on the curvature gate, so it is still reported at a cone apex or a sphere pole.
    ///
    /// Before #583.
    ...
    /// own right: it is the maximum curvature at every point of a cylinder or a cone.
    ///
    /// Before #583.
    ...
    ///
    /// Before #583 a cone apex answered false, claiming the two.
...
extension Shape {
    ...
    ///
    /// Nil for a shape with no edges, such as a lone vertex.
    ///
    /// The centre of mass reported there was.
    ...
    ///   **disagrees with ``Shape/edgeArcLength``**, which measures 40.639742.
    ///
    ///   Before #603 both.
    ///   were wrong together.
    ///
    ///   Use ``Shape/edgeArcLength`` or ``Wire/length`` when you want the.
    ...
    ///
    /// Nil for a shape with no closed volume.
    ///
    /// It used to return three orthonormal unit vectors.
...
extension Shape {
    ...
    /// type exists for callers that specifically want the half-sizes as individual Doubles.
    ...
    /// three separate Doubles instead of one packed `SIMD3<Double>`, and has no volume,.
    /// dimensions or corners equivalent.
    ///
    /// Use ``orientedBoundingBox(optimal:)`` unless you

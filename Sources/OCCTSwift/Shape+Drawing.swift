...
extension Shape {
    ...
    public func normalProjection(
        ...
        guard
            let h = OCCTShapeNormalProjection(
                wireOrEdge.handle, handle,
    ...
    public static func projectWire(
        _ wire: Shape, onto target: Shape,
        direction: SIMD3<Double>
    ) -> Shape? {
        guard
            let h = OCCTShapeProjectWire(
                wire.handle, target.handle,
    ...
    public static func projectWireConical(
        ...
        guard
            let h = OCCTShapeProjectWireConical(
                wire.handle, target.handle,
                eye.x, eye.y, eye.z)
        else { return nil }
        return Shape(handle: h)
    ...
    public func reflectLines(
        normal: SIMD3<Double>, viewPoint: SIMD3<Double>,
        up: SIMD3<Double>
    ) -> Shape? {
        guard
            let h = OCCTHLRReflectLines(
                handle,
    ...
    public func reflectLinesFiltered(
        normal: SIMD3<Double>, viewPoint: SIMD3<Double>,
        up: SIMD3<Double>, edgeType: HLREdgeType,
        visible: Bool, in3d: Bool
    ) -> Shape? {
        guard
            let h = OCCTHLRReflectLinesFiltered(
                handle,
                normal.x, normal.y, normal.z,
                viewPoint.x, viewPoint.y, viewPoint.z,
                up.x, up.y, up.z,

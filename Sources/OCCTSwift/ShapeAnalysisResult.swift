...
public struct ShapeAnalysisResult {
    ...
    /// ``Shape/isSelfIntersecting(timeout:)``'s own nil = indeterminate).
    ///
    /// Either way, treat.
    ...
    /// (the BOPAlgo_ArgumentAnalyzer\'s self-interference test, #319) is orders of magnitude more.
    ...
    /// cost of the rest of the scan combined (a few ms vs 30s at a 30s timeout).
    ///
    /// On ordinary.
    /// shapes (a primitive, a several-boolean-and-fillet part, a 662-face mesh-sewn import) the.
    /// measured overhead was 1x-3x, cheap enough to opt into freely.
    ///
    /// See.
    ...
    /// always 0, never computed).
    ///
    /// Non-nil is reachable on a real path (not an always-closed.
    ...
    ///
    /// Via ShapeAnalysis_Shell.
    ///
    /// Before #702 this was hardcoded to 0 for every shape: the bridge
    ...
    /// closed.
    ///
    /// Same #702 fix as `freeEdgeCount`, since both come from the same per-shell scan.
    ///
    /// Not counted in `totalProblems`: this is a derived summary of the same scan, not an
    /// independent defect.
    ///
    /// It is never nonzero unless `freeEdgeCount` is also nonzero (a shell.

...
///   through (100,200,300) sampled at parameter 5.
///
///   Refusing would discard a right answer.
/// - **An input OCCT rejects** has no answer.
///
/// Both curve members build a gp_Dir from caller data,.
///   and that throws on a zero-length vector: coincident endpoints for a segment, a zero normal for
///   an arc.
///
///   Those return nil rather than a length of 0 with a centre of (0,0,0).
///
...
public enum GeometryProperties {
    ...
    ///
    /// Nil means two coincident endpoints, which give no direction to build a line from.
    ///
    /// That used.
    ...
    ///
    /// An empty set has no centroid.
    ///
    /// This used to report (0,0,0), which no caller could tell apart.
...
extension GeometryProperties {
    ...
    ///
    /// Returns (totalMass, centroid), with a nil centroid.
    ...
    /// that point.
    ///
    /// The result used to come back as mass 0 with a centroid of (0,0,0), which reads.

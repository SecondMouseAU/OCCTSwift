...
///
/// Backs the ConstructionContext\'s plane/axis/point storage.
...
internal final class EntityStore<ID: ConstructionEntityID, Value: Sendable>: @unchecked Sendable {
    ...
    ///   second round).
    ///
    ///   Every consumer of this either passes the array through opaquely or.
...
public final class ConstructionContext: @unchecked Sendable {
    ...
    ///
    /// EntityStore lock, not this one.
    ///
    /// That's a deliberate narrowing from the pre-#886.
    ...
    /// was never the problem the earlier version of this comment reasoned about).
    ///
    /// The motivating.
    ...
    /// single-kind drain).
    ///
    /// This lock is still the right thing to hold here: it keeps `remove()`
    ...
    /// that could make it observable.
    ///
    /// What it does **not**, and cannot, do: make an external
    ...
    /// already reflecting a concurrent `removeAll()`/`remove()` while others don't.
    ///
    /// Both callers.
    ...
    /// outside this lock.
    ///
    /// Neither `BRepGraph.resolve` nor `Document.addConstructionShape` nor any.
    ///
    ...
    ///   second round).
    ///
    ///   The inner per-entity element tuples keep their `(id:, name:, value:)`
...
extension Document {
    ...
    ///
    /// Lazy-associated; created on first access.
    ///
    /// Construction entities added to the context live.
    /// alongside the document's shapes but are not part of the XDE shape tree, they're pure.
    ///
    /// Swift-side recipes.
    ///
    /// For persistence guidance see ConstructionContext doc comments.
    ///
    ...
    ///
    /// Called from `Document.deinit`.
    ///
    /// This is not optional bookkeeping: the association is keyed
    /// on ObjectIdentifier, which is the instance pointer and is therefore only unique among.
    /// *live* objects.
    ///
    /// If the entry outlives the document, the next Document allocated at the.
...
///   among **live** objects.
///
///   Owners MUST call ``clear(for:)`` from their deinit.
///
///   An entry that.
///   outlives its owner is not merely a leak, the allocator readily hands the same address to the.
///   next instance, which then resolves to the dead owner's value and inherits its state.
///
///   This is.

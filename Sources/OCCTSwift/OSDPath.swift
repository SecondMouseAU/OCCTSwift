...
///
/// The single path-parsing surface in OCCTSwift since #499.
///
/// The TDocStd_PathParser-backed.
///
...
public enum OSDPath {
    ...
    /// becomes `^`.
    ///
    /// This is the OSD_Path\'s system-independent directory representation, not a filesystem.
    /// path, so do not hand it to FileManager.
    ///
    /// Use ``folder(_:)`` for a usable directory.
    ///
    /// ```swift.
    ///
    ...
    /// A syntax check only.
    ///
    /// It does not test whether the path exists, and in practice.
    ///

...
///
/// Returns values unchanged if it has exactly 12 elements, nil otherwise.
///
/// Extracted so the.
...
///
/// Returned by the TransformFactory3D\'s mirror/rotation/scale/translation builders, and accepted by.
...
///   three translation entries last: `[r00...r22, tx, ty, tz]`).
///
///   Before #835 all three Shape.
///   transform methods took a plain `[Double]`, so a caller could feed one method's array shape.
///   to another and silently get a garbled rotation/translation split, with no error.
///
///   These two.
...
public struct TransformMatrix3D: Sendable {
    ...
    ///   handle gracefully.
    ///
    ///   See the review finding on PR #870.
...
///   `[r00,r01,r02,tx, r10,r11,r12,ty, r20,r21,r22,tz]`).
///
///   Before #835 all three Shape transform.
///   methods took a plain `[Double]`, so a caller could feed one method's array shape to another.
///   and silently get a garbled rotation/translation split, with no error.
///
///   These two distinct.

...
/// the `Harnesses` target links `OCCTSwift` normally and cannot see an internal type. It labels
/// its own row "the form that shipped", so a change here that is not mirrored there leaves the
/// harness reporting a number for something else under that name. Keep the two in step.
struct CoveringEdges {
    ...
    /// term. Two `IsSame` shapes therefore always land in the same bucket, so a bucket miss is a
    /// definite non-match. The mask can only ADD collisions, never remove a match, which is why
    ...
    /// shape rather than in the graph's. `IsSame` is the identity the wire-to-shape conversion

...
public final class ShapeFixer: @unchecked Sendable {
    ...
    ///
    /// the ShapeFix_Shape\'s own header documents only these DONEi flags (it never assigns).
    ///
    ...
    ///   specific DONEi/FAILi sub-flag fired.
    ///
    ///   Prefer the `status(_ status: ShapeFixStatus)`
    ///   overload above, which exposes the full ShapeExtend_Status flag space.
    ///   `ShapeFix_Shape::Status` actually reports (#849).
    ///
    ///   Kept, unchanged, for source.
    ///   compatibility.

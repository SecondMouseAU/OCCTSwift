...
/// uses.
///
/// Every radius must be positive, and the parameters must lie in `0...1` and strictly.
...
/// ```.
///
/// > Note: OCCT stretches the law across the whole edge.
///
/// With one or two points the parameters are.
...
public struct EvolvingFilletEdge: Sendable {
    ...
    ///
    /// This was 1-based until #520, the one edge index in the fillet family that was.
    ///
    /// It now.
    ...
    /// diagnostic, so the spelling was retired rather than reinterpreted.
    ///
    /// Build the spec from the.
    ///

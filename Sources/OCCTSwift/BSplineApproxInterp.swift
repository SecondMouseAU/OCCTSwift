...
///
/// Fits a B-spline curve to the points, minimising the 3D deviation.
///
/// Inspect.
/// `maxError` for the worst-case residual.
///
/// > Note: OCCT 8.0.0p1 removed the Approx_BSplineApproxInterp solver this type
/// > originally wrapped, so it is now backed by GeomAPI_PointsToBSpline.
///
/// The API is kept.
...
/// > and ``performOptimal(maxIterations:)`` is identical to ``perform()``.
///
/// The two knobs.
...
public final class BSplineApproxInterp: @unchecked Sendable {
    ...
    ///
    /// > Note: No-op since OCCT 8.0.0p1.
    ///
    /// GeomAPI_PointsToBSpline has no per-point exact
    /// > interpolation or C0-break control.
    ///
    /// The approximation still passes near every point.
    /// - Parameters:
    ...
    /// Perform the fit.
    ///
    /// Identical to ``perform()``: GeomAPI_PointsToBSpline has no
    ...
    /// No-op.
    ///
    /// Originally set the parametrization power (0=uniform, 0.5=centripetal,.
    /// 1=chord-length).
    ///
    /// GeomAPI_PointsToBSpline takes an Approx_ParametrizationType.
    ...
    /// No-op.
    ///
    /// Originally set the minimum pivot value for the Gauss solver (default 1e-20).
    ///
    ...
    /// No-op.
    ///
    /// Originally set the relative tolerance for closed-curve detection.
    /// (default 1e-12).
    ///
    ...
    /// No-op.
    ///
    /// Originally set the tolerance for knot insertion during kink handling.
    ...
    /// Sets the 3D fit tolerance (default 1e-3).
    ///
    ...
    /// Tightens the 3D fit tolerance to `min(current, value)` (default 1e-6).
    ///
    /// Values `<= 0`.
    /// are ignored.
    ///
    /// This shares one tolerance with ``setConvergenceTolerance(_:)``, so the two

...
extension Document.DimensionType {
    ...
    ///
    /// Asked of OCCT's own `XCAFDimTolObjects_DimensionObject::IsDimensionalSize`.
    ///
    /// Neither.
...
extension Document {
    ...
    /// than standing in for an absent answer. `HasQualifier()` is exactly `!= .none`.
    ///
    /// Spell the.
    ...
    ///
    /// `.none` is a linear zone width.
    ///
    /// The other two make the same number a diameter, so reading.
    ...
    public struct GeomTolerance: Sendable, Hashable {
        ...
        ///
        /// nil when OCCT stored none.
        ///
        /// It stores this only when positive, so a zero is not.
    ...
    public struct Datum: Sendable, Hashable {
        ...
        /// `.rectangle`.
        ///
        /// An `.area` target keeps neither, because OCCT stores its own shape.
        /// instead of a placement.
        ///
        /// Measured per type in `Scripts/repro/1004-gdt-accessors/`.
    ...
    /// sizes a buffer from that report instead of guessing.
    ///
    /// The first call uses a 64-element buffer.

...
// SelfInterMode for self-intersection; HelixBRep_BuilderHelix for the helices) with every number printed at
// %.17g, so each can be compared with what the test observed.
...
int main()
{
  ...
  printf("curved spine, rect 5x3: enum frenet=%.17g enum corrected=%.17g oracle frenet=%.17g oracle corrected=%.17g\n",
  ...
  printf("straight spine, circle r2: frenet=%.17g corrected=%.17g\n", vol(viaEnum(straight, c2, true)),
...

  {
    ...
    printf("inflection spine: minCurvature=%.17g frenetSelfIntersects=%d correctedSelfIntersects=%d\n", kmin,
...

  {
    ...
    printf("spring (placed at measured start (%.17g, %.17g, %.17g)): enum frenet=%.17g enum corrected=%.17g oracle frenet=%.17g "
           "oracle corrected=%.17g textbook=%.17g\n",
  ...
  for (const auto& pv : cases)
  {
    ...
    double       tb   = M_PI * wr * wr * turns * std::sqrt(std::pow(2 * M_PI * r, 2) + pitch * pitch);
    printf("misplaced pitch=%g: frenet/textbook=%.17g corrected/textbook=%.17g (issue's %.3f)\n", pitch,

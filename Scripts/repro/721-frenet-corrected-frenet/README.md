...
rad discontinuity (pitch=30 shows the same effect, -0.3176 -> 0.0000), reproduced at every pitch
tested, not floating-point noise (compare `GeomFill_Frenet`'s own `N.axis`, which is continuous
(0.0000 -> 0.0000, both ends the same point mod one full turn) across the same boundary).

// Epic #766 evidence correction for PR #2671. probe.mm printed the removed volume at %.6f and the
...
static void drill(const char* label, const TopoDS_Shape& s, double oz, int mode, double p0 = 0, double p1 = 0)
{
  ...
  if (h.Status() == BRepFeat_NoError)
  {
    ...
    printf(" removed=%.17g solids=%d valid=%s", vol(s) - vol(r), solids(r),
           BRepCheck_Analyzer(r).IsValid() ? "true" : "false");
  }
  printf("\n");
}
...
int main()
{
  ...
  printf("bore(r5 x 20)=%.17g 2*bore=%.17g 3*bore=%.17g blind15=%.17g\n", bore, 2 * bore, 3 * bore,
         M_PI * 25 * 15);
  ...
  printf("singleSolid expected=%.17g\n", (M_PI * 25 - 2 * seg) * 20);

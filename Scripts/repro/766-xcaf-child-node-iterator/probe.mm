...
// ChildNodeIteratorTests: OCCTChildNodeIteratorCount on tag 400 of a fresh document.
int main()
{
  Handle(TDocStd_Application) app;
  Handle(TDocStd_Document)    d = newDoc(app);
  TDF_Label                   l = tagLabel(d, 400);
  Handle(TDataStd_TreeNode)   node;
  bool has = l.FindAttribute(TDataStd_TreeNode::GetDefaultTreeID(), node);
  printf("tag 400 has default TreeNode=%s\n", tf(has));
  int count = 0;
  if (has)
    for (TDataStd_ChildNodeIterator it(node, false); it.More(); it.Next())
      count++;

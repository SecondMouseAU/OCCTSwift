...
callers in one pass, and zero warnings landed in `Sources/`, confirming zero internal production
...
- `AAG.buildGraph()` was `OCCTFacesAreAdjacent`'s *only* Swift caller, and it no longer calls it:
  ...
  and the first edge (for convexity) anyway, so the "full walk" cost was already being paid by that
...
|---|---|---|
| `BRepGraph.GraphUID/GraphRefUID/GraphItemUID.init(...)`, `.generation`, `TopologyGraph` | #295/#303/#333 | v1.12.0 to v1.15.0 (real releases, weeks old) |
...
| Every other row (42 Swift symbols plus the bridge one) | #486/#490/#495/#498/#499/#503/#505/#536/#562/#651/#485, #783 | **Unreleased**, added and deprecated within this same branch's own refactor work, never in a tagged release |
...
| 53 | ThreadFeatures.swift | `ThreadBuild.boolean` (case) | message: use `.auto`/`.direct`, no longer differs for buildable threads, forced cut path is faceted/unreliable | **Remove** | v1.8.1, released (over a month old, #254). Verified the claim directly rather than trusting the message: `threadedShaft`'s `build:` parameter is accepted but never read anywhere in the function body; every mode already takes the identical code path. `Issue254BuildModesTests.swift` was initially deleted outright on the reasoning that its whole purpose was proving `.boolean` matched `.direct`, but that was wrong: `allModesAreDirect()` asserted three things, and only the third (`fBool == fDirect`) concerned `.boolean`. The first two, that a direct build is a smooth low-face helix (`fDirect < 40`, against the faceted cut's ~893) and that `.auto` resolves to `.direct` for a single start (`fAuto == fDirect`), are the substance of #254 and were about surviving API; nothing else in the tree covers them (`Issue189ThreadGuardTests.swift`, the only remaining file mentioning `ThreadBuild`, tests thread building and worm-pitch rejection, not this equivalence). Corrected: the file is kept, renamed to `autoMatchesDirect()`/`Issue254BuildModes` reflecting what it now proves, with the `.boolean` shaft, its helper, the third expectation, and the `.boolean`-only `booleanIsInEnvelope()` test (whose subject no longer exists, and whose remaining claim, that the direct build's crest is in-envelope, is already covered by `Issue222EnvelopeTests.swift`) all removed. |
...
or `Scripts/repro/censuses/`, none in `Sources/`. That is the strongest form of evidence this task

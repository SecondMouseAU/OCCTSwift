...
All four transcripts have their absolute paths normalised and say so in their own first lines,
because the originals named an agent worktree that no longer exists. Three use `<repo>` and
`<work>`; `url-consumer.txt` has no repo path in it and uses only `<work>`.

Two further measurements the nine build rows do not cover:

- `warnings-and-stdlib.txt`, regenerated by `measure-warnings.sh`, carries the deprecation figures
...
- **A consumer's own `.m` can reach this package's C bridge**, which matters because it is the
  route the reporting project was one `#import` away from. Measured with a consumer package whose
  Objective-C target does `#import "OCCTBridge.h"` and calls `OCCTShapeCreateBox`,
  `OCCTShapeGetVolume` and `OCCTShapeRelease`: it compiles, links and runs, printing
  `23.999999999999996` for a 2x3x4 box. `OCCTBridge` is not a product and its C surface carries no
  compatibility promise, so this is a route to understand rather than to recommend, and
  `docs/architecture/overview.md` said it was impossible until this measurement.

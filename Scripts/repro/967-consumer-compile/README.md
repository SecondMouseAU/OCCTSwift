...
because the originals named an agent worktree that no longer exists. Which placeholders each uses
depends on what it contains: `<repo>` and `<work>` where both appear, `<work>` alone in
`url-consumer.txt` and `bridge-reach.txt`, `<repo>` alone in `warnings-and-stdlib.txt`.
...
- `bridge-reach.txt`, regenerated by `measure-bridge-reach.sh`, answers how far a consumer can

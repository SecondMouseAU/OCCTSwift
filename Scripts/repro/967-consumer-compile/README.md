...
Captured output is in `swiftpm-rows.txt`. Proved able to fail: with row 1's expectation flipped from
`type_traits` to `pass` the run reports `MISMATCH` and exits 1, and restored it reports 5 of 5 and
exits 0.

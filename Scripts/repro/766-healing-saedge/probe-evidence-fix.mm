// PR #2487 evidence fix: probe.mm unchanged except that every double is printed at %.17g
// (maxdev, the vertex tolerances, the overlap tolerance, BoundUV, the end tangent and the vertices),
// so each per-test record can carry the kernel's value to the gate's 1e-9 tolerance. The three-digit
// %.3e of the original cannot show, for instance, that 1.000e-07 is 1e-07.
...
int main()
{
  ...
  printf("CheckSameParameter(e0)=%d maxdev=%.17g\n", (int)sp, maxdev);
  ...
  printf("FirstVertex(e0)=(%.17g, %.17g, %.17g) LastVertex(e0)=(%.17g, %.17g, %.17g)\n", a.X(), a.Y(), a.Z(), b.X(), b.Y(), b.Z());
  double t1 = 0, t2 = 0;
  bool   vt = sae.CheckVertexTolerance(e0, f0, t1, t2);
  printf("CheckVertexTolerance(e0, f0)=%d toler1=%.17g toler2=%.17g\n", (int)vt, t1, t2);
  double ov = 1e-7;
  bool   o1 = sae.CheckOverlapping(e0, e1, ov, 0.0);
  printf("CheckOverlapping(e0, e1, 1e-7)=%d tol=%.17g\n", (int)o1, ov);
  ov      = 1e-7;
  bool o2 = sae.CheckOverlapping(e0, e0, ov, 0.0);
  printf("CheckOverlapping(e0, e0, 1e-7)=%d tol=%.17g\n", (int)o2, ov);
  gp_Pnt2d p1, p2;
  bool     bu = sae.BoundUV(fe, f0, p1, p2);
  printf("BoundUV(fe, f0)=%d first=(%.17g, %.17g) last=(%.17g, %.17g)\n", (int)bu, p1.X(), p1.Y(), p2.X(), p2.Y());
  gp_Pnt2d pos;
  gp_Vec2d tan;
  bool     et = sae.GetEndTangent2d(fe, f0, false, pos, tan);
  printf("GetEndTangent2d(fe, f0, start)=%d pos=(%.17g, %.17g) tangent=(%.17g, %.17g)\n", (int)et, pos.X(), pos.Y(), tan.X(), tan.Y());

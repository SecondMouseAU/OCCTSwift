...
// Issue1435DatumDocumentToolTableTests: AddDatum through XCAFDoc_DocumentTool::DimTolTool(Main())
// (what OCCTDocumentCreateDatum uses) lands under Main().FindChild(4); a DimTolTool set on Main()
// itself (the #1435 regression) does not.
int main()
{
  ...
  TDF_Tool::Entry(t->Label(), e);
  printf("DocumentTool::DimTolTool label=%s\n", e.ToCString());
  t->AddDatum();
  printf("after one AddDatum: Main().FindChild(4) children=%d\n", d->Main().FindChild(4, false).NbChildren());
  t->AddDatum();
  t->AddDatum();
  TDF_LabelSequence ls;
  t->GetDatumLabels(ls);
  printf("after three: children=%d GetDatumLabels=%d\n", d->Main().FindChild(4, false).NbChildren(), ls.Length());

  Handle(TDocStd_Application) app2;
  Handle(TDocStd_Document)    d2 = newDoc(app2);
  Handle(XCAFDoc_DimTolTool)  wrong = XCAFDoc_DimTolTool::Set(d2->Main());
  TDF_Tool::Entry(wrong->Label(), e);
  wrong->AddDatum();
  TDF_Label four = d2->Main().FindChild(4, false);
  printf("DimTolTool::Set(Main) label=%s; after AddDatum Main().FindChild(4) %s\n", e.ToCString(),
         four.IsNull() ? "does not exist" : (TCollection_AsciiString("has ") + four.NbChildren() + " children").ToCString());
  return 0;
}

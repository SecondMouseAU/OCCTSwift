...
// first argument in TopExp::MapShapes order (OCCTBooleanHistoryModified / ...Generated / ...IsDeleted).
...
static void faces(const char* label, BRepAlgoAPI_BuilderAlgo& op, const TopoDS_Shape& input)
{
  ...
  for (int i = 1; i <= m.Extent(); i++)
    printf(" [%d]=(%d,%d,%d)", i - 1, op.Modified(m(i)).Extent(), op.Generated(m(i)).Extent(), op.IsDeleted(m(i)));
  printf("\n");
}
...
int main()
{
  ...
  }
  {
    ...
    printf("intersectionWithFullHistory: done=%d volume=%.17g box1Volume=%.17g\n", op.IsDone(), volume(op.Shape()), volume(a));

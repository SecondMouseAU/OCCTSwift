...
struct SelfIntersectingProfileGuard263 {
    ...
    func extrudeRefusesSelfIntersecting() {
        guard let wire = Self.bowtieWire() else {
            Issue.record("fixture: bowtie wire")  // #766: was a silent `return`
            return
        }
    ...
    func healRefusesSelfIntersecting() {
        guard let wire = Self.bowtieWire(), let face = Shape.face(from: wire, planar: true) else {
            Issue.record("fixture: bowtie face")  // #766: was a silent `return`
            return
        }
    ...
    func cleanProfileStillWorks() throws {
        guard
            ...
            Issue.record("fixture: square wire")  // #766: was a silent `return`
            return
        }
        // #766: was `if let s = solid`, so a nil extrusion only failed the `!= nil` line.
        let s = try #require(Shape.extrude(profile: wire, direction: SIMD3(0, 0, 1), length: 1))
        #expect(s.isValidSolid)
        #expect(abs((s.volume ?? 0) - 1.0) < 1e-9)

...
// Epic #766: every fixture is unwrapped with `try #require` rather than `!`, so a construction
// that returns nil fails the test instead of trapping the whole run. The expected areas and
// volume are the values Scripts/repro/766-math-geometry-construction/probe.mm measures.
@Suite("Geometry Construction Tests")
struct GeometryConstructionTests {

    @Test("Create face from rectangular wire")
    func faceFromRectangle() throws {
        let rect = try #require(Wire.rectangle(width: 10, height: 5))
    ...
    func faceFromCircle() throws {
        let circle = try #require(Wire.circle(radius: 5))
        let face = try #require(Shape.face(from: circle))
    ...
    func faceWithHole() throws {
        let outer = try #require(Wire.rectangle(width: 20, height: 20))
        let hole = try #require(Wire.circle(radius: 5))

        let face = try #require(Shape.face(outer: outer, holes: [hole]))
    ...
    func faceWithMultipleHoles() throws {
        let outer = try #require(Wire.rectangle(width: 30, height: 30))
        // Use offset3D to position the holes
        let hole1 = try #require(
            Wire.circle(radius: 3)?.offset3D(distance: 8, direction: SIMD3(-1, 0, 0)))
        let hole2 = try #require(
            Wire.circle(radius: 3)?.offset3D(distance: 8, direction: SIMD3(1, 0, 0)))

        let face = try #require(Shape.face(outer: outer, holes: [hole1, hole2]))
    ...
    func extrudeFace() throws {
        ...
        let solid = try #require(Shape.extrude(profile: rect, direction: SIMD3(0, 0, 1), length: 3))

...
struct Issue187ScrewThreadTests {
    ...
    @Test(
        "threadedShaft cuts a TIGHT in-envelope thread (crest ~= nominal radius)",
        arguments: [
            (6.0, 1.0),  // M6
            (8.0, 1.25),  // M8
            (10.0, 1.5),  // M10
    ...
    func tightInEnvelopeThread(nominal: Double, pitch: Double) {
        ...
        guard let b = shank.boundingBoxOptimal(), let c = threaded.boundingBoxOptimal() else {
            Issue.record("no optimal bounds")
            return
        }
        let tol = 0.25
    ...
    func deterministic() {
        guard let shank = Shape.cylinder(radius: 3, height: 20) else {
            Issue.record("no shank")
            return
        }
    ...
    func smoothFewFaces() {
        guard let shank = Shape.cylinder(radius: 6, height: 16) else {
            Issue.record("no shank")
            return
        }
        ...
        guard
            ...
            Issue.record("threadedShaft nil")
            return
        }

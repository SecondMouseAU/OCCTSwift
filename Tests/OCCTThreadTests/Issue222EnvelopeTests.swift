...
struct Issue222Envelope {
    ...
    func directInEnvelopeISO() {
        ...
        let threaded = rod.threadedShaft(
            axisOrigin: .zero, axisDirection: SIMD3(0, 0, 1),
            spec: spec, length: 40, build: .direct)
        guard let t = threaded else {
            #expect(Bool(false), "direct build returned nil")
            return
        }
        #expect(t.isValid)
    ...
    func autoStillBuilds() {
        guard let rod = Shape.cylinder(radius: 5, height: 20) else {
            #expect(Bool(false))
            return
        }
        ...
        if let t = threaded {
            ...
            #expect(Bool(false), "auto build returned nil")
        }
    }
}

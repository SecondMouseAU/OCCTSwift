...
struct Geom2dEvalSineWaveTests {
    ...
    @Test func sineWave2DD0AtZero() throws {
        let p = try #require(Geom2dEval.sineWaveD0(amplitude: 1.5, omega: 2.0, phase: 0.0, u: 0.0))
        #expect(abs(p.x) < 1e-10)
        #expect(abs(p.y) < 1e-10)
    }

    @Test func sineWave2DD0Peak() throws {
        let omega = 2.0
    ...
    @Test func sineWave2DD1() throws {
        let r = try #require(Geom2dEval.sineWaveD1(amplitude: 1.5, omega: 2.0, phase: 0.0, u: 0.0))

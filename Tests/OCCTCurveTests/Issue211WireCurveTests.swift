...
struct Issue211WireCurve {
    ...
    func length() {
        guard let w = lWire(), let wc = WireCurve(w) else {
            #expect(Bool(false))
            return
        }
    ...
    func tangentAcrossCorner() {
        ...
        if let t1 = wc.tangent(atAbscissa: 5) {
            #expect(simd_distance(t1, SIMD3(1, 0, 0)) < 1e-6)
        } else {
            #expect(Bool(false), "tangent on first edge nil")
        }
        if let t2 = wc.tangent(atAbscissa: 15) {
            #expect(simd_distance(t2, SIMD3(0, 1, 0)) < 1e-6)
        } else {
            #expect(Bool(false), "tangent on second edge nil")
        }
    }
...
struct Issue211EdgeCurve {
    ...
    func lengthAndSampling() {
        guard let e = anEdge(), let ec = EdgeCurve(e) else {
            #expect(Bool(false))
            return
        }
    ...
    func parameterRangeMatchesEndpoints() {
        ...
        guard let start = ec.point(atParameter: range.first),
            ...
            #expect(Bool(false), "point(atParameter:)/point(atAbscissa:) nil")
            return
        }

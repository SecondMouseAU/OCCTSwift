...
struct Issue603SingleSpanQuadratureTests {
    ...
    private func ellipseArc(
        a: Double, b: Double, from t0: Double, to t1: Double,
        panels n: Int = 200_000
    ) -> Double {
    ...
    private func ellipse(_ a: Double, _ b: Double) -> Curve3D? {
        Curve3D.ellipse(
            center: SIMD3(0, 0, 0), normal: SIMD3(0, 0, 1), majorRadius: a, minorRadius: b)
    }
    ...
    func wholeEllipseLength() {
        let cases: [(Double, Double, Double)] = [
            (8, 3, 0.00337), (8, 7, 0.0000096), (10, 1, 0.01485), (100, 30, 0.00550),
            (1, 0.05, 0.01737),
        ]
        for (a, b, oldRelativeError) in cases {
            ...
            #expect(
                abs(chord - truth) / truth < 1e-9,
                "references disagree for \(a) x \(b): \(chord) vs \(truth)")
            #expect(
                abs(measured - truth) / truth < 1e-9,
    ...
    func wideParabolaLength() {
        guard
            let c = Curve3D.parabola(
                center: SIMD3(0, 0, 0), normal: SIMD3(0, 0, 1),
                focal: 3),
            let measured = c.length(from: -100, to: 100)
        else {
            Issue.record("parabola failed to build or measure")
            return
        }
    ...
    func wideHyperbolaLength() {
        guard
            let c = Curve3D.hyperbola(
                center: SIMD3(0, 0, 0), normal: SIMD3(0, 0, 1),
                majorRadius: 5, minorRadius: 2),
            let measured = c.length(from: -4, to: 4)
        else {
            Issue.record("hyperbola failed to build or measure")
            return
        }
    ...
    func whippingBezierLength() {
        guard
            let c = Curve3D.bezier(poles: [
                SIMD3(0, 0, 0), SIMD3(0.2, 40, 0),
                SIMD3(9.8, -40, 0), SIMD3(10, 0, 0),
            ]),
            let measured = c.length
        else {
            Issue.record("Bezier failed to build or measure")
            return
        }
    ...
    func lengthParametrizedCurvesUnchanged() {
        guard
            ...
            let straight = line.length
        else {
            Issue.record("closed-form fixtures failed")
            return
        }
    ...
    func inverseAgreesWithTheLengthItInverts() {
        for (a, b) in [(8.0, 3.0), (10.0, 1.0), (1.0, 0.05)] {
            guard let c = ellipse(a, b), let total = c.length else {
                Issue.record("ellipse \(a) x \(b) failed")
                continue
            }
            let end = c.parameterAtLength(total, from: c.domain.lowerBound)
            #expect(
                abs(end - c.domain.upperBound) < 1e-7,
    ...
    func inverseIsAccurateMidCurve() {
        ...
        for fraction in [0.25, 0.5, 0.75] {
            ...
            #expect(
                abs(arc - total * fraction) / total < 1e-8,
    ...
    func curve2DAgrees() {
        guard let c = Curve2D.ellipse(center: SIMD2(0, 0), majorRadius: 8, minorRadius: 3),
            ...
            Issue.record("2D ellipse failed")
            return
        }
    ...
    func edgeAgrees() {
        guard let c = ellipse(10, 1), let curveLength = c.length,
            let edge = Shape.edgeFromCurve(c)
        else {
            Issue.record("edge fixture failed")
            return
        }
        let truth = ellipseArc(a: 10, b: 1, from: 0, to: 2 * .pi)
        #expect(
            abs(edge.edgeArcLength - truth) / truth < 1e-9,
            "edgeArcLength \(edge.edgeArcLength) vs \(truth)")
        #expect(
            abs(edge.edgeArcLength - curveLength) / truth < 1e-9,
        ...
        #expect(
            abs(end - domain.upperBound) < 1e-7,
        ...
        #expect(
            abs(firstHalf - truth / 2) / truth < 1e-8,
    ...
    func wireAgrees() {
        guard let c = ellipse(10, 1), let edge = Shape.edgeFromCurve(c),
            let e = edge.edges().first, let wire = Wire.wireFromEdges([e])
        else {
            Issue.record("wire fixture failed")
            return
        }
        let truth = ellipseArc(a: 10, b: 1, from: 0, to: 2 * .pi)
    ...
    func edgeCurveAgrees() {
        guard let c = ellipse(10, 1), let shape = Shape.edgeFromCurve(c),
            let edge = shape.edges().first, let ec = EdgeCurve(edge)
        else {
            Issue.record("EdgeCurve fixture failed")
            return
        }
        let truth = ellipseArc(a: 10, b: 1, from: 0, to: 2 * .pi)
        ...
        if let end = ec.parameter(atAbscissa: ec.length) {
            #expect(
                abs(end - ec.parameterRange.last) < 1e-7,

...
struct GeomEvalSineWaveTests {
    ...
    @Test func sineWaveD0AtZero() throws {
        let p = try #require(GeomEval.sineWaveD0(amplitude: 2.0, omega: 3.0, phase: 0.0, u: 0.0))
        #expect(abs(p.x) < 1e-10)
        #expect(abs(p.y) < 1e-10)
    }

    @Test func sineWaveD0AtPiOver2() throws {
        // At t=pi/(2*omega): sin(omega*t) = sin(pi/2) = 1
        let omega = 3.0
        let t = .pi / (2.0 * omega)
        let p = try #require(GeomEval.sineWaveD0(amplitude: 2.0, omega: omega, phase: 0.0, u: t))
        #expect(abs(p.x - t) < 1e-10)
        #expect(abs(p.y - 2.0) < 1e-6)  // A*sin(pi/2) = A
    }

    @Test func sineWaveD1() throws {
        let r = try #require(GeomEval.sineWaveD1(amplitude: 2.0, omega: 3.0, phase: 0.0, u: 0.0))

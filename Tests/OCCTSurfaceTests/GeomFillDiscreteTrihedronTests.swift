...
struct GeomFillDiscreteTrihedronTests {
    ...
    func discreteTrihedronEdge() throws {
        // #766: this looped over edges until one answered and checked only |T| > 0.1, and its
        // guards returned silently. Edge 0 is the top circle; GeomFill_DiscreteTrihedron at 0
        // gives T (0, 1, 0), N (-1, 0, 0), B (0, 0, 1), see Scripts/repro/766-geomfill-a/.
        let cyl = try #require(Shape.cylinder(radius: 10, height: 5))
        let edge = try #require(cyl.subShapes(ofType: .edge).first)
        let frame = try #require(edge.discreteTrihedron(at: 0))

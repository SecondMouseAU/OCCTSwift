...
/// all three) answer "total surface area" through two different OCCT overloads.
///
/// The audit's own question was whether to unify `totalFaceArea` onto the aggregate call or keep
/// both, clearly documented; this repo kept both (see the PR body for the tradeoffs) and pinned
/// the measured behavior here so the decision stays evidenced rather than assumed.
...
struct Issue885TotalAreaDivergenceTests {
    ...
    /// `occtSurfaceMassProperties` bridge helper), so they should be bit-identical.
    ///
    /// This is the premise every doc comment that says "these three are one number,
    /// `totalFaceArea` is a different one" relies on.
    ...
    /// ordinary shape.
    ///
    /// This is what makes the divergence easy to miss in normal use.
    ...
    /// integration "perform non-adaptive integration" instead.
    ///
    /// Past that boundary, `Face.area(tolerance:)` -- and so `ShapeMeasurements.totalFaceArea` --
    /// can land on a meaningfully different number than the untunable aggregate, which never moves
    /// at all. Measured on this fixture: `totalFaceArea(linearTolerance: 0.5)` is ~1216.31 against
    /// the aggregate's fixed ~1256.64 (~3.2% apart) -- a real divergence, not floating-point noise.
    @Test(
        "loosening past the adaptive/non-adaptive boundary makes the two totals genuinely diverge")
    ...
    /// them takes a tolerance parameter.
    ///
    /// A caller "fixing" a totalFaceArea/surfaceArea mismatch by tightening `linearTolerance`
    /// makes the two disagree *more*, not less, on any shape where they were not already exactly
    /// equal -- tightening only ever moves this one number.

...
struct Issue274HoleWireOrientationTests {
    ...
    func holeAlreadyOppositeIsKept() {
        ...
        guard let face = Shape.face(outer: outer, holes: [hole]) else {
            Issue.record("face(outer:holes:) returned nil for opposite-wound hole")
            return
        }
        #expect(face.isValid)
        #expect(face.subShapeCount(ofType: .wire) == 2)  // outer + one real hole
        guard let area = face.surfaceArea else {
            Issue.record("no surfaceArea")
            return
        }
    ...
    func holeSameSenseIsReversed() {
        guard let outer = outerCCW(), let hole = holeCCW() else {
            Issue.record("setup")
            return
        }
        guard let face = Shape.face(outer: outer, holes: [hole]) else {
            Issue.record("face(outer:holes:) returned nil for same-wound hole")
            return
        }
        #expect(face.isValid)
    ...
    func bothWindingsAgree() {
        guard let outer1 = outerCCW(), let outer2 = outerCCW(),
            let hCW = holeCW(), let hCCW = holeCCW()
        else {
            Issue.record("setup")
            return
        }
        guard let faceFromCW = Shape.face(outer: outer1, holes: [hCW]),
            let faceFromCCW = Shape.face(outer: outer2, holes: [hCCW])
        else {
            Issue.record("one of the faces was nil")
            return
        }

...
struct Issue202BooleanOptions {
    ...
    func defaultParity() {
        ...
        if let u = a.union(b) {
            #expect(abs((u.volume ?? 0) - 2000.0) < 1.0)
        } else {
            #expect(Bool(false), "union(_:) returned nil")
        }

        guard let big = Shape.box(origin: SIMD3(0, 0, 0), width: 10, height: 10, depth: 10),
            let tool = Shape.box(origin: SIMD3(2, 2, 0), width: 6, height: 6, depth: 10)
        else {
            #expect(Bool(false))
            return
        }
        if let cut = big.subtracting(tool) {
            #expect(abs((cut.volume ?? 0) - (1000.0 - 360.0)) < 1.0)
        } else {
            #expect(Bool(false), "subtracting(_:) returned nil")
        }
    }
    ...
    func gluedUnion() {
        ...
        for glue in [Shape.BooleanGlue.shift, .full] {
            guard let u = a.union(b, fuzzyValue: 0, glue: glue) else {
                #expect(Bool(false), "glued union (\(glue)) returned nil")
                continue
            }
            #expect(u.isValid)
    ...
    func fuzzyThinWallSubtract() {
        guard let outer = Shape.box(origin: SIMD3(0, 0, 0), width: 10, height: 10, depth: 10),
            ...
            let inner = Shape.box(origin: SIMD3(1, 1, 0), width: 8, height: 8, depth: 10)
        else {
            #expect(Bool(false))
            return
        }
        guard let shell = outer.subtracting(inner, fuzzyValue: 1e-4, glue: .off) else {
            #expect(Bool(false), "fuzzy subtract returned nil")
            return
        }
    ...
    func fuzzyIntersection() {
        ...
        guard let x = a.intersection(b, fuzzyValue: 1e-4) else {
            #expect(Bool(false), "fuzzy intersection returned nil")
            return
        }
    ...
    func negativeFuzzyIgnored() {
        ...
        if let u = a.union(b, fuzzyValue: -5) {
            #expect(abs((u.volume ?? 0) - 2000.0) < 1.0)
        } else {
            #expect(Bool(false), "union with negative fuzzy returned nil")
        }
    }
}

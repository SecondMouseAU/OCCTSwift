...
struct Issue490FilletContinuityTests {
    ...
    func documentedDomainBuilds(continuity: Int) throws {
        let volume = try #require(
            filletedVolume(continuity: continuity),
    ...
    @Test(
        "past the documented domain the fillet still builds, it does not crash or fail",
        arguments: [3, 4, 99, -1])
...
struct Issue490ThruSectionsContinuityTests {
    ...
    @Test(
        "BRepOffsetAPI_ThruSections takes every value without failing",
    ...
    func everyValueLofts(continuity: Int) throws {
        ...
        let volume = try #require(
            loftedVolume(continuity: continuity),

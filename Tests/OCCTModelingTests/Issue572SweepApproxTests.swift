...
struct Issue572SweepApproxTests {
    ...
    static func cornerSpine() -> Shape? {
        ...
        guard let curve = Curve3D.bspline(poles: poles,
                                          knots: [0.0, 0.5, 1.0],
                                          multiplicities: [3, 2, 3],
                                          degree: 2) else { return nil }
    ...
    static func build(forceApproxC1: Bool) -> Shape? {
        guard let spine = cornerSpine(),
              ...
              let builder = PipeShellBuilder(spine: spine) else { return nil }
        ...
        for i in 0...samples {
            for j in 0...samples {
                ...
                guard let p = truth.point(atU: u, v: v),
                      let proj = candidate.project(point: p) else { continue }
    ...
    func forcedC1CoversTheUnforcedSweep() {
        guard let plain = Self.build(forceApproxC1: false),
              let forced = Self.build(forceApproxC1: true) else { Issue.record("build"); return }
        ...
        else { Issue.record("no bspline face"); return }
    ...
    func unforcedSweepIsUnchanged() {
        guard let plain = Self.build(forceApproxC1: false) else { Issue.record("build"); return }
        guard let face = plain.faces().first(where: { $0.surfaceType == .bsplineSurface }) else {
            Issue.record("no bspline face"); return
        }

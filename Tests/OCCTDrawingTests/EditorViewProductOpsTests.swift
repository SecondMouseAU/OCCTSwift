...
// #766: the first three tests used to nest their assertions in `if let box` / `if let graph`
// (and the first also in `if let linked`), so a nil graph or link skipped them and passed. They
// now record it, and the first pins the product/occurrence ids the kernel mints instead of `>= 0`.
...
struct EditorViewProductOpsTests {
    ...
    func createAndLinkProducts() {
        ...
        // Kernel ids: empty product 0, topology product 1, occurrence 1, occurrence ref 1
        // (Scripts/repro/766-drawing-editorview/transcript.txt).
        #expect(parentProduct == 0)
        #expect(childProduct == 1)
        #expect(linked.occurrenceIndex == 1)
        #expect(linked.occurrenceRefIndex == 1)
    }

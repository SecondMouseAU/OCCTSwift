...
| #1393 | `LocOpe_SplitDrafts::Perform`, via `GeomFill_Pipe` and `GeomConvert::CurveToBSplineCurve` | `Perform` accepts only a planar face and then pipes an infinite `Geom_Line` (the plane/plane intersection) along another infinite `Geom_Line` (the face normal). `CurveToBSplineCurve` refuses an infinite curve **by documented design**, so **every valid call threw** `Standard_DomainError("No such curve")` and `Shape.splitDrafts` could never succeed. The missing untrimmed-`Geom_Line` case is not the bug: the trimmed branch has always handled a line, and the caller is what hands an infinite one to an API that says it rejects them | patch `0034`, **unpinned**: trims both lines to the shape's bounding-box extent. **Not upstream-bound and never will be**, OCCT master deleted `LocOpe_SplitDrafts` outright in [OCCT#1442](https://github.com/Open-Cascade-SAS/OCCT/pull/1442) (dead code, no caller in the tree). Retires by deletion, not by a repin: a bump past that tag removes the class and `Shape.splitDrafts` with it | `Scripts/repro/1393-splitdrafts/` |

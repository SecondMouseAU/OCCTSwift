...
| #2748 | `Set3dCircleConversion(true)` with `Set3dConicConversion(false)` should convert a cylinder's circular edges to Bezier, since `circleMode` names exactly that curve kind | it cannot. `ShapeUpgrade_ConvertCurve3dToBezier::Compute()` skips a curve when `(IsKind(Geom_Conic) && !myConicMode) \|\| (IsKind(Geom_Circle) && !myCircleMode)`, read from a same-tag `V8_0_1` `occt-src` checkout. `Geom_Circle` is a subtype of `Geom_Conic`, so for any circle the first clause alone is true whenever `myConicMode` is false: circle conversion needs **both** modes true, and `circleMode` on its own can only exclude circles from an already-enabled conic pass, never independently include them. The cylinder's rim edges are bare `Geom_Circle` (not `Geom_TrimmedCurve`), so the trimmed-vs-free-standing framing in the issue was not the mechanism. Bridge and Swift wrapper pass both flags to their correctly-named setters; this is `ShapeUpgrade`'s own behavior. Documented on `Shape.convertCurves3dToBezier` and `docs/reference/Shape-Builders-2.md` instead of changed, per `okf/policies/scope-boundary.md` | `Scripts/repro/2748-bezier-circle-mode/` |

...
| #1393 | `GeomConvert::CurveToBSplineCurve`, reached through `GeomFill_Pipe` from `LocOpe_SplitDrafts::Perform` | the type chain has no `Geom_Line` case and its final `else` throws `Standard_DomainError("No such curve")`. `LocOpe_SplitDrafts` only accepts a planar face and then pipes along the intersection of two planes, which is always a line, so **every valid call throws** and `Shape.splitDrafts` can never succeed on this kernel | none yet; upstream fix queued (a line is a degree-1 B-spline). The bridge's `catch (...)` already turns it into `nil`, and no guard is added, since refusing up front would refuse the whole operation | `Scripts/repro/1393-splitdrafts/` |

...
| #1393 | `GeomConvert::CurveToBSplineCurve`, reached through `GeomFill_Pipe` from `LocOpe_SplitDrafts::Perform` | the untrimmed branch refuses an infinite curve by documented design, and `LocOpe_SplitDrafts` only accepts a planar face and then pipes along the intersection of two planes, which is always a line, so **every valid call threw** and `Shape.splitDrafts` could never succeed | **the wrapper is gone**. Upstream deleted `LocOpe_SplitDrafts` outright in [OCCT#1442](https://github.com/Open-Cascade-SAS/OCCT/pull/1442) (2026-08-07, dead code, no caller in the tree), so there was nothing to fix and nothing to carry: wrapping a class its own maintainer removed is not staying faithful to OCCT. Removed in v4.0.0, no patch carried | `Scripts/repro/1393-splitdrafts/` |
...
| #603 | `CPnts_AbscissaPoint::Length`, `CPnts_MyRootFunction::Value` | one fixed-order Gauss rule over the whole range: an ellipse 1.7% long, a parabola 3.1% short | patch `0021`, pinned; [OCCT#1420](https://github.com/Open-Cascade-SAS/OCCT/pull/1420) open. The bridge also subdivided (`occtArcConvergedLength`); **retired at the v4.0.0-kernel.1 repin**, measured inert across 6,384 tests. `BRepGProp::LinearProperties` has its own integrator and is still wrong | `Scripts/repro/603-single-span-quadrature/`, `Scripts/repro/1690-arc-quadrature-redundancy/` |
...
| #1022, #1030 | `XCAFDoc_Datum::GetObject` | reads the point's X from the annotation plane's array: wrong value with both, uncatchable SIGSEGV with a point and no plane. Reachable from all seven bridge datum functions via `occtDocumentDatumObjectAt` and from an OCAF load | patch `0029`, **pinned from v4.0.0-kernel.1**; [OCCT#1483](https://github.com/Open-Cascade-SAS/OCCT/pull/1483) open. Bridge guard #1030 refused the crashing shape and is **retired**, in all six files that carried it | `Scripts/repro/1022-datum-point-from-plane-array/`, `Scripts/repro/1030-datum-lookup-guard/` |
| #2746 | `BRepCheck_Edge::InContext` (`BRepCheck_Edge.cxx:488`) | unchecked `down_cast<GeomAdaptor_Curve>(myHCurve)` in the planar on-the-fly projection branch. When the edge has no valid 3D curve but has a pcurve, `Minimum()` falls back to a curve-on-surface for `myCref` and sets `myHCurve` to an `Adaptor3d_CurveOnSurface`, the cast yields null, `Gac->Curve()` dereferences it. Uncatchable SIGSEGV, and `Minimum()` reports `NoError` beforehand. Crashes for the one owning face whose pcurve became `myCref` and not the other, so it reads as intermittent. Reached from 19 bridge sites via `BRepCheck_Analyzer::Perform`, and a `.brep` round trip preserves the state | **nothing carried, nothing guarded**. Upstream `master` is byte-identical to the pin on this file and has no open PR or issue for it; the 8.0.2 repin is the next chance to re-survey. The bridge guard is #2750: refuse a shape with a **non-degenerated edge whose `BRep_Tool::Curve` is null and which has at least one curve-on-surface representation**. Not "has a null `Curve3D` representation": that predicate misses the state a `.brep` round trip produces, which crashes too | `Scripts/repro/2746-brepcheck-incontext-sigsegv/` |

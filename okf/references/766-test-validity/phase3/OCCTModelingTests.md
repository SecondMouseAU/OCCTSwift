...
| unionWithFullHistory | `OCCTBooleanHistoryIsDeleted` inverts the builder's IsDeleted answer, or `OCCTBooleanHistoryModified` (or `OCCTBooleanHistoryGenerated`) reports one spurious entry for each face, or `OCCTBooleanUnionWithHistory` returns nullptr | `:28 Expectation failed: !rec.isDeleted`; `:29 Expectation failed: rec.modified.count == 0` (spurious modified); `:30 Expectation failed: rec.generated.count == 0` (spurious generated); `:15 Expectation failed: box1.unionWithFullHistory(box2)` (nullptr) | pass | `OCCTBooleanHistoryIsDeleted` | PASS: strengthened: `modified.count <= 1` passed with an empty history and with one spurious entry per face, and only face 0 was read |
| subtractedWithFullHistorySplitsFace | `OCCTBooleanHistoryModified` or `OCCTBooleanHistoryGenerated` reports an empty list for every input, or `OCCTBooleanHistoryIsDeleted` returns false for every input, or `OCCTBooleanSubtractWithHistory` returns nullptr | `:66 Expectation failed: foundSplit`, `:77 Expectation failed: recs.map { $0.modified.count } == [1, 1, 0, 0, 1, 1]` (modified empty); `:78 Expectation failed: recs.map { $0.generated.count } == [1, 0, 0, 0, 1, 1]` (generated empty); `:79 Expectation failed: recs.map(\.isDeleted) == [false, false, false, true, false, false]` (never deleted); `:44 Expectation failed: big.subtractedWithFullHistory(tool)` (nullptr) | pass | `OCCTBooleanHistoryModified` | PASS: the slab cuts a 4 mm strip off the +Y side rather than bisecting the box; faces 0, 4, 5 reach modified+generated = 2 |
| intersectionWithFullHistory | `OCCTBooleanIntersectWithHistory` builds a BRepAlgoAPI_Fuse instead of a BRepAlgoAPI_Common, or `OCCTBooleanHistoryIsDeleted` returns false for every input, or `OCCTBooleanHistoryModified` / `OCCTBooleanHistoryGenerated` reports an empty list | `:91 Expectation failed: volume < box1Volume`, `:98 Expectation failed: abs(volume - 125) < 1e-6`, `:102 Expectation failed: recs.map(\.isDeleted) == [true, false, true, false, true, false]` (fuse); `:102` (never deleted); `:103 Expectation failed: recs.map { $0.modified.count } == [0, 1, 0, 1, 0, 1]`, `:104 Expectation failed: recs.map { $0.generated.count } == [0, 3, 0, 3, 0, 3]` (empty lists) | pass | `OCCTBooleanIntersectWithHistory` | PASS: strengthened: the history loop was `_ = r.history.record(...)`, so the title's "non-overlap region inputs are deleted" was never checked and a bridge with a wrong or empty history passed |
| splitWithFullHistory | `OCCTBooleanHistoryIsDeleted` inverts the builder's IsDeleted answer, or `OCCTShapeCompoundChildren` reports one piece, or `OCCTBooleanHistoryModified` / `OCCTBooleanHistoryGenerated` reports an empty list | `:133 Expectation failed: !rec.isDeleted`; `:125 Expectation failed: r.pieces.count == 2` (one piece); `:138 Expectation failed: recs.map { $0.modified.count } == [2, 2, 0, 1, 2, 2]`, `:139 Expectation failed: recs.map { $0.generated.count } == [1, 0, 0, 0, 1, 0]` (empty lists) | pass | `OCCTBooleanHistoryIsDeleted` | PASS: strengthened: `pieces.count >= 1` held for an unsplit box |
| historyHandleSurvives | `OCCTBooleanHistoryModified` and `OCCTBooleanHistoryGenerated` report an empty list for every input | `:165 Expectation failed: c1.modified.count == 1`, `:166 Expectation failed: c1.generated.count == 3` | pass | `OCCTBooleanHistoryModified` | PASS: strengthened: face 0 is untouched by box2, so its 0 == 0 comparisons held for an empty history too; face 1's counts are now pinned |

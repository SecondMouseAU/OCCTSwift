...
| draftPrismTwoHeights | `OCCTLocOpeDPrism` returns nullptr, or builds the single-height `LocOpe_DPrism(face, height1, angle)` | `:17 Expectation failed: face.draftPrism(height1: 5, height2: 3, angle: 0.1)` (nullptr); `:19 Expectation failed: result.faceCount == 10`, `:21 Expectation failed: abs(volume - 64.050646231928056) < 1e-6` (single-height) | pass | `OCCTLocOpeDPrism` | PASS: rewritten: it asserted only `!= nil`, so any shape passed |
| draftPrismSingleHeight | `OCCTLocOpeDPrismSingleHeight` returns nullptr, or builds the two-height `LocOpe_DPrism(face, height, 3, angle)` | `:32 Expectation failed: face.draftPrism(height: 5, angle: 0.1)` (nullptr); `:34 Expectation failed: result.faceCount == 6`, `:36 Expectation failed: abs(volume - 24.085995119264528) < 1e-6` (two-height) | pass | `OCCTLocOpeDPrismSingleHeight` | PASS: rewritten: it asserted only `!= nil`, so any shape passed |
| draftPrismHasFaces | `OCCTLocOpeDPrism` builds the single-height `LocOpe_DPrism(face, height1, angle)` instead of the two-height one, or returns nullptr | `:49 Expectation failed: result.faceCount == 10`; `:48 Expectation failed: face.draftPrism(height1: 5, height2: 3, angle: 0.1)` (nullptr) | pass | `OCCTLocOpeDPrism` | PASS: rewritten: the old `if let` skipped its only assertion on nil and accepted any count |
...
| glueByFace | `OCCTLocOpeGlue` returns nullptr; `OCCTShapeCreateBoxAt` returns nullptr, or a cylinder (3 faces) | `:27 Expectation failed: b1.locOpeGlue(b2, facePairs: [(base: faces1[1], glued: faces2[0])])` (glue nullptr); `:21 Expectation failed: Shape.box(origin: SIMD3(0, 0, 0), width: 10, height: 10, depth: 10)` (box nullptr); `:25 Expectation failed: faces1.count == 6` (cylinder) | pass | `OCCTLocOpeGlue` | PASS: rewritten: the search helper kept the non-touching pair (0,0), whose glue is invalid, and returned silently on nil; the `guard faces1.count == 6, faces2.count == 6 else { return }` that remained is now `try #require` |
### `LocOpeLinearFormTests.swift` (1 tests)
Probe: `Scripts/repro/766-modeling-locope-linear-form/`.
| linearForm | `OCCTLocOpeLinearForm` returns nullptr, or returns its input shape unswept | `:14 Expectation failed: face.localLinearForm(direction: SIMD3(0, 0, 10), from: SIMD3(0, 0, 0), to: SIMD3(0, 0, 10))` (nullptr); `:21 Expectation failed: result.faceCount == 12`, `:23 Expectation failed: abs(volume - 5.0) < 1e-6` (unswept) | pass | `OCCTLocOpeLinearForm` | PASS: rewritten: it asserted only `!= nil`, so any shape passed |
### `LocOpeSplitShapeTests.swift` (1 tests)
Probe: `Scripts/repro/766-modeling-locope-split-shape/`.
| splitEdge | `OCCTLocOpeSplitShapeByVertex` returns nullptr, or returns its input shape unsplit | `:17 Expectation failed: box.splitEdge(at: 0, parameter: 0.5)` (nullptr); `:18 Expectation failed: r.subShapes(ofType: .edge).count == 2`, `:20 Expectation failed: vertices.count == 3`, `:21 Expectation failed: vertices.contains { simd_distance($0, SIMD3(-5, -5, 0)) < 1e-9 }` (unsplit) | pass | `OCCTLocOpeSplitShapeByVertex` | PASS: rewritten: `!r.vertices().isEmpty || true` inside `if let` was a tautology |
### `LocOpeSpliterTests.swift` (1 tests)
Probe: `Scripts/repro/766-modeling-locope-spliter/`.
| splitByWireOnFace | `OCCTLocOpeSplitByWireOnFace` resolves `faceIndex - 1` (the pre-#541 1-based off-by-one), or returns nullptr, or returns its input shape unsplit | `:22 Expectation failed: top.faceCount == 7`, `:25 Expectation failed: box.splitByWireOnFace(wireShape, faceIndex: 6) == nil` (off by one, and unsplit); `:20 Expectation failed: box.splitByWireOnFace(wireShape, faceIndex: 5)` (nullptr) | pass | `OCCTLocOpeSplitByWireOnFace` | PASS: rewritten: the loop stopped at the first non-nil result (face 1, unsplit) and accepted no result at all |
### `LocOpeSpliterV71Tests.swift` (2 tests)
Probe: `Scripts/repro/766-modeling-locope-spliter-v71/`.
| splitByWireOnFace | `OCCTLocOpeSplitByWires` returns nullptr, or returns its input shape unsplit, or `OCCTShapeCreateBoxAt` returns a cylinder | `:27 Expectation failed: b.locOpeSplit(wiresOnFaces: [(wire: w, face: face)])` (nullptr); `:31 Expectation failed: faceCounts == [6, 6, 6, 6, 6, 7]`, `:32 Expectation failed: (faceCounts.max() ?? 0) > origFaceCount` (unsplit); `:24 Expectation failed: origFaceCount == 6` (cylinder) | pass | `OCCTLocOpeSplitByWires` | PASS: rewritten: the nested `if let` skipped every assertion on a nil fixture or nil split and the only count assertion was `best > original` |
| autoSplit | `OCCTLocOpeSplitByWiresAuto` returns nullptr, or returns its input shape unsplit | `:47 Expectation failed: b.locOpeSplitAuto(wires: [onTopWire])` (nullptr); `:48 Expectation failed: split.subShapes(ofType: .face).count == 7` (unsplit) | pass | `OCCTLocOpeSplitByWiresAuto` | PASS: rewritten: nested `if let` and `faces >= 6` passed on nil and on no split; its edge was off the centred box |

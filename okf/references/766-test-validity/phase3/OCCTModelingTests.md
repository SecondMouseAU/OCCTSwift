...
| foreignFaceInMixedRequestFails | `occtDefeaturingFacesFromShapes` passes a foreign face on and lets the kernel ignore it (the pre-#578 behaviour) | `:88 Expectation failed: filleted.defeature(faces: [filletFace, foreign]) == nil`, `:91 Expectation failed: filleted.defeature(faces: [foreign, filletFace]) == nil` | pass | `OCCTShapeDefeature` | EXPECTED_DIVERGENCE: the kernel ignores the foreign face and succeeds (6 faces) where the bridge's membership rule refuses the mixed request (#578); [foreign] alone agrees |
| mixedCarrierFails | `occtDefeaturingFacesFromShapes` passes a foreign face on and lets the kernel ignore it (the pre-#578 behaviour) | `:113 Expectation failed: filleted.defeature(faces: [mixedCarrier]) == nil` | pass | `OCCTShapeDefeature` | EXPECTED_DIVERGENCE: the kernel explodes the mixed compound, ignores the foreign face and succeeds (6 faces) where the bridge refuses (#578); the clean compound agrees |
| membershipIsIdentityNotGeometry | `occtDefeaturingFacesFromShapes` passes a foreign face on and lets the kernel ignore it (the pre-#578 behaviour) | `:141 Expectation failed: a.defeature(faces: [aFillet, bFillet]) == nil` | pass | `OCCTShapeDefeature` | EXPECTED_DIVERGENCE: the kernel ignores the twin's face beside a real one and succeeds (6 faces) where the bridge refuses (#578); the twin alone and the own face agree |
| elementWithNoFaceFails | `occtDefeaturingFacesFromShapes` accepts a carrier that contributes no face | `:162 Expectation failed: filleted.defeature(faces: [filletFace, edges[0]]) == nil`, `:165 Expectation failed: filleted.defeature(faces: [filletFace, vertices[0]]) == nil` | pass | `OCCTShapeDefeature` | EXPECTED_DIVERGENCE: the kernel skips an edge or vertex beside a real face and succeeds (6 faces) where the bridge refuses (#578); an edge alone agrees |
...
| bothSpellingsRefuseAFaceThatDoesNotBelong | `occtDefeaturingFacesFromShapes` passes a foreign face on and lets the kernel ignore it (the pre-#578 behaviour) | `:241 Expectation failed: filleted.defeature(faces: [filletFace, foreign]) == nil` | pass | `OCCTShapeDefeature` | EXPECTED_DIVERGENCE: with the foreign face or the missing index dropped the kernel succeeds (6 faces) where the bridge refuses (#578, #497); the two spellings agree on the removal they can name |

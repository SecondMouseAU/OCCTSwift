...
| coneFromCircle | `OCCTShapeCreateLoftAdvanced` returns nullptr, or doubles the apex height | `:11 Expectation failed: Shape.loft(profiles: [circle], solid: true, ruled: true, lastVertex: SIMD3(0, 0, 10))` (nullptr); `:22 Expectation failed: abs(volume - Double.pi * 25 * 10 / 3) < 1e-6` (apex doubled) | pass | `OCCTShapeCreateLoftAdvanced` | PASS: strengthened: `volume > 0` held for any solid |
| bicone | `OCCTShapeCreateLoftAdvanced` returns nullptr, or doubles both apex heights | `:28 Expectation failed: Shape.loft(profiles: [circle], solid: true, ruled: true, firstVertex: SIMD3(0, 0, -20), lastVertex: SIMD3(0, 0, 20))` (nullptr); `:39 Expectation failed: abs(volume - 2 * Double.pi * 100 * 20 / 3) < 1e-6` (apexes doubled) | pass | `OCCTShapeCreateLoftAdvanced` | PASS: strengthened: it asserted only non-nil, so any shape passed |
| smoothCone | `OCCTShapeCreateLoftAdvanced` returns nullptr, or doubles the apex height | `:45 Expectation failed: Shape.loft(profiles: [circle], solid: true, ruled: false, lastVertex: SIMD3(0, 0, 10))` (nullptr); `:55 Expectation failed: abs(volume - Double.pi * 25 * 10 / 3) < 1e-6` (apex doubled) | pass | `OCCTShapeCreateLoftAdvanced` | PASS: strengthened: it asserted only non-nil, so any shape passed |
### `MakeConnectedTests.swift` (1 tests)
Probe: `Scripts/repro/766-modeling-make-connected/`.
| connectBoxes | `OCCTShapeMakeConnected` returns nullptr, or returns only its first argument | `:13 Expectation failed: Shape.makeConnected([box1, box2])` (nullptr); `:19 Expectation failed: connected.subShapes(ofType: .solid).count == 2`, `:20 Expectation failed: connected.subShapes(ofType: .face).count == 11`, `:22 Expectation failed: abs(volume - 2000) < 1e-6` (first argument only) | pass | `OCCTShapeMakeConnected` | PASS: strengthened: it asserted only non-nil, so any shape passed |
### `MissingShapeOpsTests.swift` (7 tests)
Probe: `Scripts/repro/766-modeling-missing-shape-ops/`.
| torusCreation | `OCCTShapeCreateTorus` builds the torus with minorRadius * 1.1, or returns nullptr | `:17 Expectation failed: abs(vol - expected) / expected < 0.01`; `:12 Expectation failed: Shape.torus(majorRadius: 10, minorRadius: 3)` (nullptr) | pass | `OCCTShapeCreateTorus` | PASS |
| chamferBox | `OCCTShapeChamfer` returns the input shape unchamfered, or chamfers only the first edge, or returns nullptr | `:26 Expectation failed: chamfered.faces().count > 6`, `:31 Expectation failed: chamfered.faces().count == 26`, `:33 Expectation failed: abs(volume - 945.333333333333) < 1e-6` (unchamfered); `:31`, `:33` (one edge chamfered: `> 6` still holds); `:23 Expectation failed: box.chamfered(distance: 1)` (nullptr) | pass | `OCCTShapeChamfer` | PASS: strengthened: `> 6` held for a box with a single edge chamfered |
| offsetSolid | `OCCTShapeOffset` offsets by -distance, or returns nullptr | `:44 Expectation failed: offsetVol > originalVol`, `:50 Expectation failed: abs(offsetVol - 1200) < 1e-6`; `:39 Expectation failed: box.offset(by: 1.0)` (nullptr) | pass | `OCCTShapeOffset` | PASS: kernel volume 1200, not the 1728 a 12 mm cube would have; the test pins 1200 (it asked only for a volume above the original) |
| scaleShape | `OCCTShapeScale` scales by factor * 0.75, or returns nullptr | `:59 Expectation failed: abs(scaledSize.x - 20) < 0.01`, `:60 Expectation failed: abs(scaledSize.y - 20) < 0.01`, `:61 Expectation failed: abs(scaledSize.z - 20) < 0.01`; `:56 Expectation failed: box.scaled(by: 2.0)` (nullptr) | pass | `OCCTShapeScale` | PASS |
| mirrorShape | `OCCTShapeMirror` never calls `SetMirror` (identity transform), or mirrors across the y plane, or mirrors across x = 3, or returns nullptr | `:71 Expectation failed: mirroredCenter.x < 0`, `:75 Expectation failed: simd_distance(mirroredCenter, SIMD3(-10, 5, 5)) < 1e-6` (identity, and y plane); `:75` alone (x = 3: the centre lands at x = -4, so `x < 0` still holds); `:67 Expectation failed: box.mirrored(planeNormal: SIMD3(1, 0, 0))` (nullptr) | pass | `OCCTShapeMirror` | PASS: strengthened: `x < 0` held for a mirror across the wrong plane |
| sliceAtZ | `OCCTShapeSliceAtZ` / `OCCTShapeSectionWiresAtZ` cut at z + 1, or `OCCTShapeSliceAtZ` returns nullptr | `:87 Expectation failed: slice.subShapes(ofType: .edge).count == 4`, `:88 Expectation failed: slice.center.map { abs($0.z - 5) < 1e-6 } ?? false`; `:85 Expectation failed: box.sliceAtZ(5)` (nullptr) | pass | `OCCTShapeSliceAtZ` | PASS: rewritten: an empty section (z off the shape) is also non-nil and valid |
| sectionWiresAtZ | `OCCTShapeSliceAtZ` / `OCCTShapeSectionWiresAtZ` cut at z + 1, or `OCCTShapeSectionWiresAtZ` returns no wires, or an open wire of 3 edges, or two wires | `:95 Expectation failed: !wires.isEmpty`, `:100 Expectation failed: wires.count == 1` (cut at z + 1, and no wires); `:103 Expectation failed: wireShape.isClosedShape`, `:104 Expectation failed: wire.edges().count == 4` (open wire); `:100` (two wires) | pass | `OCCTShapeSectionWiresAtZ` | PASS: strengthened: `!isEmpty` held for an open wire, a wire of any length, and several wires |

...
### `BiTgteBlendTests.swift` (3 tests)

Probe: `Scripts/repro/766-modeling-bitgte-blend/` (`probe-evidence-fix.mm`, and `probe-concave-scan.mm` for the edge-by-edge scan). `blendBoxEdge` and `blendMultipleEdges` used `guard let result else { return }` after `#expect(result != nil)`; the result goes through `try #require` now, and each also asserts the result is a SHELL (the kernel's blend of a box edge is a shell with the box's own faces and volume, so the type is what tells it from the input solid handed back unchanged). They do not test a blend: every edge of a plain box is convex, and on a convex edge BiTgte_Blend returns the box's shell unchanged (their display names and comments now say so). `blendConcaveEdge` is the test that exercises the blend, added under #766: an L-shaped solid (a 40 x 40 x 10 slab fused with a 40 x 10 x 30 wall, 14 faces, 24000 mm3) whose one concave edge, the inner corner along X at y = 10, z = 10 (index 22 today, found by position), is the only one of its 28 edges on which the blend changes anything (`transcript-concave-scan.txt`); a ball of radius 3 there gives a SHELL of 17 faces and 24076.815240690972 mm3. Injections, each reverted (one build, the behaviour chosen by an `INJ` environment variable, not committed): `OCCTBiTgteBlend` returns nullptr (`:31`, `:45`, `:72`, the `#require`); returns its input shape (the type at `:32`, `:46`, `:73`, and for the concave test also the face count `:75` and the volume `:77`); blends at half the radius (`:77` only: 17 faces and a shell still, the volume names the radius); blends edge 0 instead of the requested edge (`:75`, `:77`); `OCCTShapeCreateBoxAt` returns nullptr (`:29`, `:39`, `:57`, the fixtures' `#require`) or shifts its origin by 1 (`:66`, `corner.count == 1`, the edge is no longer where the test looks); the union returns nullptr (`:59`).

| Test | Injection | Red (failing expectation) | Green | Bridge function | Parity |
|---|---|---|---|---|---|
| blendBoxEdge | `OCCTBiTgteBlend` returns nullptr / returns its input shape | `:31 Expectation failed: box.biTgteBlend(edgeIndices: [0], radius: 5)`; input returned: `:32 Expectation failed: result.shapeType == .shell` | pass | `OCCTBiTgteBlend` | PASS: rewritten; pins the kernel's no-op on a convex edge, not a blend test |
| blendMultipleEdges | `OCCTBiTgteBlend` returns nullptr / returns its input shape | `:45 Expectation failed: box.biTgteBlend(edgeIndices: [0, 1], radius: 3)`; input returned: `:46 Expectation failed: result.shapeType == .shell` | pass | `OCCTBiTgteBlend` | PASS: rewritten; had no assertion; pins the kernel's no-op on convex edges, not a blend test |
| blendConcaveEdge | `OCCTBiTgteBlend` returns nullptr / returns its input shape / blends at half the radius / blends another edge | `:72 Expectation failed: lShape.biTgteBlend(edgeIndices: [corner[0].index], radius: 3)`; input returned: `:73 Expectation failed: blend.shapeType == .shell`, `:75 Expectation failed: blend.subShapes(ofType: .face).count == 17`, `:77 Expectation failed: abs(volume - 24076.815240690972) < 1e-4`; half radius: `:77`; another edge: `:75`, `:77` | pass | `OCCTBiTgteBlend` | PASS: added under #766; SHELL, valid, 17 faces and 24076.815 mm3, bridge and kernel to full precision |

### `BooleanCheckTests.swift` (11 tests)

Probe: `Scripts/repro/766-modeling-boolean-check/`. Rewritten under #766: `singleShapeValid`, `sphereValid`, `pairShapesValidForFuse`, `pairShapesValidForCut` and `singleShapeNoSelfInterference` asserted inside `if let` on their fixtures (a shape that failed to build skipped the test and it passed) and take them through `try #require` now. The two parity tests compared a wrapper with the call it forwards to (one bridge function called twice) on a box and a sphere for which every setting of the small-edge and self-interference tests answers true, so neither could see the forwarded defaults drift, and the comment claiming they could was false. They now compare the wrapper with the kernel's own `BOPAlgo_ArgumentAnalyzer` (`Shape.analyzeBoolean`, `OCCTBOPAlgoAnalyzeArguments`) on a thin box (small-edge fault only) and a compound of two overlapping cubes (self-interference fault only) as well as the valid pair. Proof, each injection reverted: the old parity tests pass and the new ones fail when `Shape.isValidForBoolean` forwards `testSmallEdges: false` (`:144`, `:145` on the thin box) or `testSelfInterference: false` (`:144`, `:145` on the overlapping compound), and when `Shape.isValidForBoolean(with:)` forwards `testSelfInterference: false` (`:167`, `:170`) or `testSmallEdges: false` (`:167`, `:170`); when `OCCTShapeBooleanCheckSingle` and `OCCTShapeBooleanCheckPair` always return true they fail at `:144` and `:167` and pass before this change; the rows below give the always-false injection.
...
| singleShapeValid | `OCCTShapeBooleanCheckSingle` returns false | `:36 Expectation failed: box.isBooleanValid()` | pass | `OCCTShapeBooleanCheckSingle` | PASS |
| sphereValid | `OCCTShapeBooleanCheckSingle` returns false | `:41 Expectation failed: sphere.isBooleanValid()` | pass | `OCCTShapeBooleanCheckSingle` | PASS |
| pairShapesValidForFuse | `OCCTShapeBooleanCheckPair` returns false | `:50 Expectation failed: box.isBooleanValidWith(sphere, operation: 2)` | pass | `OCCTShapeBooleanCheckPair` | PASS: operation 2 is `BOPAlgo_CUT`, not fuse, despite the test's name |
| pairShapesValidForCut | `OCCTShapeBooleanCheckPair` returns false | `:58 Expectation failed: box.isBooleanValidWith(sphere, operation: 3)` | pass | `OCCTShapeBooleanCheckPair` | PASS: operation 3 is `BOPAlgo_CUT21` |
| solidVsFaceCutAndCut21Disagree | `OCCTShapeBooleanCheckPair` swaps `BOPAlgo_CUT` (2) and `BOPAlgo_CUT21` (3) in its `static_cast<BOPAlgo_Operation>` | `:79 Expectation failed: !box.isBooleanValidWith(face, operation: 2, testSmallEdges: false, testSelfInterference: false)`, `:85 Expectation failed: box.isBooleanValidWith(face, operation: 3, testSmallEdges: false, testSelfInterference: false)` | pass | `OCCTShapeBooleanCheckPair` | PASS |
| singleShapeNoSelfInterference | `OCCTShapeBooleanCheckSingle` returns false | `:92 Expectation failed: box.isBooleanValid(testSmallEdges: false, testSelfInterference: true)` | pass | `OCCTShapeBooleanCheckSingle` | PASS |
| singleShapeParityWithBooleanValid | `OCCTShapeBooleanCheckSingle` returns false, or `Shape.isValidForBoolean` forwards one test switched off | `:144 Expectation failed: c.shape.isValidForBoolean == analyzer` (also `:145`, and `:150`, `:151` for the always-false bridge) | pass | `OCCTShapeBooleanCheckSingle` | PASS: `valid` and the kernel's `BOPAlgo_ArgumentAnalyzer` verdict agree on a box, a sphere, a thin box and an overlapping compound |
| pairParityWithBooleanValidWith | `OCCTShapeBooleanCheckPair` returns false, or `Shape.isValidForBoolean(with:)` forwards one test switched off | `:167 Expectation failed: c.first.isValidForBoolean(with: c.second) == analyzer` (also `:170`, and `:175`, `:178` for the always-false bridge) | pass | `OCCTShapeBooleanCheckPair` | PASS: `valid` and the kernel's `BOPAlgo_ArgumentAnalyzer` verdict agree on four pairs, two of them faulty in one argument only |

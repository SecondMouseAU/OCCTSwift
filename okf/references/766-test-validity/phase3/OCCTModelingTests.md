...
Probe: `Scripts/repro/766-modeling-boolean-history/`. Tightened under #766: `fuseWithHistory` read `r.shape.volume!` inside `#expect` (a force-unwrap the repo's test conventions forbid) against `> 0`, which any positive volume satisfies, and both tests took the result through `#expect(result != nil)` and an `if let r`. Fixtures and the result go through `try #require` now, and each test asserts the kernel's measured values: solid count and volume of the fused shape, the exact modified count (8, and 0 for the disjoint cubes) and that every modified shape is a face. Injections, each reverted (`INJ=<name>` selected the behaviour at run time in one build): `OCCTShapeFuseWithHistory` appends one duplicate entry (count 9: the old `> 0` passes it, the new test fails at `:30`); returns the whole first cube for every modified face (count 8, the shapes are solids: fails `:31`); reports an unmodified input face as modified (itself: fails `:40` and `:30`); returns -1 (`:23`, `:38`, the `#require`); `OCCTShapeCreateBox` returns nullptr (`:14`, the fixture helper's `#require`); the union returns only its first argument (`:27` volume 1000 against 1500 with the old `> 0` passing it, and `:42`, `:44` for the disjoint cubes); `OCCTShapeTranslate` returns nullptr (`:16`). A glue-shift injection (`runBooleanEx` forcing `BOPAlgo_GlueShift`) does not change either result, since the cubes share every y and z extent (measured), so it is not evidence.

| Test | Injection | Red (failing expectation) | Green | Bridge function | Parity |
|---|---|---|---|---|---|
| fuseWithHistory | `OCCTShapeFuseWithHistory` appends a duplicate modified shape, returns the wrong shapes or -1; the union drops the second argument | `:30 Expectation failed: r.modifiedFaces.count == 8`; `:31 Expectation failed: r.modifiedFaces.allSatisfy { $0.shapeType == .face }`; `:23 Expectation failed: box1.fuseWithHistory(box2)`; `:27 Expectation failed: abs(volume - 1500) < 1e-6` | pass | `OCCTShapeFuseWithHistory` | PASS: one solid of 1500 and 8 modified faces |
| fuseNonOverlappingHistory | `OCCTShapeFuseWithHistory` reports an unmodified input face as modified (itself), returns -1; the union drops the second argument | `:40 Expectation failed: r.modifiedFaces.count == 0`; `:38 Expectation failed: box1.fuseWithHistory(box2)`; `:42 Expectation failed: r.shape.subShapes(ofType: .solid).count == 2`, `:44 Expectation failed: abs(volume - 250) < 1e-6` | pass | `OCCTShapeFuseWithHistory` | PASS: two solids of 125 and no modified face |

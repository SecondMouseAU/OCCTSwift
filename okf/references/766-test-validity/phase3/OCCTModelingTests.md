...
Probe: `Scripts/repro/766-modeling-boolean-tolerance/`. Rewritten under #766: the fixtures used the centred `Shape.box(width:height:depth:)` as if it were corner-based, so the fuzzy value and the glue mode never reached the result (the second cube of `fuseWithTolerance` and `fuseWithGlue` sat clear of the first, the commons met at one corner, the cut returned its first cube unchanged, and `cutWithGlue` and `commonWithGlue` passed glue `.off`, the default). Each test now uses corner-based cubes chosen so the option changes the answer, asserts the same operation with the option off as a control, and fails when the bridge drops the argument. Injection for every row: `runBooleanEx` (`OCCTBridge_Modeling_Boolean.mm`) skips `SetFuzzyValue` and selects `BOPAlgo_GlueOff` whatever `glue` is (the tolerance tests never pass glue and the glue tests never pass a tolerance, so each test answers to exactly one of the two).

| Test | Injection | Red (failing expectation) | Green | Bridge function | Parity |
|---|---|---|---|---|---|
| fuseWithTolerance | `runBooleanEx` skips `SetFuzzyValue` | `:48 Expectation failed: fused.subShapes(ofType: .solid).count == 1` (also `:49`, `:51`) | pass | `OCCTShapeUnionEx` | PASS: cubes 0.001 apart, fuzzy 0.01 joins them into one solid of 10 faces; the control keeps two solids |
| cutWithTolerance | `runBooleanEx` skips `SetFuzzyValue` | `:74 Expectation failed: abs(volume - 1000) < 1e-6` | pass | `OCCTShapeSubtractEx` | PASS: a slab 0.001 below the cube's top face: fuzzy 0.01 removes nothing (1000), the control removes 0.1 (999.9) |
| commonWithTolerance | `runBooleanEx` skips `SetFuzzyValue` | `:96 Expectation failed: common.subShapes(ofType: .solid).isEmpty` | pass | `OCCTShapeIntersectEx` | PASS: a 0.001 sliver of overlap: fuzzy 0.01 leaves a valid empty common, the control keeps the 0.1 sliver |
| fuseWithGlue | `runBooleanEx` selects `BOPAlgo_GlueOff` for every glue value | `:126 Expectation failed: fused.subShapes(ofType: .solid).count == 2` (also `:128`) | pass | `OCCTShapeUnionEx` | PASS: interpenetrating cubes, GlueShift skips the face/face intersection and keeps both (2 solids, 2000), the control is the true union (1 solid, 1875) |
| cutWithGlue | `runBooleanEx` selects `BOPAlgo_GlueOff` for every glue value | `:151 Expectation failed: cut.subShapes(ofType: .face).count == 6` (also `:153`) | pass | `OCCTShapeSubtractEx` | PASS: interpenetrating cubes, GlueFull removes nothing (8000), the control removes the 125 overlap (7875) |
| commonWithGlue | `runBooleanEx` selects `BOPAlgo_GlueOff` for every glue value | `:174 Expectation failed: common.subShapes(ofType: .solid).isEmpty` | pass | `OCCTShapeIntersectEx` | PASS: interpenetrating cubes, GlueShift finds no common, the control finds the 125 overlap |

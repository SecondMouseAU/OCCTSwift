...
Probe: `Scripts/repro/766-modeling-bitgte-blend/`. Both tests used `guard let result else { return }` after `#expect(result != nil)`; the result goes through `try #require` now, and each also asserts the result is a SHELL (the kernel's blend of a box edge is a shell with the box's own faces and volume, so the type is what tells it from the input solid handed back unchanged). Two injections, both reverted: `OCCTBiTgteBlend` returns nullptr (`:23`, `:37`, the `#require`) and `OCCTBiTgteBlend` returns its input shape unchanged (`:24`, `:38`, the type check; the face-count and volume assertions still pass, so the tests before this change accepted it).

| Test | Injection | Red (failing expectation) | Green | Bridge function | Parity |
|---|---|---|---|---|---|
| blendBoxEdge | `OCCTBiTgteBlend` returns nullptr / returns its input shape | `:23 Expectation failed: box.biTgteBlend(edgeIndices: [0], radius: 5)`; input returned: `:24 Expectation failed: result.shapeType == .shell` | pass | `OCCTBiTgteBlend` | PASS: rewritten; the kernel blend leaves the box unrounded |
| blendMultipleEdges | `OCCTBiTgteBlend` returns nullptr / returns its input shape | `:37 Expectation failed: box.biTgteBlend(edgeIndices: [0, 1], radius: 3)`; input returned: `:38 Expectation failed: result.shapeType == .shell` | pass | `OCCTBiTgteBlend` | PASS: rewritten; had no assertion |

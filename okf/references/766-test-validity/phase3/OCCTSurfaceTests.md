...
### Measured: NLPlateG2G3Tests.swift, ParametricPlateSurfaceTests.swift (8 tests), probe Scripts/repro/766-nlplate-g2g3-platethrough/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| NLPlate G2/G3 Constraints | nlPlateG2Deformation | `OCCTSurfaceNLPlateG2` | fit result dropped, nil (INJ_NLP_FAIL) | NLPlateG2G3Tests.swift:36 result != nil | ✅ | MISMATCH | MISMATCH, pinned with withKnownIssue. Was `result != nil` inside `if let plane` |
| NLPlate G2/G3 Constraints | nlPlateG3Deformation | `OCCTSurfaceNLPlateG3` | fit result dropped, nil (INJ_NLP_FAIL) | NLPlateG2G3Tests.swift:66 result != nil | ✅ | MISMATCH | MISMATCH, pinned with withKnownIssue |
| NLPlate G2/G3 Constraints | nlPlateIncrementalSolve | `OCCTSurfaceNLPlateIncrementalG0` | target z + 1 (INJ_NLP_TARGET_OFF) | NLPlateG2G3Tests.swift:86 near(result, SIMD2(0.5, 0.5), SIMD3(0.5, 0.5, 1.0), 1e-3) | ✅ | MATCH | Rewritten: asserted only non-nil |
| NLPlate G2/G3 Constraints | nlPlateDerivative | `OCCTSurfaceNLPlateEvaluateDerivative` | iu and iv swapped (INJ_NLP_DERIV_SWAP) | NLPlateG2G3Tests.swift:102 simd_length(deriv - SIMD3(1, 0, 0.99999950000024984)) < 1e-12 | ✅ | MATCH | Rewritten: asserted only non-nil |
| Parametric Plate Surface Tests | Plate through points returns parametric surface | `OCCTSurfacePlateThrough` | point z + 1 (INJ_PT_ZOFF) | ParametricPlateSurfaceTests.swift:31 maxDistance(s, points) < 1e-6 | ✅ | MATCH | Rewritten: `uMax > uMin` passed a plate that missed its points. The v bounds (-7.77817459, 7.77817459, the probe's) are pinned at :29 and :30; they are red under a second injection that keeps the geometry and doubles the V knots (where the u bounds and every distance check stay green) |
| Parametric Plate Surface Tests | Plate through points is evaluable | `OCCTSurfacePlateThrough` | point z + 1 (INJ_PT_ZOFF) | ParametricPlateSurfaceTests.swift:51 abs(pt.z) < 1e-9 | ✅ | MATCH | Rewritten: finite coordinates only |
| Parametric Plate Surface Tests | Plate through rejects fewer than 3 points | `OCCTSurfacePlateThrough` | none run: refused by the Swift guard, the bridge guard and a kernel throw | not red: three independent layers refuse two points | ✅ | N/A | NOT PROVEN RED: removing one layer leaves the others (same shape as AdvancedPlateSurfaceTests, #2416) |
| Parametric Plate Surface Tests | Plate through with custom degree | `OCCTSurfacePlateThrough` | point z + 1 (INJ_PT_ZOFF) | ParametricPlateSurfaceTests.swift:73 maxDistance(s, points) < 1e-3 | ✅ | MATCH | Rewritten: asserted only non-nil |

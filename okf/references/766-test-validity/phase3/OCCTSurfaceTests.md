...
### Measured: Issue266FaceAnalysisFollowupTests.swift, Issue266FaceWithHolesTests.swift (13 tests), probe Scripts/repro/766-issue233-244-266-317/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| Issue #266 follow-up, surface analysis extras | UVFromIso refines (u,v) for a point on a cylinder | `OCCTSurfaceUVFromIso` | U and V swapped on output (INJ_266_UVISO_SWAP) | Issue266FaceAnalysisFollowupTests.swift:23 abs(r.v - 3) < 1e-6 | ✅ | MATCH |  |
| Issue #266 follow-up, surface analysis extras | singularity detail at a cone apex | `OCCTSurfaceSingularityDetail` | apex x and z swapped (INJ_266_SING_SWAP) | Issue266FaceAnalysisFollowupTests.swift:42 abs(s.point.x) < 1e-6 | ✅ | MATCH | Added the apex z pin. Also reaches OCCTSurfaceSingularityCount |
| Issue #266 follow-up, surface analysis extras | out-of-range singularity index returns nil | `OCCTSurfaceSingularityDetail` | index ignored, singularity 1 returned (INJ_266_SING_ANYNUM) | Issue266FaceAnalysisFollowupTests.swift:60 cone.singularity(99) == nil | ✅ | MATCH |  |
| Issue #266 follow-up, surface analysis extras | domain-restricted projection lands inside the domain | `OCCTSurfaceProjectPointUVInDomain` | v + 1 on output (INJ_266_PROJ_VOFF) | Issue266FaceAnalysisFollowupTests.swift:77 abs(r.uv.y - 2) < 1e-9 | ✅ | MATCH | Rewritten: the domain check passed a v off by 1; pins uv |
| Issue #266 follow-up, surface analysis extras | BRepGProp_Face integration orders + U knots on a planar face | `OCCTBRepGPropFaceUKnots` | U knots written reversed (INJ_266_UKNOTS_REVERSED) | Issue266FaceAnalysisFollowupTests.swift:98 knots == [0, 10] | ✅ | MATCH | Rewritten: orders != nil and count >= 2; pins orders and knots. Also reaches OCCTBRepGPropFaceIntegrationOrders |
| Issue #266 follow-up, surface analysis extras | tangent plane is two-sided: TangentU and TangentV both defined and independent | `OCCTFaceLPropTangentV` | TangentV returns TangentU (INJ_266_TV_EQ_TU) | Issue266FaceAnalysisFollowupTests.swift:126 simd_length(cross) > 0.5 | ✅ | MATCH | Added direction pins. Also reaches OCCTFaceLPropTangentU |
| Issue #266 follow-up, surface analysis extras | BRepGProp_Face V knots + surface-integration params | `OCCTBRepGPropFaceSurfaceIntegration` | SIntOrder reported as 0 (INJ_266_SIORDER_ZERO) | Issue266FaceAnalysisFollowupTests.swift:144 si.order == 3 | ✅ | MATCH | Tightened `>= 1` to the kernel values. Also reaches OCCTBRepGPropFaceVKnots |
| Issue #266 follow-up, surface analysis extras | BRepGProp_Face boundary integration on a face edge | `OCCTBRepGPropFaceBoundaryIntegration` | edge index ignored, edge 0 used (INJ_266_BI_ANYEDGE) | Issue266FaceAnalysisFollowupTests.swift:164 face.faceBoundaryIntegration(edgeIndex: 99) == nil | ✅ | MATCH | Tightened `>= 1` to the kernel values |
| Issue #266 follow-up, surface analysis extras | FaceFixer tolerance clamps run without crashing | `OCCTFaceFixerResult` | Result() reported as null (INJ_266_FIXER_NULLRESULT) | Issue266FaceAnalysisFollowupTests.swift:178 fixer.result != nil | ✅ | MATCH | Rewritten: `if let r` passed a fixer with no result. Also reaches OCCTFaceFixerSetMinTolerance, OCCTFaceFixerSetMaxTolerance, OCCTFaceFixerPerform |
| Issue #266, face from surface with holes | outer + one hole yields a valid face with the window removed | `OCCTShapeCreateFaceFromSurfaceWireWithHoles` | holes not added (INJ_266_NOHOLES) | Issue266FaceWithHolesTests.swift:39 face.subShapeCount(ofType: .wire) == 2 | ✅ | MATCH | `if let area` made unconditional |
| Issue #266, face from surface with holes | empty innerWires gives the plain trimmed face (full area) | `OCCTShapeCreateFaceFromSurfaceWireWithHoles` | no holes refused (INJ_266_REQUIRE_HOLES); reversing the outer wire (INJ_266_OUTER_REVERSED) stays GREEN because ShapeFix_Face reorients it | Issue266FaceWithHolesTests.swift:51 Issue.record("nil") | ✅ | MATCH | `if let area` made unconditional |
| Issue #266, face from surface with holes | multiple holes each become an opening | `OCCTShapeCreateFaceFromSurfaceWireWithHoles` | holes not added (INJ_266_NOHOLES) | Issue266FaceWithHolesTests.swift:83 face.subShapeCount(ofType: .wire) == 3 | ✅ | MATCH | `if let area` made unconditional |
| Issue #266, face from surface with holes | a hole off the surface fails rather than producing garbage | `OCCTShapeCreateFaceFromSurfaceWireWithHoles` | holes not added (INJ_266_NOHOLES) | Issue266FaceWithHolesTests.swift:112 abs((face?.surfaceArea ?? 0) - 84) < 1e-6 | ✅ | MATCH | FINDING / rewritten: accepted nil or any face. The kernel takes a hole 5 units off the plane and ShapeFix_Face projects it in, so the call returns a valid area-84 face instead of refusing as the title says; pinned, with the refusal kept as withKnownIssue |

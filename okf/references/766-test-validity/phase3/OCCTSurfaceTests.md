...
### Measured: Issue620DrawMeshCountContractTests.swift, Issue748NetworkSurfaceCornersTests.swift, Issue791ConvertSphereHelperTests.swift (13 tests), probe Scripts/repro/766-issue620-748-791/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| drawMesh honours its documented minimum of one sample per direction (#620) | uCount 1 returns a single V iso-row, not an empty grid | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift !grid.isEmpty | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | vCount 1 returns a single U iso-row, not an empty grid | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift !grid.isEmpty | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | On a bounded surface the single row sits at domain.uMin | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift Issue.record("expected a 1x8 grid ...") | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | On an unbounded surface the single row sits at the clamp, not at domain.uMin | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift Issue.record("expected a 1x5 grid ...") | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | A one-row grid equals row 0 of a many-row grid | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift Issue.record("grids not served ...") | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | A 1x1 grid is one point, at the sampled-range corner | `OCCTSurfaceDrawMesh` | OCCTSurfaceDrawMesh refuses fewer than two samples per direction, the pre-#620 contract (INJ_620_MIN2) | Issue620DrawMeshCountContractTests.swift !grid.isEmpty | ✅ | MATCH |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | Counts below 1 and products past the ceiling are still rejected | `(Swift) Sampling.gridTotal` | sample-count ceiling removed (INJ_620_NOCEIL, Swift) | Issue620DrawMeshCountContractTests.swift:216 s.drawMesh(uCount: 5000, vCount: 2001).isEmpty | ✅ | N/A |  |
| drawMesh honours its documented minimum of one sample per direction (#620) | Bezier keeps its minimum of two poles per direction, which is OCCT's own | `OCCTSurfaceCreateBezier` | Swift and bridge two-pole guards lowered to one (INJ_620_BZ_MIN1): stays GREEN, the kernel refuses a single row or column itself | none: cannot be turned red at the wrapper; the Geom_BezierSurface constructor throws and the bridge catch returns nil | ✅ | MATCH | Three layers refuse (Swift guard, bridge guard, kernel throw); the test pins the kernel rule as its title says |
| networkSurface places every corner where its input curves say (#748) | a square 2x2 network puts all four corners where the curves do | `OCCTGeomFillNetworkSurface` | intersection grid transposed, the pre-#748 layout (INJ_748_SWAPPED) | Issue748NetworkSurfaceCornersTests.swift:53 Self.cornersMatch(s, ...) == 0 | ✅ | MATCH |  |
| networkSurface places every corner where its input curves say (#748) | a non-square 2x3 network builds at all, which the swapped grid could not | `OCCTGeomFillNetworkSurface` | intersection grid transposed (INJ_748_SWAPPED) | Issue748NetworkSurfaceCornersTests.swift:72 status == .done | ✅ | MATCH |  |
| Issue791 Sphere BSpline helper consolidation | unitOriginMatchesPriorBaseline | `OCCTConvertSphereToBSplineSurface` | radius scaled by 1.001 (INJ_791_RADIUS) | Issue791ConvertSphereHelperTests.swift:260 p == baseline.point | ✅ | MATCH |  |
| Issue791 Sphere BSpline helper consolidation | offsetTiltedMatchesPriorBaseline | `OCCTConvertSphereToBSplineSurface` | axis forced to +Z (INJ_791_AXIS_Z) | Issue791ConvertSphereHelperTests.swift:260 p == baseline.point | ✅ | MATCH |  |
| Issue791 Sphere BSpline helper consolidation | convertedSphereStaysOnTheAnalyticSphere | `OCCTConvertSphereToBSplineSurface` | radius scaled by 1.001 (INJ_791_RADIUS) | Issue791ConvertSphereHelperTests.swift:293 abs(dist - c.radius) < 1e-6 | ✅ | MATCH |  |

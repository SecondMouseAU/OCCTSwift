...
### Measured: Issue485SurfaceContinuityTests.swift, Issue486SurfaceGridTests.swift (11 tests), probe Scripts/repro/766-issue485-486-495/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| Surface measured continuity (#485) | ContinuityClass mirrors the real GeomAbs_Shape ordinals | `(Swift) ContinuityClass` | ContinuityClass g1 and c1 raw values swapped at compile time in a separate build; raw values cannot be switched by environment | Issue485SurfaceContinuityTests.swift:29 ContinuityClass.g1.rawValue == 1 | ✅ | MATCH |  |
| Surface measured continuity (#485) | ContinuityClass orders by increasing smoothness | `(Swift) ContinuityClass.<` | `<` compares reversed (INJ_485_LT_REVERSED, Swift) | Issue485SurfaceContinuityTests.swift:45 ContinuityClass.c0 < .g1 | ✅ | N/A |  |
| Surface measured continuity (#485) | satisfies() answers a parametric floor without raw-value comparison | `(Swift) ContinuityClass.satisfies` | raw-value comparison, the pre-#485 form (INJ_485_RAWCOMPARE, Swift) | Issue485SurfaceContinuityTests.swift:59 !ContinuityClass.c1.satisfies(.c2) | ✅ | N/A |  |
| Surface measured continuity (#485) | Knot multiplicity drives the measured class, at GeomAbs_Shape's own ordinals | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (C2 as 2, CN as 99) (INJ_485_OLDENC) | Issue485SurfaceContinuityTests.swift:91 c2.continuityClass == .c2 | ✅ | MATCH | Rewritten: each fixture sat behind `if let`; now asserted to exist first |
| Surface measured continuity (#485) | Analytic surfaces report CN as ordinal 6, not 99 | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (C2 as 2, CN as 99) (INJ_485_OLDENC) | Issue485SurfaceContinuityTests.swift:110 plane.continuityClass == .cN | ✅ | MATCH | Rewritten: both fixtures sat behind `if let` |
| Surface measured continuity (#485) | continuity and continuityClass agree on the same surface, in every class | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (C2 as 2, CN as 99) (INJ_485_OLDENC) | Issue485SurfaceContinuityTests.swift:133 surface.continuity != 99 | ✅ | MATCH |  |
| Surface measured continuity (#485) | The result vocabulary stays distinct from the two request vocabularies | `(Swift) ContinuityClass` | ContinuityClass g1 and c1 raw values swapped at compile time in a separate build; raw values cannot be switched by environment | Issue485SurfaceContinuityTests.swift:149 ContinuityClass.c1.rawValue != ParametricContinuity.c1.rawValue | ✅ | N/A |  |
| Issue 486: surface grid layout | evaluateGridD1 indexes each (u, v) at its own parameter, not transposed | `OCCTSurfaceEvaluateGridD1` | output index transposed (INJ_486_D1_TRANSPOSE); D1V written from D1U (INJ_486_D1V_IS_D1U) | Issue486SurfaceGridTests.swift:38 simd_length(actual.point - expected.point) < 1e-6 | ✅ | MATCH | The `guard !grid.isEmpty else { return }` now has an expectation in front of it |
| Issue 486: surface grid layout | evaluateGrid and evaluateGridD1 agree point-for-point under .at(u:v:) | `OCCTSurfaceEvaluateGridD1` | output index transposed (INJ_486_D1_TRANSPOSE) | Issue486SurfaceGridTests.swift:59 simd_length(d0.at(u: iu, v: iv) - d1.at(u: iu, v: iv).point) < 1e-9 | ✅ | MATCH | Also reaches OCCTSurfaceEvaluateGrid |
| Issue 486: surface grid layout | drawMesh, evaluateGrid and evaluateGridD1 all read U-major | `OCCTSurfaceEvaluateGridD1` | output index transposed (INJ_486_D1_TRANSPOSE) | Issue486SurfaceGridTests.swift:84 simd_length(mesh.at(u: iu, v: iv) - d1.at(u: iu, v: iv).point) < 1e-6 | ✅ | MATCH | Also reaches OCCTSurfaceDrawMesh |
| Issue 486: surface grid layout | empty parameter arrays give an empty grid, not a grid of zeroes | `(Swift) Surface.evaluateGrid` | a 1x1 zero grid returned for an empty parameter list (INJ_486_EMPTY_ZERO, Swift) | Issue486SurfaceGridTests.swift:92 sphere.evaluateGrid(uParameters: [], vParameters: [0.0]).isEmpty | ✅ | N/A |  |

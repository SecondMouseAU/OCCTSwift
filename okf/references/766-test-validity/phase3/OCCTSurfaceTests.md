...
### Measured: SurfaceOperationsTests.swift, SurfacePipeTests.swift, SurfaceQueriesV123Tests.swift (12 tests), probe Scripts/repro/766-surface-operations-pipe/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| Surface Operations | Trim sphere surface | `OCCTSurfaceTrim` | v2 + 0.1 (INJ_OP_TRIM) | SurfaceOperationsTests.swift:22 abs(tDom.vMax - vMid) < 1e-10 | ✅ | MATCH | Caught as written |
| Surface Operations | Offset surface | `OCCTSurfaceOffset` | distance negated (INJ_OP_OFFSET) | SurfaceOperationsTests.swift:35 abs(dist - 7.0) < 1e-6 | ✅ | MATCH | Caught as written; point pinned |
| Surface Operations | Translate surface | `OCCTSurfaceTranslate` | dy + 1 (INJ_OP_T) | SurfaceOperationsTests.swift:50 simd_length(p - (pOrig + SIMD3(10, 0, 0))) < 1e-12 | ✅ | MATCH | Rewritten: only x was checked |
| Surface Operations | Scale surface | `OCCTSurfaceScale` | factor + 0.5 (INJ_OP_SCALE) | SurfaceOperationsTests.swift:62 abs(dist - 10.0) < 1e-6 | ✅ | MATCH | Caught as written; point pinned |
| Surface Operations | Mirror surface across XY plane | `OCCTSurfaceMirrorPlane` | mirrored through the plane origin as a point (INJ_OP_MPL) | SurfaceOperationsTests.swift:82 simd_length(p - SIMD3(2, 0, -5)) < 1e-12 | ✅ | MATCH | Rewritten: only z was checked, which a point mirror also negates |
| Surface Operations | Rotate surface around an axis | `OCCTSurfaceRotate` | angle doubled (INJ_OP_ROT) | SurfaceOperationsTests.swift:106 abs(p.x - expectedX) < 1e-6 | ✅ | MATCH | Caught as written |
| Surface Operations | Mirror surface across a point | `OCCTSurfaceMirrorPoint` | mirror point x + 1 (INJ_OP_MP) | SurfaceOperationsTests.swift:133 abs(p.x - expectedX) < 1e-6 | ✅ | MATCH | Caught as written |
| Surface Operations | Mirror surface across an axis | `OCCTSurfaceMirrorAxis` | axis mirror done as a plane mirror (INJ_OP_MA) | SurfaceOperationsTests.swift:154 abs(p.x + pOrig.x) < 1e-6 | ✅ | MATCH | Caught as written |
| Surface Pipe | Pipe with circular cross-section | `OCCTSurfaceCreatePipe` | radius + 1 (INJ_PIPE) | SurfacePipeTests.swift:21 simd_length(p - SIMD3(-1.9021130325903073, -0.61803398874989446, 6)) < 1e-9 | ✅ | MATCH | Rewritten: ordered bounds passed any pipe |
| Surface Pipe | Pipe with section curve | `OCCTSurfaceCreatePipeWithSection` | section scaled 1.1 (INJ_PIPE) | SurfacePipeTests.swift:34 simd_length(p - SIMD3(-0.92705098312484202, 2.8531695488854609, 6)) < 1e-9 | ✅ | MATCH | Rewritten: asserted only non-nil |
| v0.123.0, Surface queries | Cylinder U period | `OCCTSurfaceGetUPeriod` | period doubled (INJ_SA_UPERIOD) | SurfaceQueriesV123Tests.swift:18 abs(p - 2.0 * .pi) < 1e-10 | ✅ | MATCH | Caught as written; its two `if let`s now record an issue on nil |
| v0.123.0, Surface queries | Plane has no period | `OCCTSurfaceGetVPeriod` | Swift periodicity guard skipped and bridge reports 1.0 (INJ_Q_PLANE_PERIOD) | SurfaceQueriesV123Tests.swift:32 vPeriod == nil \|\| vPeriod == 0.0 | ✅ | MATCH | Caught as written; `if let` made unconditional |

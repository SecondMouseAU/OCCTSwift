...
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| GeomConvert_CompBezierSurfacesToBSplineSurface | Join two Bezier patches into BSpline | `OCCTSurfaceJoinBezierPatches` | patch order reversed (INJ_JOIN_REVERSE) | JoinBezierPatchesTests.swift:30 simd_length(joined.point(atU: 1.5, v: 0.5) - SIMD3(7.5, 5, 0)) < 1e-12 | ✅ | MATCH | Rewritten: `joined.handle != nil` is true of any Surface |
| GeomConvert_CompBezierSurfacesToBSplineSurface | Rejects a rational patch instead of silently dropping its weights (#725) | `OCCTSurfaceJoinBezierPatches` | rational-patch guard skipped (INJ_JOIN_NORATCHECK) | JoinBezierPatchesTests.swift:65 Surface.joinBezierPatches([patch], rows: 1, cols: 1) == nil | ✅ | N/A | Caught as written; a wrapper refusal with no kernel counterpart |
...
| Local Revolution Tests | Revolve with angular offset | `OCCTLocOpeRevolWithOffset` | offset ignored (INJ_LOCREV_OFFSET) | LocalRevolutionTests.swift:71 box.min.y > 6.3 and :72 box.min.x < -7.7 (both red; the compound expectation reported only the first) | ✅ | MATCH | Rewritten. The old input (the centred 0.1-thick box, a solid) gave an EMPTY compound, which `!= nil` accepted |
...
| LocOpe Pipe Tests | Pipe sweep along wire spine | `OCCTLocOpePipe` | returns the profile unchanged (INJ_LOCPIPE_INPUT) | LocOpePipeTests.swift:28 abs((result.volume ?? 0) - 40) < 1e-9 | ✅ | MATCH | Rewritten: its XY profile contained the spine, giving an invalid zero-volume solid that `!= nil` accepted |

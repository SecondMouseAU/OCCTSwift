...
| Convert Elementary Surfaces Tests | conePatch | `OCCTConvertConeToBSplineSurface` | reference radius + 1 (INJ_CONV_WRONG) | ConvertElementarySurfacesTests.swift:37 simd_length(pointAt30x60(s) - SIMD3(4.6488686489193523, ...)) < 1e-12 | ✅ | MATCH | Rewritten: asserted only non-nil; the domain pin at :36 (uMax == pi, vMax == 10, as for the cylinder) is red under a second injection that keeps the geometry and doubles the U knots, where the point pin stays green, and under v2 + 1 |
| Convert Elementary Surfaces Tests | fullTorus | `OCCTConvertTorusToBSplineSurface` | major radius + 1 (INJ_CONV_WRONG) | ConvertElementarySurfacesTests.swift:48 simd_length(pointAt30x60(s) - SIMD3(-5.3865777080062784, ...)) < 1e-12 | ✅ | MATCH | Rewritten: asserted only non-nil |

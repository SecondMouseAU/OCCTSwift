...
### Measured: Issue619SurfaceContinuityEncodingTests.swift, Issue623ContinuityFloorTests.swift (11 tests), probe Scripts/repro/766-issue562-571-619-623/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| Surface measured continuity encoding after the retirement (#619) | An analytic surface reports CN as ordinal 6, the old encoding's 99 is unreachable | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (INJ_485_OLDENC) | Issue619SurfaceContinuityEncodingTests.swift:25 surface.continuityClass == .cN | ✅ | MATCH |  |
| Surface measured continuity encoding after the retirement (#619) | A C1 surface reports C1 as ordinal 2, not 1 | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (INJ_485_OLDENC) | Issue619SurfaceContinuityEncodingTests.swift:41 c1.continuityClass == .c1 | ✅ | MATCH |  |
| Surface measured continuity encoding after the retirement (#619) | The sibling bezierContinuity accessor uses the same encoding, so CN is 6 there too | `OCCTSurfaceBezierContinuity` | bezierContinuity returns 4 (INJ_619_BZ_OLD) | Issue619SurfaceContinuityEncodingTests.swift:58 bezier.bezierContinuity == 6 | ✅ | MATCH | Also reaches OCCTSurfaceGetContinuity |
| Surface measured continuity encoding after the retirement (#619) | A raw threshold of 2 now admits a merely-C1 surface; satisfies(.c2) still refuses it | `OCCTSurfaceGetContinuity` | satisfies() by raw-value comparison (INJ_485_RAWCOMPARE, Swift) | Issue619SurfaceContinuityEncodingTests.swift:70 !c1.continuityClass.satisfies(.c2) | ✅ | MATCH |  |
| ContinuityClass floor checks agree with the ordering (#623) | A geometric class meets the C0 floor, and nothing above it | `(Swift) ContinuityClass.satisfies` | satisfies() fails every geometric class, the pre-#623 form (INJ_623_GEOM_NIL_FAILS, Swift); also red under satisfies() by raw-value comparison (INJ_485_RAWCOMPARE, Swift) | Issue623ContinuityFloorTests.swift:38 ContinuityClass.g1.satisfies(.c0) | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | Every measured class clears the positional floor | `(Swift) ContinuityClass.satisfies` | satisfies() fails every geometric class, the pre-#623 form (INJ_623_GEOM_NIL_FAILS, Swift) | Issue623ContinuityFloorTests.swift:61 measured.satisfies(.c0) | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | satisfies() and >= agree across the whole 7x7 matrix, bar one documented pair | `(Swift) ContinuityClass` | `<` reversed (INJ_485_LT_REVERSED); also red under satisfies() by raw-value comparison (INJ_485_RAWCOMPARE, Swift) and satisfies() fails every geometric class, the pre-#623 form (INJ_623_GEOM_NIL_FAILS, Swift) | Issue623ContinuityFloorTests.swift disagreements.count == 1 | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | A floor check never claims more than the ordering allows | `(Swift) ContinuityClass` | satisfies() by raw-value comparison (INJ_485_RAWCOMPARE, Swift) | Issue623ContinuityFloorTests.swift:116 measured >= required | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | The floor is monotonic in the requested order, and in the measured class bar one pair | `(Swift) ContinuityClass` | satisfies() fails every geometric class, the pre-#623 form (INJ_623_GEOM_NIL_FAILS, Swift) | Issue623ContinuityFloorTests.swift:163 violations.count == 1 | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | The junction-analysis bitmask contract is independent of the floor check | `(Swift) ContinuityClass.analysisFlagBit` | flag bit shifted by one (INJ_623_FLAGBIT, Swift) | Issue623ContinuityFloorTests.swift:18 measured.analysisFlagBit == 1 << Int(measured.rawValue) | ✅ | N/A |  |
| ContinuityClass floor checks agree with the ordering (#623) | The floor gate holds on geometry OCCT actually measured | `OCCTSurfaceGetContinuity` | OCCTSurfaceGetContinuity returns the pre-#485 C-level encoding (INJ_485_OLDENC) | Issue623ContinuityFloorTests.swift plane.continuityClass == .cN | ✅ | MATCH | Both fixtures sat behind `if let`; each is now asserted to exist first |

...
### Measured: Issue838FindSurfaceConsolidationTests.swift, Issue965SurfacePropertyLifetimeTests.swift, Issue999NLPlateParametersTests.swift (13 tests), probe Scripts/repro/766-issue838-findsurface/

| Suite | Test | Bridge function | Injection | Red (failing expectation) | Green | Parity | Notes |
|-------|------|-----------------|-----------|---------------------------|-------|--------|-------|
| Issue #838, findSurface bridge consolidation | findSurface(tolerance:) forwards an explicit non-default tolerance, not a hardcoded one | `OCCTShapeFindSurface` | tolerance replaced by the -1 default (INJ_FS_TOL) | Issue838FindSurfaceConsolidationTests.swift:76 found != nil | ✅ | MATCH | Caught as written |
| Issue #838, findSurface bridge consolidation | findSurface(tolerance:) (1-param) behaves as onlyPlane: false, matching the explicit call | `OCCTShapeFindSurface` | onlyPlane inverted (INJ_FS_ONLYPLANE_INV) | Issue838FindSurfaceConsolidationTests.swift:90 bare != nil | ✅ | MATCH | Caught as written |
| Issue #838, findSurface bridge consolidation | findSurface(tolerance:onlyPlane: true) finds nothing on a wire with only a non-planar existing surface | `OCCTFindSurface` | onlyPlane inverted (INJ_FS_ONLYPLANE_INV) | Issue838FindSurfaceConsolidationTests.swift:105 onlyPlaneTrue == nil | ✅ | MATCH | Caught as written |
| Issue #838, findSurface bridge consolidation | findSurface, findSurface(onlyPlane: false) and findSurfaceEx(onlyPlane: false) agree at matching tolerance | `OCCTShapeFindSurfaceEx` | onlyPlane inverted (INJ_FS_ONLYPLANE_INV) | Issue838FindSurfaceConsolidationTests.swift:127 viaBare != nil (and 8 more) | ✅ | MATCH | Caught as written; also reaches OCCTFindSurface, OCCTFindSurfaceTolerance, OCCTFindSurfaceExisted |
| Surface *Properties views keep their parent alive (#965) | every Surface *Properties accessor keeps its parent alive | `Surface.sphereProperties (Swift, no bridge)` | view built on a copy of the surface, so it does not hold the parent (INJ_965_COPY) | Issue965SurfacePropertyLifetimeTests.swift:54 result.parentAlive | ✅ | N/A | Caught as written (sphereProperties row) |
| Surface *Properties views keep their parent alive (#965) | a view outliving its parent still reads the right values | `Surface.sphereProperties (Swift, no bridge)` | view on a copy (INJ_965_COPY) | Issue965SurfacePropertyLifetimeTests.swift:64 result.parentAlive | ✅ | N/A | Caught as written |
| Surface *Properties views keep their parent alive (#965) | a view outliving its parent survives 400 intervening allocations | `Surface.sphereProperties (Swift, no bridge)` | view on a copy (INJ_965_COPY): stays GREEN, a copy reads the same radius | not red: only the original use-after-free would change the radius read, and it is not safely injectable | ✅ | N/A | NOT PROVEN RED |
| Surface *Properties views keep their parent alive (#965) | a setter called through a view is visible on the parent | `OCCTSurfaceSphereSetRadius` | view on a copy (INJ_965_COPY) | Issue965SurfacePropertyLifetimeTests.swift:86 surface.sphereProperties.radius == 13 | ✅ | N/A | Caught as written |
| NLPlate G2/G3 parameters are live (#999) | The constraint target moves the G2 result | `OCCTSurfaceNLPlateG2` | target z fixed at 1 (INJ_999_TARGET_CONST) | Issue999NLPlateParametersTests.swift:129 maxDelta > 1.0 | ✅ | N/A | Caught as written |
| NLPlate G2/G3 parameters are live (#999) | Repeating the same G2 request gives the same surface | `OCCTSurfaceNLPlateG2` | target nudged by a call counter (INJ_999_NONDET) | Issue999NLPlateParametersTests.swift:152 maxDelta == 0 | ✅ | N/A | Caught as written |
| NLPlate G2/G3 parameters are live (#999) | The constraint target moves the G3 result | `OCCTSurfaceNLPlateG3` | target z fixed at 1 (INJ_999_TARGET_CONST) | Issue999NLPlateParametersTests.swift:178 maxDelta > 1.0 | ✅ | N/A | Caught as written |
| NLPlate G2/G3 parameters are live (#999) | Varying the tolerance changes the G2 result | `OCCTSurfaceNLPlateG2` | tolerance fixed at 1e-3 (INJ_999_TOL_FIXED) | Issue999NLPlateParametersTests.swift:201 maxDelta > 1e-3 | ✅ | N/A | Caught as written; see kernel note on why the delta is ~2e5 |
| NLPlate G2/G3 parameters are live (#999) | Varying the tolerance changes the G3 result | `OCCTSurfaceNLPlateG3` | tolerance fixed at 1e-3 (INJ_999_TOL_FIXED) | Issue999NLPlateParametersTests.swift:220 maxDelta > 1e-3 | ✅ | N/A | Caught as written |

...
| #1193: addCuttingPlaneLine direction projection | addCuttingPlaneLine's traceStart/traceEnd/arrowDirection match a hand-derived projection | `Drawing.addCuttingPlaneLine (Swift) vs gp_Ax2` | `perpendicularBasis` returns `(up, right)` | `:87`, `:90`, `:93` | ✔ | N/A (Swift annotation geometry, no kernel counterpart) |
...
| Drawing.ProjectionType is honoured (#999) | A non-positive focal distance is refused rather than silently reinterpreted | `OCCTDrawingCreate` | `!(focus > 0)` check removed (0 and -100 are still refused by the reach guard; NaN is not) | `:100` (NaN) | ✔ | EXPECTED_DIVERGENCE |

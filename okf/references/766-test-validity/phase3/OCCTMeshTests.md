...
## Evidence correction pass

The parity records in `kernel-parity/OCCTMeshTests.json` were corrected in a second pass. For 62 records the bridge and kernel payloads carried different keys or value types
under an `equal` of true; both sides now carry the same quantity under the same keys, with a `source` note
on each record saying how the raw values map. Bridge values are in
`Scripts/repro/766-mesh-evidence-fix/bridge-observed.txt`, kernel values in the committed `transcript.txt`
files. Two refusal records became N/A (bridge policy with no kernel value), three Boolean records became
EXPECTED_DIVERGENCE (#2301). The Parity column below is the outcome for each row.
...
| Mesh Tests | Mesh boolean union | `OCCTMeshUnion` | C2: occtMeshBoolean returns nullptr | :285 `unionMesh != nil` | ✔ | EXPECTED_DIVERGENCE (#2301): bridge 72 triangles, volume 2000 vs the solid Boolean's 28 triangles, volume 1500, asserted under withKnownIssue |
| Mesh Tests | Mesh boolean subtraction | `OCCTMeshSubtract` | C2: occtMeshBoolean returns nullptr | :305 `diffMesh != nil` | ✔ | EXPECTED_DIVERGENCE (#2301): bridge 120 triangles, volume 1000 (nothing removed) vs the solid Boolean's 116 triangles, volume 860.0003, asserted under withKnownIssue |
| Mesh Tests | Mesh boolean intersection | `OCCTMeshIntersect` | C2: occtMeshBoolean returns nullptr | :323 `intersectMesh != nil` | ✔ | EXPECTED_DIVERGENCE (#2301): bridge an empty mesh (0 triangles) vs the solid Boolean's 332 triangles, volume 955.42, asserted under withKnownIssue |

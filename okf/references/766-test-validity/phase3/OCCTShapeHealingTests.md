...
## #766 measured: Issue446 unify input, Issue484 connectedFaces and Face.fixed (15 tests)
Red = the failing expectation under the named injection (env-gated `INJ766` token in the bridge, one build); Green = same build, no token. Parity against `Scripts/repro/766-healing-446-484/transcript.txt`.
| `unifiedDoesNotMutateInput` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | UNIFYNOCOPY: unify the caller's shape in place, no BRepBuilderAPI_Copy (the #446 defect) | `Issue446UnifyInputMutationTests.swift:45:9: Expectation failed: serialized(body) == before` | pass | PASS |
| `resultDoesNotShareSubShapesWithInput` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | UNIFYNOCOPY: unify the caller's shape in place, no BRepBuilderAPI_Copy (the #446 defect) | `Issue446UnifyInputMutationTests.swift:66:9: Expectation failed: merged.isSame(as: box) == false` | pass | PASS |
| `builderDoesNotMutateInput` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | UNIFYNOCOPY: unify the caller's shape in place, no BRepBuilderAPI_Copy (the #446 defect) | `Issue446UnifyInputMutationTests.swift:88:9: Expectation failed: serialized(body) == before` | pass | PASS |
| `declinedMergeLeavesInputIntact` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | USDNOBUILD: UnifySameDomainBuilder.build() skips Build() | `Issue446UnifyInputMutationTests.swift:115:9: Expectation failed: builder.shape?.subShapeCount(ofType: .face) == 3` | pass | PASS |
| `simplifiedDoesNotMutateInput` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | UNIFYNOCOPY: unify the caller's shape in place, no BRepBuilderAPI_Copy (the #446 defect) | `Issue446UnifyInputMutationTests.swift:123:9: Expectation failed: serialized(body) == before` | pass | PASS |
| `keepShapeSurvivesTheCopy` | Issue446UnifyInputMutationTests.swift | `occtUnifySameDomain (OCCTShapeUnifySameDomain)` | KEEPNOMAP: KeepShape handed the caller's edge, not its image in the copy | `Issue446UnifyInputMutationTests.swift:156:17: Expectation failed: facesKeeping(edge) == 4` | pass | PASS |
| `boxShellRoundTrips` | Issue484ConnectedFacesTests.swift | `OCCTShapeFixFaceConnect` | CONNECTNULL: report failure (nil) after connecting | `Issue484ConnectedFacesTests.swift:32:25: Issue recorded` | pass | PASS |
| `explicitTolerance` | Issue484ConnectedFacesTests.swift | `OCCTShapeFixFaceConnect` | CONNECTNULL: report failure (nil) after connecting | `Issue484ConnectedFacesTests.swift:46:29: Issue recorded` | pass | PASS |
| `multiShellCompoundKeepsEveryShell` | Issue484ConnectedFacesTests.swift | `OCCTShapeFixFaceConnect` | CONNECTFIRST: keep only the first shell (first-of-N) | `Issue484ConnectedFacesTests.swift:75:9: Expectation failed: connected.faces().count == 12` | pass | PASS |
| `noShellReturnsNil` | Issue484ConnectedFacesTests.swift | `OCCTShapeFixFaceConnect` | CONNECTNOSHELL: wrap a shell-less shape's faces into a shell instead of refusing | `Issue484ConnectedFacesTests.swift:87:9: Expectation failed: face.connectedFaces() == nil` | pass | PASS |
| `genuinelyDisconnectedFacesGetConnected` | Issue484ConnectedFacesTests.swift | `OCCTShapeFixFaceConnect` | CONNECTNOSEW: keep the unconnected shell instead of ShapeFix_FaceConnect's result | `Issue484ConnectedFacesTests.swift:127:9: Expectation failed: connected.edgeCount == 7` | pass | PASS |
| `periodicConicalFaceSurvivesFaceFix` | Issue484FaceFixContextTests.swift | `OCCTFaceFix` | FACEFIXNULL: return nullptr before ShapeFix_Face runs | `Issue484FaceFixContextTests.swift:59:25: Issue recorded` | pass | N/A: fixture built through interpolate + Surface.cone + face(from:boundary:); not reproduced in C++. The ReShape-context removal left all four green on this patched kernel (#317's null deref is fixed upstream), so the context is backstopped, not isolated |
| `unhealedPeriodicConicalUVFaceSurvivesFaceFix` | Issue484FaceFixContextTests.swift | `OCCTFaceFix` | FACEFIXNULL: return nullptr before ShapeFix_Face runs | `Issue484FaceFixContextTests.swift:91:25: Issue recorded` | pass | N/A: fixture built through Surface.cone.toFace(uvBoundary:); not reproduced in C++ |
| `wellFormedBoxFacesUnaffected` | Issue484FaceFixContextTests.swift | `OCCTFaceFix` | FACEFIXNULL: return nullptr before ShapeFix_Face runs | `Issue484FaceFixContextTests.swift:109:29: Issue recorded` | pass | PASS |
| `cylinderFacesUnaffected` | Issue484FaceFixContextTests.swift | `OCCTFaceFix` | FACEFIXNULL: return nullptr before ShapeFix_Face runs (red at :135 and :142); FACEFIXDUP: return the face and a copy of it in a compound (red at :139 and :142); FACEFIXEMPTY: return an empty compound (red at :139 and :142). The earlier isValid-only test stayed green under FACEFIXDUP and FACEFIXEMPTY | `Issue484FaceFixContextTests.swift:135:29: Issue recorded`, `:139` `fixed.faces().count == 1`, `:142` `validSingle == 3` | pass | PASS: 3 faces, 3 valid single faces, bridge and kernel equal |

...
| `divideOnePart` | DivideByNumberTests.swift | `OCCTShapeDivideByNumber` | DIVNUM1: bypass the Swift parts>1 guard and return the input when Perform() is false | DivideByNumberTests.swift:34 `result == nil` | pass | N/A: the nil is the Swift `parts > 1` guard, which returns before any kernel call; the kernel also reports Perform false at (1,1) |
...
| `fixSolidNoSolid` | Issue442FixSolidMultiBodyTests.swift | `OCCTShapeSolidFromShell (occtBodyBoundingShells)` | FIXSOLIDPASS: fixSolid hands back the input when it holds no solid | `Issue442FixSolidMultiBodyTests.swift:108:9: Expectation failed: face.fixSolid() == nil` | pass | N/A: the nil comes from the bridge's own empty body list (no TopAbs_SOLID in a face), before any kernel call |
...
| `solidFromShellNoShell` | Issue442FixSolidMultiBodyTests.swift | `OCCTShapeSolidFromShell (occtBodyBoundingShells)` | SHELLFROMFACES: a shell-less input's faces wrapped into a shell instead of refusing | `Issue442FixSolidMultiBodyTests.swift:271:9: Expectation failed: face.solidFromShellFixed() == nil` | pass | N/A: the nil comes from the bridge's own empty body list (no shell in a face), before any kernel call |
...
| `solidFromNoShell` | Issue443FirstOfNTests.swift | `occtBodyBoundingShells via OCCTShapeCreateSolidFromShell / OCCTShapeUpgrade` | SHELLFROMFACES: a shell-less input's faces wrapped into a shell instead of refusing | `Issue443FirstOfNTests.swift:175:9: Expectation failed: Shape.solid(from: face) == nil` | pass | N/A: the nil comes from the bridge's own empty body list (no shell in a face), before any kernel call |
...
| `conversionGapIsDeprecatedSentinel` | ShapeCustomSurfacePeriodicTests.swift | `OCCTSurfaceConvertToPeriodic / OCCTSurfaceConversionGap` | GAPSENTINEL: deprecated gap reported as 0.0 instead of the -1.0 sentinel | `ShapeCustomSurfacePeriodicTests.swift:37:9: Expectation failed: bsp.conversionGap == -1.0` | pass | EXPECTED_DIVERGENCE: the deprecated accessor returns the constant -1.0 (#1510); the kernel's ShapeCustom_Surface::Gap() is 0.0 before and after ConvertToPeriodic |
...
| `multipleEdgeAtInvalidIndex` | SewingExtrasTests.swift | `OCCTSewingNbMultipleEdges / OCCTSewingMultipleEdge` | MULTIIDXOFF: multiple-edge index off by one | `SewingExtrasTests.swift:46:9: Expectation failed: s.multipleEdge(at: 1) != nil` | pass | N/A: indexes 2 and 999 are refused by the bridge's own range guard; the kernel's MultipleEdge has no usable range check in this build (a forked probe child died on signal 11); index 1 agrees |
...
## #766 measured: ShapeFix ComposeShell / EdgeConnect / Edge / EdgeExtended / EdgeProjAux / FixerBuilder / Fixing / IntersectionTool / SmallSolid / Solid / SplitCommonVertex / Tolerance / WireframeExt / WireVertex (27 tests)
Red = the failing expectation under the named injection (env-gated `INJ766` token in the bridge, one build); Green = same build, no token. Parity against `Scripts/repro/766-healing-shapefix/transcript.txt`.
| `composeShellPlanar` | ShapeFixComposeShellTests.swift | `OCCTShapeFixComposeShell` | CSNULL: composeShell returns nil | `ShapeFixComposeShellTests.swift:16:26: Expectation failed: face.composeShell()` | pass | PASS: kept as written (rewritten under #1638) |
| `fixEdgeConnect` | ShapeFixEdgeConnectTests.swift | `OCCTShapeFixEdgeConnect` | ECNULL: fixEdgeConnect returns nil | `ShapeFixEdgeConnectTests.swift:14:25: Expectation failed: box.fixEdgeConnect()` | pass | PASS: the test pins faces and volume; the kernel's result is also BRepCheck-invalid (3 of 6 wires, 5 of 6 faces), recorded with withKnownIssue in PR #2303 |
| `fixSameParameter` | ShapeFixEdgeTests.swift | `OCCTShapeFixEdgeSameParameter / VertexTolerance` | SFECOUNT: every edge / wire counted as fixed | `ShapeFixEdgeTests.swift:15:9: Expectation failed: box.fixEdgeSameParameter() == 0` | pass | PASS |
| `fixVertexTolerance` | ShapeFixEdgeTests.swift | `OCCTShapeFixEdgeSameParameter / VertexTolerance` | SFECOUNT: every edge / wire counted as fixed | `ShapeFixEdgeTests.swift:21:9: Expectation failed: box.fixEdgeVertexTolerance() == 0` | pass | PASS |
| `addRemoveCurve3d` | ShapeFixEdgeExtendedTests.swift | `OCCTShapeFixEdge*` | SFEINV: every ShapeFix_Edge answer inverted | `ShapeFixEdgeExtendedTests.swift:24:9: Expectation failed: Shape.fixEdgeRemoveCurve3d(edge) == false` | pass | PASS |
| `addPCurve` | ShapeFixEdgeExtendedTests.swift | `OCCTShapeFixEdge*` | SFEINV: every ShapeFix_Edge answer inverted | `ShapeFixEdgeExtendedTests.swift:32:9: Expectation failed: Shape.fixEdgeAddPCurve(edge, face: face, isSeam: false) == false` | pass | PASS |
| `removePCurve` | ShapeFixEdgeExtendedTests.swift | `OCCTShapeFixEdge*` | SFEINV: every ShapeFix_Edge answer inverted | `ShapeFixEdgeExtendedTests.swift:38:9: Expectation failed: Shape.fixEdgeRemovePCurve(edge, face: face) == false` | pass | PASS |
| `fixReversed2d` | ShapeFixEdgeExtendedTests.swift | `OCCTShapeFixEdge*` | SFEINV: every ShapeFix_Edge answer inverted | `ShapeFixEdgeExtendedTests.swift:44:9: Expectation failed: Shape.fixEdgeReversed2d(edge, face: face) == false` | pass | PASS |
| `projectEdge` | ShapeFixEdgeProjAuxTests.swift | `OCCTShapeFixEdgeProjAux` | PROJAUXSWAP: FirstParam answered with LastParam | `ShapeFixEdgeProjAuxTests.swift:15:9: Expectation failed: result.first == 0` | pass | PASS |
| `basicFixer` | ShapeFixerBuilderTests.swift | `OCCTShapeFixer*` | FIXERPERFINV: Perform() answer inverted | `ShapeFixerBuilderTests.swift:19:9: Expectation failed: fixer.perform() == false` | pass | PASS |
| `fixerStatus` | ShapeFixerBuilderTests.swift | `OCCTShapeFixer*` | FIXERSTATINV: Status() answer inverted | `ShapeFixerBuilderTests.swift:30:9: Expectation failed: !fixer.status(3)` | pass | PASS |
| `fixHealthyShape` | ShapeFixingTests.swift | `OCCTShapeFixDetailed / OCCTShapeHeal` | FIXDETAILEDNULL: detailed fix returns nil | `ShapeFixingTests.swift:15:25: Expectation failed: box.fixed(tolerance: 0.001)` | pass | PASS |
| `fixWithSelectiveModes` | ShapeFixingTests.swift | `OCCTShapeFixDetailed / OCCTShapeHeal` | FIXDETAILEDNULL: detailed fix returns nil | `ShapeFixingTests.swift:24:25: Expectation failed: box.fixed(tolerance: 0.001, fixSolid: false, fixShell: true, fixFace: true, fixWire: true)` | pass | PASS |
| `existingHealStillWorks` | ShapeFixingTests.swift | `OCCTShapeFixDetailed / OCCTShapeHeal` | HEALNULL: heal refuses every shape | `ShapeFixingTests.swift:33:26: Expectation failed: box.healed()` | pass | PASS |
| `fixIntersectingWires` | ShapeFixIntersectionToolTests.swift | `OCCTShapeFixIntersectingWires` | ITOOLINV: FixIntersectingWires answer inverted | `ShapeFixIntersectionToolTests.swift:14:9: Expectation failed: !fixed` | pass | PASS |
| `fixIntersectingWiresActuallyAppliesTheFix` | ShapeFixIntersectionToolTests.swift | `OCCTShapeFixIntersectingWires` | ITOOLINV: FixIntersectingWires answer inverted | `ShapeFixIntersectionToolTests.swift:40:9: Expectation failed: fixed` | pass | PASS |
| `removeSmallSolids` | ShapeFixSmallSolidTests.swift | `OCCTShapeFixSmallSolid*` | SMALLSOLIDTHR: volume threshold ignored (0) | `ShapeFixSmallSolidTests.swift:21:9: Expectation failed: result.solids.count == 1` | pass | PASS |
| `mergeSmallSolids` | ShapeFixSmallSolidTests.swift | `OCCTShapeFixSmallSolid*` | MERGENULL: merge returns nil | `ShapeFixSmallSolidTests.swift:32:26: Expectation failed: compound.mergeSmallSolids(widthFactorThreshold: 1.0)` | pass | PASS |
| `fixSolid` | ShapeFixSolidTests.swift | `OCCTShapeFixSolid / OCCTShapeSolidFromShell` | SINGLECOMP: a single-body result left wrapped in its compound | `ShapeFixSolidTests.swift:16:9: Expectation failed: fixed.shapeType == .solid` | pass | PASS |
| `solidFromShellFixed` | ShapeFixSolidTests.swift | `OCCTShapeFixSolid / OCCTShapeSolidFromShell` | SINGLECOMP: a single-body result left wrapped in its compound | `ShapeFixSolidTests.swift:23:9: Expectation failed: result.shapeType == .solid` | pass | PASS |
| `splitVertices` | ShapeFixSplitCommonVertexTests.swift | `OCCTShapeSplitCommonVertex` | SPLITCVNULL: SplitCommonVertex returns nil | `ShapeFixSplitCommonVertexTests.swift:16:21: Expectation failed: box.splitCommonVertices()` | pass | PASS |
| `setTolerance` | ShapeFixToleranceTests.swift | `OCCTShapeSetTolerance / OCCTShapeLimitTolerance` | SETTOLX10: tolerance set 10x too large | `ShapeFixToleranceTests.swift:18:9: Expectation failed: abs(box.toleranceValue(mode: .maximum) - 1e-5) < 1e-15` | pass | PASS |
| `limitTolerance` | ShapeFixToleranceTests.swift | `OCCTShapeSetTolerance / OCCTShapeLimitTolerance` | LIMITTOLSWAP: lower limit replaced by the upper one | `ShapeFixToleranceTests.swift:26:9: Expectation failed: box.limitTolerance(min: 1e-7, max: 1e-3) == false` | pass | PASS |
| `fixWireGapsReturnsShape` | ShapeFixWireframeExtTests.swift | `OCCTShapeFixWireGaps / OCCTShapeFixSmallEdges` | WIREGAPSNULL: FixWireGaps returns nil | `ShapeFixWireframeExtTests.swift:18:25: Expectation failed: try box().fixWireGaps(tolerance: 1e-7)` | pass | PASS |
| `fixSmallEdgesDropMode` | ShapeFixWireframeExtTests.swift | `OCCTShapeFixWireGaps / OCCTShapeFixSmallEdges` | SMALLTOLHUGE: precision forced to 20, so every edge of the 10-box is small | `ShapeFixWireframeExtTests.swift:26:9: Expectation failed: fixed.isValid` | pass | PASS |
| `fixSmallEdgesMergeMode` | ShapeFixWireframeExtTests.swift | `OCCTShapeFixWireGaps / OCCTShapeFixSmallEdges` | SMALLTOLHUGE + MERGEASDROP: the dropSmall flag inverted at a precision where every edge is small (run M2). SMALLTOLHUGE alone stays green (D2): merge mode with the 0.01 rad angle limit refuses to merge edges meeting at 90 degrees, and so does SMALLTOLHUGE + MERGEANGLEPI (M1), which ignores the angle limit | `ShapeFixWireframeExtTests.swift:33:9: Expectation failed: fixed.isValid` | pass | PASS |
| `fixWireVertices` | ShapeFixWireVertexTests.swift | `OCCTShapeFixWireVertices` | SFECOUNT: every edge / wire counted as fixed | `ShapeFixWireVertexTests.swift:15:9: Expectation failed: box.fixWireVertices(precision: 1e-4) == 0` | pass | PASS |
## #766 measured: Issue837, Issue839, Issue849, Issue870, Issue999 (14 tests)
Red = the failing expectation under the named injection (env-gated `INJ766` token in the bridge, one build); Green = same build, no token. Parity against `Scripts/repro/766-healing-837-839-849-870-999/transcript.txt`.
| `fixtureHasBadOrientation` | Issue837FixDetailedModeFlagsTests.swift | `OCCTShapeFixDetailed (FixFreeFaceMode) / OCCTCheckFaceWireOrientation` | ORIENTOK: wire-orientation check reports NoError for every face | `Issue837FixDetailedModeFlagsTests.swift:79:9: Expectation failed: face.checkFaceWireOrientation() == .badOrientationOfSubshape` | pass | PASS |
| `fixFaceFalseLeavesDefectInPlace` | Issue837FixDetailedModeFlagsTests.swift | `OCCTShapeFixDetailed (FixFreeFaceMode) / OCCTCheckFaceWireOrientation` | FIXFACEON: FixFreeFaceMode forced on | `Issue837FixDetailedModeFlagsTests.swift:99:9: Expectation failed: resultFace.checkFaceWireOrientation() == .badOrientationOfSubshape` | pass | PASS |
| `fixFaceTrueCorrectsDefect` | Issue837FixDetailedModeFlagsTests.swift | `OCCTShapeFixDetailed (FixFreeFaceMode) / OCCTCheckFaceWireOrientation` | FIXFACEOFF: FixFreeFaceMode forced off | `Issue837FixDetailedModeFlagsTests.swift:119:9: Expectation failed: resultFace.checkFaceWireOrientation() == .noError` | pass | PASS |
| `defaultsAgreeOnBorderlineEdge` | Issue839SmallEdgeToleranceAlignmentTests.swift | `OCCTShapeDropSmallEdges / OCCTShapeFixSmallEdges` | SMALLTOLFIXED: fixSmallEdges precision fixed at 1.0 instead of the shared default | `Issue839SmallEdgeToleranceAlignmentTests.swift:52:9: Expectation failed: dropped.edges().count == fixed.edges().count` | pass | PASS |
| `bothAgreeAtASharedExplicitTolerance` | Issue839SmallEdgeToleranceAlignmentTests.swift | `OCCTShapeDropSmallEdges / OCCTShapeFixSmallEdges` | SMALLNOOP: fixSmallEdges hands back its input | `Issue839SmallEdgeToleranceAlignmentTests.swift:91:9: Expectation failed: dropped.edges().count == fixed.edges().count` | pass | PASS |
| `rawValuesMatchTheRealOCCTEnum` | Issue849ShapeFixStatusTests.swift | `ShapeFixStatus / OCCTShapeFixerStatusFlag` | ENUMSHIFT (a source edit, not an env switch; built, run and reverted): `ShapeFixStatus` restored to the pre-#849 shift, `fail1 = 9` ... `fail8 = 16`, `done = 17` | `Issue849ShapeFixStatusTests.swift:32:9: Expectation failed: ShapeFixStatus.done.rawValue == 9` (and 9 more, fail1 through fail8; run E1) | pass | PASS |
| `faceFixerStatusIsTheSharedType` | Issue849ShapeFixStatusTests.swift | `ShapeFixStatus / OCCTShapeFixerStatusFlag` | ENUMSHIFT (as above) | `Issue849ShapeFixStatusTests.swift:48:9: Expectation failed: FaceFixer.Status.done.rawValue == 9` (and :49, :50; run E1) | pass | PASS |
| `legacyAndTypeSafeOverloadsAgree` | Issue849ShapeFixStatusTests.swift | `ShapeFixStatus / OCCTShapeFixerStatusFlag` | LEGACYMAP: legacy status(1) mapped to DONE instead of OK | `Issue849ShapeFixStatusTests.swift:72:9: Expectation failed: fixer.status(1) == fixer.status(.ok)` | pass | PASS |
| `legacyOverloadStaysNarrow` | Issue849ShapeFixStatusTests.swift | `ShapeFixStatus / OCCTShapeFixerStatusFlag` | LEGACYWIDE: legacy status(Int) passes any ordinal through | `Issue849ShapeFixStatusTests.swift:90:9: Expectation failed: !fixer.status(4)` | pass | EXPECTED_DIVERGENCE: the legacy overload answers false outside 1 to 3 without asking the kernel; ShapeFix_Shape::Status is true for DONE4 and DONE (9) after Perform on a box |
| `nullShapeReturnsUnknownSentinel` | Issue870ShapeExtendShapeTypeFailureTests.swift | `OCCTShapeExtendShapeType` | NULLTYPE7: null-shape sentinel 7 (TopAbs_VERTEX) instead of -1 | `Issue870ShapeExtendShapeTypeFailureTests.swift:22:9: Expectation failed: OCCTShapeExtendShapeType(nil, true) == -1` | pass | N/A: a null C pointer; the bridge returns -1 before any kernel call |
| `sentinelDecodesToUnknownNotVertex` | Issue870ShapeExtendShapeTypeFailureTests.swift | `OCCTShapeExtendShapeType` | NULLTYPE7: null-shape sentinel 7 (TopAbs_VERTEX) instead of -1 | `Issue870ShapeExtendShapeTypeFailureTests.swift:30:9: Expectation failed: ShapeType(rawValue: Int(raw)) == .unknown` | pass | N/A: the -1 sentinel and the Swift .unknown case are not kernel values; no kernel call |
| `ordinaryShapeUnaffected` | Issue870ShapeExtendShapeTypeFailureTests.swift | `OCCTShapeExtendShapeType` | TYPEPLUS1: predominant type shifted by one | `Issue870ShapeExtendShapeTypeFailureTests.swift:37:9: Expectation failed: box.predominantShapeType() == .solid` | pass | PASS |
| `verdictVariesWithTheWire` | Issue999OuterBoundTests.swift | `OCCTSAWireCheckOuterBound` | OBFACEWIRE: CheckOuterBound run on the face's outer wire instead of the given wire | `Issue999OuterBoundTests.swift:33:9: Expectation failed: verdicts.filter { $0 == true }.count == 1` | pass | PASS |
| `plainFaceReportsNoProblem` | Issue999OuterBoundTests.swift | `OCCTSAWireCheckOuterBound` | OBVERDICTINV: CheckOuterBound verdict inverted | `Issue999OuterBoundTests.swift:49:9: Expectation failed: SAWireAnalysis.checkOuterBound(wire: wire, face: face) == false` | pass | PASS |
...
Red = the failing expectation under the named injection (env-gated `INJ766` token in the bridge, one build); Green = same build, no token. Parity against `Scripts/repro/766-healing-702-999/transcript.txt`. The probe is shared with the Issue837/839/849/870/999 section (PR #2533, merged), so it also covers those issues; the kernel-parity records for them live there, and this section carries the records for Issue702 and Issue772 only. Parity is N/A for `defaultDoesNotCheck` (asserts the kernel check is not run) and `totalProblemsReflectsOnlyWhatWasChecked` (Swift arithmetic over one kernel answer); see the `source` field of each record.
...
| `defaultDoesNotCheck` | Issue772SelfIntersectionAnalysisTests.swift | `OCCTShapeSelfIntersectsBounded (via Shape.analyze)` | SIDEFAULT: analyze() checks self-intersection even with no timeout | `Issue772SelfIntersectionAnalysisTests.swift:61:9: Expectation failed: analysis.hasSelfIntersection == nil` | pass | N/A |
...
| `totalProblemsReflectsOnlyWhatWasChecked` | Issue772SelfIntersectionAnalysisTests.swift | `OCCTShapeSelfIntersectsBounded (via Shape.analyze)` | SIINVERT: isSelfIntersecting's answer inverted | `Issue772SelfIntersectionAnalysisTests.swift:94:9: Expectation failed: checked.totalProblems == unchecked.totalProblems + 1` | pass | N/A |

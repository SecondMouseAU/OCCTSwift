...
| Empty boundary returns nothing | OCCTHatchLines | Degenerate boundary rejection | drop the boundary.count >= 3 guard in HatchPattern.generate AND the boundaryCount < 3 guard in OCCTHatchLines | ✅ | ✅ | Red: HatchTests.swift:58 `islandOnly.isEmpty` (the original assertion at :50 stays green under the same injection). Parity EXPECTED_DIVERGENCE, `Scripts/repro/766-hatch/`: the bridge guard returns 0 for the island-only case where the unguarded kernel path returns 3. Rewritten: could not fail. The kernel itself returns nothing for an empty boundary, so the original assertion passed with every guard removed. Island case added. |

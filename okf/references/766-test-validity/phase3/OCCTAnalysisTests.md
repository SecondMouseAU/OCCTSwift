...
| lineSphereIntersection | OCCTIntCSPoint | Curve-surface intersection points | x + 1 on every point | ✅ | ✅ | Rewritten: every assertion sat under `if let`s and a count check, so a nil line, sphere or result passed with no assertion run; now `try #require` on each (red at `IntCSResultsTests.swift:16` line nil, `:17` sphere nil, `:18` result nil, `:19` one point), count pinned to 2 and x tolerance 0.1 tightened to 1e-9 (the kernel gives (5,0,0) and (-5,0,0); x + 0.05 passed the old tolerance and is red at `:24`, `:25`) |
| Ray intersects box | OCCTIntCurvesFaceShapeIntersect | Ray-shape intersection | NbPnt forced to 0 | ✅ | ✅ | Rewritten: `guard let box else { return }` passed with no assertion when the box was nil; now `try #require` on the box and the result (red at `IntCurvesFaceShapeIntersectorTests.swift:11` box nil, `:12` no result), and `count >= 2` tightened to `== 2` (the kernel gives two hits; a third is red at `:19`) |
| Ray nearest intersection with sphere | OCCTIntCurvesFaceShapeIntersectNearest | Ray-shape nearest hit | hit z + 1 | ✅ | ✅ | Rewritten: `guard let sphere else { return }` passed with no assertion when the sphere was nil; now `try #require` on the sphere and the hit (red at `:24` sphere nil, `:25` no hit). x and y are asserted (they were not) and the z tolerance 0.1 is tightened to 1e-9 (the kernel gives (0,0,-5)): x, y or z + 0.05 passed the old test and are red at `:31`, `:32`, `:33` |
| Ray misses shape | OCCTIntCurvesFaceShapeIntersect | Ray-shape intersection | report the ray origin as a hit when NbPnt is 0 | ✅ | ✅ | Rewritten: `guard let box else { return }` passed with no assertion when the box was nil (now `try #require`, red at `:38`). A miss is nil today (the bridge returns false when NbPnt is 0); nil or an empty array are both accepted by design and only a reported hit fails: red at `:46` under the injection |
| edge crossing face | OCCTIntToolsBeanFaceIntersect | Edge-face coincident ranges | refuse after IsDone() | ✅ | ✅ | Rewritten: minSquareDistance >= 0 inside if-lets passed on a refused call. Already `try #require` on plane, face, edge and result (red at `IntToolsBeanFaceIntersectorTests.swift:17` to `:20`) |
| edge lying on face - coincident ranges | OCCTIntToolsBeanFaceIntersect | Edge-face coincident ranges | drop every range | ✅ | ✅ | Kernel reports a zero-length range [0, 0] for a 6-long edge lying in the face. Rewritten: the whole body sat under `if let`s (and a force-unwrapped plane), so a nil face, edge or result passed with no assertion run; now `try #require` on each (red at `:29` plane, `:30` face, `:31` edge, `:32` result) and on the range count, pinned from `>= 1` to `== 1` (red at `:33` with none or two) |
| Perpendicular box faces produce intersection line | OCCTIntToolsFaceFace | Face-face intersection | report 0 curves | ✅ | ✅ | Rewritten: the whole body sat under `if let`s, so a nil plane, face or result passed with no assertion run; now `try #require` on each (red at `IntToolsFaceFaceTests.swift:14` to `:18`) and `curves.count >= 1` tightened to `== 1` (the kernel gives one line; none or two is red at `:19`) |
| Coincident planes are tangent | OCCTIntToolsFaceFace | Face-face intersection | tangent forced false | ✅ | ✅ | Rewritten: the only assertion sat under three `if let`s, so a nil plane, face or result passed with no assertion run; now `try #require` on each (red at `:26` to `:30`) |

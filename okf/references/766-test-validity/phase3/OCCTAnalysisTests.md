...
| Rectangle min thickness equals half the short side | OCCTMedialAxisMinThickness | Medial axis | min thickness doubled | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: 0.1 to 1e-9 (the kernel gives exactly 2); min thickness x 1.005 passed the old tolerance and is red at `MedialAxisRectangleTests.swift:48` |
| Rectangle nodes have valid positions and distances | OCCTMedialAxisGetNode | Medial axis | node distance -1; node count + 1 | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: the loop asserted `distance > 0 || isOnBoundary` over whatever nodes came back, nothing at all with none; now `try #require(nodes.count == 6)` (`:64`) and the two interior nodes (2 from the boundary, at (+-3, 0)) and four corners (on it, at (+-5, +-2)) are pinned. Red at `:64` with no nodes or one missing, `:74`/`:75` with the boundary flag flipped, `:77` to `:79` with distance or x + 0.05, `:82` to `:84` for the corners |
| Rectangle arcs have valid node references | OCCTMedialAxisGetArc | Medial axis | second node index past the end | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: `arcs.count == ma.arcCount` and the index-range loop passed on an empty graph; now five arcs (`:100`) joining the pinned node pairs (`:101`). Red at `:100` with no arcs (arc count 0 and every arc refused, where the old test passed) or one dropped, and at `:101` with each arc's two node indices swapped, which stayed in range |
| Rectangle arc drawing produces polylines | OCCTMedialAxisDrawArc | Medial axis | one sample fewer than asked | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: it asserted count and finiteness only; the first and last points are pinned to (-5, 2) and (-3, 0) and read with `try #require` (`:132`, `:133`), not `if let`. Red at `:134` with the first point x + 0.05, `:135` with the last, and at `:132` with no points |
| Rectangle draw all produces one polyline per arc | OCCTMedialAxisDrawAll | Medial axis | last arc dropped | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: it asserted counts and `>= 2` points, and `polylines.count == ma.arcCount` passed with none; now `try #require(polylines.count == 5)` (`:153`), 16 points per arc and each arc's first and last point pinned (`:163`, `:164`). Red at `:153` with no arcs (arc count 0, where the old test passed), at `:163` with arc 3's first point x + 0.05 alone and at `:164` with arc 5's last alone, and at `:160`/`:161` with one arc drawn empty |
| Rectangle distance on arc interpolates between endpoints | OCCTMedialAxisDistanceOnArc | Medial axis | distance + 4t(1-t) off the endpoints | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it; now also asserts such an arc exists. The `guard ma.arcCount > 0 else { return }` that let an empty graph pass is `try #require` (`:176`, red with the arc count 0). Added pins the mean-of-ends check cannot make: arc 5 is 2 at its midpoint and arc 1 is 1 (`:198`, `:199`); a constant + 0.05 on every distance passes the old check and is red at both |
| Square medial axis has symmetric structure | OCCTMedialAxisMinThickness | Medial axis | min thickness doubled | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: `> 0` counts are now `== 4` arcs and `== 5` nodes (red at `MedialAxisVariousShapesTests.swift:24`/`:25` with either count + 1) and the min thickness tolerance 0.1 is 1e-9 (red at `:28` with x 1.005) |
...
| Narrow rectangle has small min thickness | OCCTMedialAxisMinThickness | Medial axis | min thickness doubled | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: 0.1 to 1e-9 (the kernel gives exactly 0.5); x 1.005 is red at `:77` |
...
| Medial axis nodes lie inside the shape boundary | OCCTMedialAxisGetNode | Medial axis | node x + 100 | ✅ | ✅ | Re-enabled: the suite was .disabled, so no defect could fail it. Tightened: the loop ran over whatever nodes came back, nothing with none; now `try #require(nodes.count == 6)` (`:152`) and the 0.1 slack around the rectangle is 1e-9 (the corner nodes sit on it). Red at `:152` with no nodes or one missing and at `:154` with x + 0.05 |

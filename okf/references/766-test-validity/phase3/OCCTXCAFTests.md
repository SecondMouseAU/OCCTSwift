...
| `qualifierRoundTrips` | `OCCTDocumentGetDimensionInfo` reports qualifier 0 | :42 Expectation failed: qualified.qualifier == .max | passed | `OCCTDocumentGetDimensionInfo` | PASS: qualifier 0, then Max (2), then 0, with value 20 and a simple dimension kept, on both sides |
| `angularQualifierRoundTrips` | `OCCTDocumentGetDimensionInfo` reports angular qualifier 0 | :69 Expectation failed: both.angularQualifier == .large | passed | `OCCTDocumentGetDimensionInfo` | PASS: qualifier Min (1) and angular qualifier Large (2) are independent on both sides |
| `decimalPlacesDistinguishAbsenceFromZero` | `OCCTDocumentGetDimensionInfo` tests decimal-place presence with `&&` instead of `or` | :102 Issue recorded | passed | `OCCTDocumentGetDimensionInfo` | PASS: (2, 3) and (0, 4) read back, and (0, 0) reads as absent, on both sides |
...
| `toleranceSemanticsRoundTrip` | `OCCTDocumentGetGeomToleranceInfo` reports material requirement 0 | :59 Expectation failed: tol.materialRequirement == .m | passed | `OCCTDocumentGetGeomToleranceInfo` | PASS: all neutral when fresh; diameter (1), M (1), projected (1), 15, 0.25 after; value 0.1, type Position (10), on both sides |
| `toleranceZeroValuesAreAbsence` | `OCCTDocumentGetGeomToleranceInfo` treats a zero zone value as present (`>=` for `>`) | :86 Expectation failed: tol.zoneModifierValue == nil | passed | `OCCTDocumentGetGeomToleranceInfo` | PASS: zero zone and max values read as absent and the projected zone (1) survives, on both sides |
...
| `datumPositionRoundTrips` | `OCCTDocumentGetDatumInfo` treats position 0 as a place in the frame (`>=` for `>`) | :123 Expectation failed: doc.datum(at: index)?.position == nil; :129 Expectation failed: doc.datum(at: index)?.position == nil | passed | `OCCTDocumentGetDatumInfo` | PASS: position 2 reads back and 0 reads as absent on both sides |
| `datumModifiersRoundTrip` | `OCCTDocumentGetDatumInfo` drops the valued modifier's value | :152 Expectation failed: datum.modifierWithValue?.value == 12.5 | passed | `OCCTDocumentGetDatumInfo` | PASS: the sequence 21, 2, 3 and the valued modifier Projected 12.5 are independent and clear separately, on both sides |
...
| `placementOnARectangleTargetStillWorks` | `OCCTDocumentSetDatumTargetPlacement` stores the width as the length | :73 Expectation failed: target?.length == 30 | passed | `OCCTDocumentSetDatumTargetPlacement` | PASS: a rectangle target keeps its type, number 1 and the 30 x 18 placement on both sides |
...
| `longNameRoundTrips` | `OCCTDocumentGetLayerName` writes an empty name | :37 Expectation failed: !name.isEmpty; :37 Expectation failed: !name.isEmpty | passed | `OCCTDocumentGetLayerName` | EXPECTED_DIVERGENCE: the test asserts the #2413 defect: 3 tool labels read as layers on the bridge, 0 layers in the kernel's layer table |
...
| `realModifierStillStoresItsValue` | `OCCTDocumentSetGeomToleranceZoneModifier` stores 0 whatever the value | :172 Expectation failed: tol.zoneModifierValue == 15.0 | passed | `OCCTDocumentSetGeomToleranceZoneModifier` | PASS: a real projected zone keeps its value 15 on both sides |
...
| `nodeTypeMatchesRealOCCTCategories` | `OCCTAssemblyGraphGetNodeType` answers 0 | :64 Expectation failed: graph.nodeType(at: 1) == .assemblyRoot; :65 Expectation failed: graph.nodeType(at: 2) == .occurrence | passed | `OCCTAssemblyGraphGetNodeType` | PASS: 5 nodes, 1 root, types 1 3 2 3 4 (root, occurrence, subassembly, occurrence, part) on both sides |
...
| `setAndGet` | `OCCTDocumentGetAssemblyItemRef` returns null | :18 Expectation failed: path != nil | passed | `OCCTDocumentGetAssemblyItemRef` | PASS: a path is read back from the item ref on both sides (kernel "0:1:1:1") |
...
| `evolutionType` | `OCCTDocumentNamingGetEvolution` answers 99 | :64 Expectation failed: doc.namingEvolution(on: label) == .primitive | passed | `OCCTDocumentNamingGetEvolution` | PASS: a primitive record reads as PRIMITIVE on both sides |
...
| `deleteEvolution` | `OCCTDocumentNamingGetEvolution` answers 99 | :124 Expectation failed: doc.namingEvolution(on: label) == .delete | passed | `OCCTDocumentNamingGetEvolution` | PASS: primitive then delete reads as DELETE on both sides |

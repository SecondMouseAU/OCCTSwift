...
| `populatedDocumentReadsThroughOneFamily` | `OCCTDocumentGetDimensionInfo` reports the type ordinal plus 1 | :44 Expectation failed: single.type == .sizeDiameter | passed | `OCCTDocumentGetDimensionInfo` | PASS: 1 dimension, 1 tolerance, 1 datum, each plural as long as its count, entry 0 of each kind read twice alike, types 15 and 9, datum "A", on both sides |
| `rangeDimensionKeepsItsBounds` | `OCCTDocumentGetDimensionInfo` reports the first value slot as the value (the #996 defect) | :94 Expectation failed: dim.value.map { abs($0 - 11.0) < 1e-9 } == true | passed | `OCCTDocumentGetDimensionInfo` | PASS: range, not plus/minus, value 11 (the midpoint, not the first slot 10), bounds 10 and 12, no tolerances, on both sides |
| `plusMinusDimensionKeepsToleranceOrder` | `OCCTDocumentGetDimensionInfo` reads the upper tolerance as the lower | :129 Expectation failed: dim.bounds == .plusMinus(lowerTolerance: -0.3, upperTolerance: 0.7); :130 Expectation failed: dim.lowerTolerance == -0.3 | passed | `OCCTDocumentGetDimensionInfo` | PASS: plus/minus, value 20, lower tolerance -0.3 and upper +0.7 the right way round, no bounds, on both sides |
| `simpleDimensionHasNoBoundsOrTolerances` | `OCCTDocumentGetDimensionInfo` reports a simple dimension as unset | :160 Expectation failed: dim.bounds == .simple; :161 Expectation failed: dim.value.map { abs($0 - 3.5) < 1e-9 } == true | passed | `OCCTDocumentGetDimensionInfo` | PASS: simple value 3.5, no bounds, no tolerances, no class, on both sides |
| `classOfToleranceIsReadBack` | `OCCTDocumentGetDimensionInfo` reports the grade ordinal plus 1 | :199 Expectation failed: cls.grade == .it7 | passed | `OCCTDocumentGetDimensionInfo` | PASS: class present, hole, form variance H (11), grade IT7 (8), still a simple value, on both sides |
| `rangeDimensionCanCarryAClassOfTolerance` | `OCCTDocumentGetDimensionInfo` reports the grade ordinal plus 1 | :235 Expectation failed: dim.classOfTolerance?.grade == .it9 | passed | `OCCTDocumentGetDimensionInfo` | PASS: range 10..12 with a shaft class, form variance JS (12), grade IT9 (10), on both sides |
| `toleranceOnARangeDimensionIsRefused` | `OCCTDocumentSetDimensionTolerance` returns true without checking anything | :257 Expectation failed: doc.setDimensionTolerance(at: idx, lower: -0.3, upper: 0.7) == false | passed | `OCCTDocumentSetDimensionTolerance` | PASS: the tolerance write on a range is not applied, and the range 10..12 is unchanged on a fresh read, on both sides |
| `mutatorsRejectAnOutOfRangeIndex` | `OCCTDocumentSetDimensionTolerance` returns true without checking anything | :269 Expectation failed: doc.setDimensionTolerance(at: 0, lower: -0.1, upper: 0.1) == false | passed | `OCCTDocumentSetDimensionTolerance` | PASS: no dimension exists at index 0 or -1, so every write there is refused, on both sides |
| `formVarianceEnumComplete` | a compile-time extra case (`injected766 = 9999`) added to the Swift enum; removed and rebuilt for green | :279 Expectation failed: Document.DimensionFormVariance.allCases.count == 29 | passed | `OCCTDocumentGetDimensionInfo` | PASS: 29 form variances on both sides |
| `gradeEnumComplete` | a compile-time extra case (`injected766 = 9999`) added to the Swift enum; removed and rebuilt for green | :284 Expectation failed: Document.DimensionGrade.allCases.count == 20 | passed | `OCCTDocumentGetDimensionInfo` | PASS: 20 grades on both sides |

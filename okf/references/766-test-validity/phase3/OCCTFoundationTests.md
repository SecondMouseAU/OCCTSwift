...
| OSD_Host Tests | hostName | `OCCTHostName` | `"localhost"` (A) | ✅ `:940` (original: green) | ✅ | MATCH on the first DNS label (CI runner: `<host>.` vs `<host>.local`) | Rewritten; the first version failed CI comparing the whole string |
| OSD_Host Tests | systemVersion | `OCCTSystemVersion` | return "Darwin" (A) | ✅ `:949` (original: green) | ✅ | MATCH, "Darwin 27.0.0" == uname sysname + release | Rewritten: `contains("Darwin")` |
| OSD_Host Tests | internetAddress | `OCCTInternetAddress` | return nullptr (C) | ✅ `:958` (original: green) | ✅ | MATCH, parses as IPv4 | Rewritten: had no assertion at all |
| OSD_PerfMeter Tests | measureTime | `OCCTPerfMeterElapsed` | Create skips `Start()` (C); reading × 1000 (D) | ✅ C `:1015` `readOwnCPU` (original: green), D `:1011` upper bound | ✅ | MATCH in kind: live-thread CPU, see `perfmeter-churn-transcript.txt` | Rewritten twice: `>= 0`, then a wall-time bound that failed CI (0.2156 vs 0.2101). Now both bounds in CPU time; lower bound in one of up to 20 windows |
| OSD_Directory Tests | tempDirectory | `OCCTDirectoryRemove` | Remove returns true without removing (A) | ✅ `:1029` (original: green) | ✅ | MATCH, under /tmp, Exists 1, then 0 after Remove | Strengthened: removal was unchecked |
| OSD_Directory Tests | createAndRemoveDirectory | `OCCTDirectoryCreate` | Remove returns true without removing (A) | ✅ `:1040` | ✅ | MATCH, Build then Exists 1, Remove then Exists 0 |  |
| Resource_Unicode Tests | setAndGetFormat | `OCCTUnicodeSetFormat` | SetFormat no-op (A) | ✅ `:1055` `sjis == .sjis` (original: green) | ✅ | MATCH, SJIS then ANSI read back | Rewritten: set only the default `.ansi` |
| Resource_Unicode Tests | convertToUnicode | `OCCTUnicodeConvertToUnicode` | drop the first code unit (A) | ✅ `:1066` | ✅ | MATCH "hello" |  |
| Resource_Unicode Tests | convertFromUnicode | `OCCTUnicodeConvertFromUnicode` | length - 1 (A) | ✅ `:1077` | ✅ | MATCH "hello" |  |
| OSD_DirectoryIterator Tests | countDirectories | `OCCTDirectoryIteratorCount` | count starts at 1 (B) | ✅ `:1112` (original: green) | ✅ | MATCH 4 (".", "..", a, b) | Rewritten: /tmp with `count >= 0`; now a fixture |
| OSD_DirectoryIterator Tests | nameAtIndex | `OCCTDirectoryIteratorName` | index off by one (B) | ✅ `:1122` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_DirectoryIterator Tests | listDirectories | `OCCTDirectoryList` | skip the first entry (B) | ✅ `:1133` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_FileIterator Tests | countFiles | `OCCTFileIteratorCount` | count starts at 1 (B) | ✅ `:1146`, `:1147` (original: green) | ✅ | MATCH 3 | Rewritten, fixture; adds a `*.txt` mask case |
| OSD_FileIterator Tests | nameAtIndex | `OCCTFileIteratorName` | index off by one (B) | ✅ `:1157` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_FileIterator Tests | listFiles | `OCCTFileList` | skip the first entry (B) | ✅ `:1168` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_Disk | diskSize | `OCCTDiskSize` | drop the `/ 2` (A) | ✅ `:1182` (original: green) | ✅ | MATCH 971350180 KB == statvfs | Rewritten: `size >= 0` |
| OSD_Disk | diskFreeSpace | `OCCTDiskFree` | return 0 (A) | ✅ `:1187` (original: green) | ✅ | MATCH in kind: > 0 and <= size | Rewritten: `free >= 0` |
| OSD_Disk | diskIsValid | `OCCTDiskIsValid` | negate Failed() (A) | ✅ `:1193` | ✅ | MATCH, Failed() 0 |  |
| OSD_Disk | diskName | `OCCTDiskName` | return nullptr (A) | ✅ `:1200` | ✅ | MATCH "" | Pinned to the kernel's empty name (#1442) |

...
| Thread Safety: OCCTSerial | serialLockBasic | `OCCTSerialLockAcquire` | Acquire/Release made no-ops | ✅ `:1389` `!ranWhileHeld` (original: green) | ✅ | N/A lock (bridge std::recursive_mutex); box volume MATCH 1000 | Rewritten: `box != nil` inside the lock passed with no lock |
| Thread Safety: OCCTSerial | serialLockReentrant | `OCCTSerialLockAcquire` | plain std::mutex in place of the recursive one | ✅ `:1419` `try #require(gotInner)` (30 s, after the worker holds the outer lock); `:1415` `try #require(gotOuter)` if the worker never gets the outer lock | ✅ | N/A lock; volume MATCH 125 | Nested acquire tried on a worker thread; only the nested step has a tight timeout, waits behind other suites are 600 s (CI run of #2291: a 10 s wait failed under contention). Both waits are `try #require`, not `guard ... else { return }`: red at `:1415` under a first acquire that never returns and at `:1419` under a lock that blocks a nested acquire, each with the waits shortened to 3 s for the run |
| Thread Safety: OCCTSerial | deepCopyForParallel | `OCCTShapeDeepCopy` | `copy = shape->shape` in place of TNaming_CopyShape::CopyTool | ✅ `:1432` `!copy.isSame(as: orig)` (original: green); `:1431` `try #require(orig.deepCopy())` if the copy is nil, `:1430` `try #require(Shape.box(...))` if the box is | ✅ | MATCH, IsSame false, volume 1000 both | Rewritten: three nested `if let`s; a shared or nil copy passed. The box and the copy are `try #require`, not `guard let ... else { return }` |
| Thread Safety: OCCTSerial | serializedConcurrentAccess | `OCCTSerialLockAcquire` | Acquire/Release made no-ops | ✅ `:1459` `state.maxInside == 1` (original: green) | ✅ | volumes MATCH 1000/8000/27000/64000 | Rewritten: now counts workers inside the lock and pins volumes |
| v0.149 Sheet.standardLayout | firstAngleTopBelow | `OCCTDrawingCreate` | swap upper/lower row in cellCentre | ✅ `:1480` | ✅ | view bounds MATCH (HLRBRep_Algo) |  |
| v0.149 Sheet.standardLayout | thirdAngleTopAbove | `OCCTDrawingCreate` | swap upper/lower row in cellCentre | ✅ `:1492` | ✅ | view bounds MATCH |  |
| v0.149 Sheet.standardLayout | viewsFitInsideInnerFrame | `OCCTDrawingCreate` | drop the fit-to-cell clamp (`appliedScale = scale.factor`) | ✅ `:1517`-`:1520` (original: green) | ✅ | view bounds MATCH | Rewritten: checked only view centres at 1:1; now 100:1 and full placed extents |
| v0.149 Sheet.standardLayout | interCellGapIsHalfMargin | `OCCTDrawingCreate` | pre-#1572 `cellW = (innerW - margin) / 2` | ✅ `:1568` | ✅ | view bounds MATCH | A column-step injection left it green: front sits in column 0, so only cellW moves it |
| v0.149 Sheet.standardLayout | includeIsoFalseOmits | `OCCTDrawingCreate` | ignore includeIso | ✅ `:1580`, `:1581` | ✅ | view bounds MATCH |  |
| v0.149 Sheet.standardLayout | renderEmitsEveryView | `OCCTDrawingCreate` | render `placed.dropLast()` | ✅ `:1599` `counts.lines == 36` (original: green) | ✅ | MATCH, 36 lines = 8+8+8+12 HLR sharp edges | Rewritten: `> 0` passed with views missing |
| v0.149 Sheet.standardLayout | renderEmitsEveryViewPDF | `OCCTDrawingCreate` | render `placed.dropLast()` | ✅ `:1619` (original: green) | ✅ | MATCH, 36 | Rewritten, as above |
| v0.149 Sheet.standardLayout | renderEmitsEveryViewSVG | `OCCTDrawingCreate` | render `placed.dropLast()` | ✅ `:1636` (original: green) | ✅ | MATCH, 36 | Rewritten, as above |
| v0.150 BillOfMaterials | emptyBOMHeader | `none (pure Swift)` | `0..<rowCount` separators | ✅ `:1654` | ✅ | N/A, no OCCT call |  |
| v0.150 BillOfMaterials | threeItemBOM | `none (pure Swift)` | `0..<rowCount` separators | ✅ `:1669` | ✅ | N/A, no OCCT call |  |
| v0.150 BillOfMaterials | codableRoundTrip | `none (pure Swift)` | CodingKeys without `title` | ✅ `:1682` | ✅ | N/A, no OCCT call |  |
| v0.150 BillOfMaterials | sheetRenderBOM | `none (pure Swift)` | anchor at the frame's bottom-left | ✅ `:1698`-`:1701` (original: green) | ✅ | N/A, no OCCT call | Rewritten: one-sided bounds passed a table hanging 177 mm off the frame |

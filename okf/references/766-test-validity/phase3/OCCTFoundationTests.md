...
| OSD_Host Tests | hostName | `OCCTHostName` | `"localhost"` (A) | ✅ `:1035` (original: green); `:1033` `try #require(HostInfo.hostName)` if the bridge returns nil | ✅ | MATCH on the first DNS label (CI runner: `<host>.` vs `<host>.local`) | Rewritten; the first version failed CI comparing the whole string. A nil name is `try #require`, not `guard let ... else { return }`: red at `:1033` with `OCCTHostName` returning nullptr |
| OSD_Host Tests | systemVersion | `OCCTSystemVersion` | return "Darwin" (A) | ✅ `:1044` (original: green) | ✅ | MATCH, "Darwin 27.0.0" == uname sysname + release | Rewritten: `contains("Darwin")` |
| OSD_Host Tests | internetAddress | `OCCTInternetAddress` | return nullptr (C) | ✅ `:1054` (original: green); `:1052` `try #require(HostInfo.internetAddress)` if the bridge returns nil | ✅ | MATCH, parses as IPv4 | Rewritten: had no assertion at all. A nil address is `try #require`, not `guard let ... else { return }`: red at `:1052` with `OCCTInternetAddress` returning nullptr |
| OSD_PerfMeter Tests | measureTime | `OCCTPerfMeterElapsed` | Create skips `Start()` (C); reading × 1000 (D) | ✅ C `:1108` `readOwnCPU` (original: green), D `:1104` upper bound | ✅ | MATCH in kind: live-thread CPU, see `perfmeter-churn-transcript.txt` | Rewritten twice: `>= 0`, then a wall-time bound that failed CI (0.2156 vs 0.2101). Now both bounds in CPU time; lower bound in one of up to 20 windows |
| OSD_Directory Tests | tempDirectory | `OCCTDirectoryRemove` | Remove returns true without removing (A) | ✅ `:1120` (original: green); `:1116` `try #require(DirectoryUtils.buildTemporary())` if the bridge returns nil | ✅ | MATCH, under /tmp, Exists 1, then 0 after Remove | Strengthened: removal was unchecked. The directory is `try #require`, not `#expect(tmpDir != nil)` then `if let`: red at `:1116` with `OCCTDirectoryBuildTemporary` returning nullptr |
| OSD_Directory Tests | createAndRemoveDirectory | `OCCTDirectoryCreate` | Remove returns true without removing (A) | ✅ `:1130` | ✅ | MATCH, Build then Exists 1, Remove then Exists 0 |  |
| Resource_Unicode Tests | setAndGetFormat | `OCCTUnicodeSetFormat` | SetFormat no-op (A) | ✅ `:1145` `sjis == .sjis` (original: green) | ✅ | MATCH, SJIS then ANSI read back | Rewritten: set only the default `.ansi` |
| Resource_Unicode Tests | convertToUnicode | `OCCTUnicodeConvertToUnicode` | drop the first code unit (A) | ✅ `:1156` | ✅ | MATCH "hello" |  |
| Resource_Unicode Tests | convertFromUnicode | `OCCTUnicodeConvertFromUnicode` | length - 1 (A) | ✅ `:1167` | ✅ | MATCH "hello" |  |
| OSD_DirectoryIterator Tests | countDirectories | `OCCTDirectoryIteratorCount` | count starts at 1 (B) | ✅ `:1197` (original: green) | ✅ | MATCH 4 (".", "..", a, b) | Rewritten: /tmp with `count >= 0`; now a fixture |
| OSD_DirectoryIterator Tests | nameAtIndex | `OCCTDirectoryIteratorName` | index off by one (B) | ✅ `:1204` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_DirectoryIterator Tests | listDirectories | `OCCTDirectoryList` | skip the first entry (B) | ✅ `:1212` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_FileIterator Tests | countFiles | `OCCTFileIteratorCount` | count starts at 1 (B) | ✅ `:1222`, `:1223` (original: green) | ✅ | MATCH 3 | Rewritten, fixture; adds a `*.txt` mask case |
| OSD_FileIterator Tests | nameAtIndex | `OCCTFileIteratorName` | index off by one (B) | ✅ `:1230` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_FileIterator Tests | listFiles | `OCCTFileList` | skip the first entry (B) | ✅ `:1238` (original: green) | ✅ | MATCH | Rewritten, fixture |
| OSD_Disk | diskSize | `OCCTDiskSize` | drop the `/ 2` (A) | ✅ `:1252` (original: green) | ✅ | MATCH 971350180 KB == statvfs | Rewritten: `size >= 0` |
| OSD_Disk | diskFreeSpace | `OCCTDiskFree` | return 0 (A) | ✅ `:1257` (original: green) | ✅ | MATCH in kind: > 0 and <= size | Rewritten: `free >= 0` |
| OSD_Disk | diskIsValid | `OCCTDiskIsValid` | negate Failed() (A) | ✅ `:1263` | ✅ | MATCH, Failed() 0 |  |
| OSD_Disk | diskName | `OCCTDiskName` | return nullptr (A) | ✅ `:1270` | ✅ | MATCH "" | Pinned to the kernel's empty name (#1442) |

...
| Curve3D.extrema returns empty, not SIGSEGV, on parallel curves (#636) | Two bounded parallel segments with overlapping projected ranges: extrema is empty | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | exfill: an empty extrema answer gets one fake result | `Issue636ExtremaParallelCurvesTests.swift:88 a.extrema(with: b).isEmpty` | ✅ | MATCH |  |
| The extrema doc snippet is runnable and its printed values are true (#636) | the snippet's API calls resolve and its stated values hold | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | exfill: an empty extrema answer gets one fake result | `Issue636ExtremaParallelCurvesTests.swift:153 pairs.isEmpty` | ✅ | MATCH |  |
| Curve3D.extrema returns empty, not SIGSEGV, on parallel curves (#636) | Two unbounded parallel lines: extrema is empty, not a crash | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | exfill: an empty extrema answer gets one fake result | `Issue636ExtremaParallelCurvesTests.swift:79 a.extrema(with: b).isEmpty  (+2 more)` | ✅ | MATCH |  |
| Curve3D.extrema returns empty, not SIGSEGV, on parallel curves (#636) | Non-parallel curves are unaffected: extrema still reports real solutions | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | exdup: a single extremum reported twice | `Issue636ExtremaParallelCurvesTests.swift:135 result.count == 1` | ✅ | MATCH |  |
| Curve3D.extrema returns empty, not SIGSEGV, on parallel curves (#636) | Parallel segments with disjoint ranges keep their real endpoint solution | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | exdup: a single extremum reported twice | `Issue636ExtremaParallelCurvesTests.swift:119 found.count == 1` | ✅ | MATCH |  |
| Curve3D.extrema returns empty, not SIGSEGV, on parallel curves (#636) | minDistance(to:) keeps reporting the true offset for the same parallel pairs | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | mdoff: minDistance + 1e-3 | `Issue636ExtremaParallelCurvesTests.swift:99 abs(d - Self.parallelOffset) < 1e-9  (+1 more)` | ✅ | MATCH |  |
| Issue #853: uniformAbscissa(distance:) gains the ceiling its pointCount sibling always had | a distance implying more points than the ceiling returns nil, both overloads | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | uafake: a refused distance answers [0, 1] | `Issue853UniformAbscissaDistanceCeilingTests.swift:35 edge.uniformAbscissa(distance: tinySpacing) == nil  (+1 more` | ✅ | N/A |  |
| Issue #853: uniformAbscissa(distance:) gains the ceiling its pointCount sibling always had | a non-positive or non-finite distance returns nil without reaching the ceiling check | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | uafake: a refused distance answers [0, 1] | `Issue853UniformAbscissaDistanceCeilingTests.swift:49 edge.uniformAbscissa(distance: 0) == nil  (+4 more)` | ✅ | N/A |  |
| Issue #853: uniformAbscissa(distance:) gains the ceiling its pointCount sibling always had | a plausible distance is unaffected by the new ceiling check, both overloads | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | uadrop: last abscissa dropped | `Issue853UniformAbscissaDistanceCeilingTests.swift:63 params?.count == 5` | ✅ | MATCH |  |
| Issue #853: uniformAbscissa(distance:) gains the ceiling its pointCount sibling always had | pointCount and its range sibling are unaffected by sharing the new helper | `OCCTCurve3DExtrema / OCCTCurve3DMinDistanceToCurve / OCCTUniformAbscissaByDistance` | samplemin: counts below 2 clamped to 2 | `Issue853UniformAbscissaDistanceCeilingTests.swift:76 edge.uniformAbscissa(pointCount: 1) == nil` | ✅ | MATCH |  |

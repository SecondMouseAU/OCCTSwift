...
| GeomConvert CompCurveToBSpline Tests | Join two line segments | `OCCTCurve3DJoinCurves` | last curve not added (and, separately, result moved 0.5 in y) | `CurveJoinTests.swift:20 simd_distance(joined.point(at: dom.upperBound), end) < 1e-12` | ✅ | MATCH | Rewritten: `if let` and 0.01 of slack |
| GeomConvert CompCurveToBSpline Tests | Join three segments | `OCCTCurve3DJoinCurves` | last curve not added (and, separately, result moved 0.5 in y) | `CurveJoinTests.swift:20 simd_distance(joined.point(at: dom.upperBound), end) < 1e-12` | ✅ | MATCH | Rewritten: `if let` and 0.01 of slack. The first rewrite pinned the domain at 2 + sqrt 2 and failed green; the probe's 3.828 = 1 + 2 sqrt 2 is right |
| GeomConvert CompCurveToBSpline Tests | Join single curve | `OCCTCurve3DJoinCurves` | result moved 0.5 in y | `CurveJoinTests.swift:19 simd_distance(joined.point(at: dom.lowerBound), start) < 1e-12` | ✅ | MATCH | Rewritten: `if let` and 0.01 of slack |
| v0.115.0 - Curve Length and Closest | curve3DArcLength | `OCCTCurve3DGetLengthBetween` | length x 1.0001 | `CurveLengthTests.swift:25 abs(curve.arcLength(from: domain.lowerBound, to: domain.upperBound) - 10) < 1e-9` | ✅ | MATCH | Rewritten: len > 0 |
| v0.115.0 - Curve Length and Closest | curve3DClosestParameter | `OCCTCurve3DNearestParameter` | parameter + 1 | `CurveLengthTests.swift:37 abs(param - 5.0) < 1e-9` | ✅ | MATCH | Rewritten: 0.1 of slack inside `if let` |
| v0.115.0 - Curve Length and Closest | curve2DArcLength | `OCCTCurve2DGetLengthBetween` | length x 1.0001 (and, separately, end tangent loaded as start) | `CurveLengthTests.swift:53 abs(len - 14.355489462097665) < 1e-6` | ✅ | MATCH | Rewritten: len > 0 |
| Curve Planarity Check | Circle is planar | `OCCTCurve3DIsPlanar` | normal components rotated | `CurvePlanarityTests.swift:23 simd_distance(normal, SIMD3(0, 0, 1)) < 1e-10` | ✅ | MATCH | Rewritten: force-unwrap inside #expect |
| Curve Planarity Check | Line is planar | `OCCTCurve3DIsPlanar` | normal components rotated | `CurvePlanarityTests.swift:37 simd_distance(normal, SIMD3(0, 0, 1)) < 1e-10` | ✅ | MATCH | Rewritten: discarded its result, could not fail |
| v0.115.0 - Curve Split and Concatenate | splitAtContinuity3D | `OCCTCurve3DSplitAtContinuity` | no pieces returned | `CurveSplitConcatTests.swift:19 segs.count == 1` | ✅ | MATCH | Rewritten: count >= 1 inside `if let` |
| v0.115.0 - Curve Split and Concatenate | concatenateCurvesG1 | `OCCTCurve3DConcatenateG1` | result moved 0.5 in z | `CurveSplitConcatTests.swift:34 simd_distance(joined.startPoint, SIMD3(0, 0, 0)) < 1e-9` | ✅ | MATCH | Rewritten: checked only != nil |
| v0.115.0 - Curve Split and Concatenate | concatenateG1RejectsDisconnectedCurve | `OCCTCurve3DConcatenateG1` | Add() failure ignored | `CurveSplitConcatTests.swift:46 joined == nil` | ✅ | MATCH |  |
| v0.115.0 - Curve Split and Concatenate | splitCurve2DAtContinuity | `OCCTCurve2DSplitAtContinuity` | no pieces returned | `CurveSplitConcatTests.swift:56 segs.count == 1` | ✅ | MATCH | Rewritten: count >= 1 inside `if let` |
| Curve3D Operations Tests | Trim circle to quarter arc | `OCCTCurve3DTrim` | trimmed to half the requested range | `Curve3DOperationsTests.swift:24 simd_distance(end, SIMD3(0, 5, 0)) < 1e-12` | ✅ | MATCH | Strengthened: whole end points |
| Curve3D Operations Tests | Reverse segment swaps endpoints | `OCCTCurve3DReversed` | copy returned unreversed | `Curve3DOperationsTests.swift:34 simd_distance(revStart, SIMD3(10, 5, 3)) < 1e-12` | ✅ | MATCH | Strengthened: x only before |
| Curve3D Operations Tests | Translate segment | `OCCTCurve3DTranslate` | dz + 1 | `Curve3DOperationsTests.swift:43 simd_distance(start, SIMD3(5, 5, 5)) < 1e-12` | ✅ | MATCH |  |
| Curve3D Operations Tests | Rotate segment around Z axis | `OCCTCurve3DRotate` | angle x 1.01 | `Curve3DOperationsTests.swift:53 simd_distance(start, SIMD3(0, 5, 0)) < 1e-12` | ✅ | MATCH | Rewritten: tolerance 0.01 passed a 1% rotation error |
| Curve3D Operations Tests | Scale segment | `OCCTCurve3DScale` | factor + 1 | `Curve3DOperationsTests.swift:63 abs(start.x - 3) < 1e-10` | ✅ | MATCH |  |
| Curve3D Operations Tests | Mirror across XY plane | `OCCTCurve3DMirrorPlane` | plane moved 0.5 in z | `Curve3DOperationsTests.swift:71 simd_distance(mirrored.startPoint, SIMD3(0, 0, -1)) < 1e-12` | ✅ | MATCH | Strengthened: z only before |
| Curve3D Operations Tests | Mirror across a point | `OCCTCurve3DMirrorPoint` | centre moved 1 in x | `Curve3DOperationsTests.swift:85 abs(start.x + 1) < 1e-10` | ✅ | MATCH |  |
| Curve3D Operations Tests | Mirror across an axis | `OCCTCurve3DMirrorAxis` | axis moved 0.5 in y | `Curve3DOperationsTests.swift:101 abs(start.y + 1) < 1e-10` | ✅ | MATCH |  |
| Curve3D Operations Tests | Length of segment | `OCCTCurve3DGetLength` | length x 1.0001 | `Curve3DOperationsTests.swift:111 len == 5.0` | ✅ | MATCH | Rewritten: tolerance 0.01 |
| Curve3D Operations Tests | Length of circle | `OCCTCurve3DGetLength` | length x 1.0001 | `Curve3DOperationsTests.swift:119 abs((len ?? -1) - 2 * .pi * radius) < 1e-9` | ✅ | MATCH | Rewritten: tolerance 0.01 |
| Curve3D Operations Tests | Partial length | `OCCTCurve3DGetLengthBetween` | length x 1.0001 | `Curve3DOperationsTests.swift:127 halfLen == 5.0` | ✅ | MATCH | Rewritten: tolerance 0.01 |
...
| Arc length stops being one quadrature per span (#603) | parameterAtLength(length) still lands on the end of the curve | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:239 abs(end - c.domain.upperBound) < 1e-7  (+2 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | The closed forms stay closed forms | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:183 abs(circumference - 2 * .pi * 5) < 1e-12  (+1 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A wound range winds an accurate period (#600 preserved) | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:219 abs(two - 2 * one) / two < 1e-9  (+1 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | An elliptical edge measures the same as the curve it was built from | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:312 abs(edge.edgeArcLength - curveLength) / truth < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | Accurate sub-ranges stay accurate, and the parts still sum to the whole | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:204 abs(whole - 2 * h) / whole < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A whipping cubic Bezier measures its arc | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:134 abs(measured - truth) / truth < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A short multi-span interpolation gets the accuracy per-span splitting left behind | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:162 abs(measured - truth) / truth < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A whole ellipse measures its own circumference, not 0.3-1.7% more | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclen: Curve3D whole length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:72 abs(measured - truth) / truth < 1e-9  (+4 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A hyperbola over a wide range measures its arc | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclenbetween: ranged length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:113 abs(measured - truth) / truth < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A parabola over a wide range measures its arc, not 3% less | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | arclenbetween: ranged length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:95 abs(measured - truth) / truth < 1e-9` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A negative abscissa walks backwards accurately | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | pal3: parameterAtLength + 1e-5 | `Issue603SingleSpanQuadratureTests.swift:272 abs(arc - total / 4) / total < 1e-8` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A fraction of the length is that fraction of the arc | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | pal3: parameterAtLength + 1e-5 | `Issue603SingleSpanQuadratureTests.swift:257 abs(arc - total * fraction) / total < 1e-8  (+2 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A 2D ellipse measures the same as the 3D one, both spellings | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | c2len: Curve2D length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:288 abs(whole - truth) / truth < 1e-9  (+1 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | A wire containing an elliptical edge measures accurately, and so does its adaptor | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | wclen: WireCurve length x 1.001 | `Issue603SingleSpanQuadratureTests.swift:352 abs(wc.length - truth) / truth < 1e-9  (+1 more)` | ✅ | MATCH |  |
| Arc length stops being one quadrature per span (#603) | An EdgeCurve measures accurately and inverts consistently | `OCCTCurve3DGetLength / GetLengthBetween / ParameterAtLength, Curve2D, edge, wire` | eclen: EdgeCurve length x 1.0001 | `Issue603SingleSpanQuadratureTests.swift:371 abs(ec.length - truth) / truth < 1e-9  (+1 more)` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | A point on the circle but off the arc answers with the arc's end | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | npoff: nearestParameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:67 abs(Self.distance(arc.point(at: parameter), query) - 4.47213` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | A half circle queried from below answers with the near end, not the far side | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | npoff: nearestParameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:51 abs(parameter - 0) < 1e-9  (+1 more)` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | A point past the end is answered, not refused | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | npoff: nearestParameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:78 segment.nearestParameter(to: SIMD3(100, 0, 0)) == 8  (+2 mor` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | nearestParameter and projectPoint agree on every query | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | npoff: nearestParameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:110 abs(Self.distance(curve.point(at: parameter), query) - proje` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | The locate-from-a-guess primary search stays local, maxima included | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | locoff: locateNearestPoint parameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:166 abs(atFarSide.parameter - .pi / 2) < 1e-9` | ✅ | MATCH |  |
| nearestParameter reports the nearest point over the range (#615) | The locate-from-a-guess fallback answers over the whole curve, correctly | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | locoff: locateNearestPoint parameter + 1e-3 | `Issue615NearestParameterRangeTests.swift:134 abs(atStart.parameter - 0) < 1e-9  (+3 more)` | ✅ | MATCH |  |
| Measured continuity encoding after the retirement (#619) | A C1 spline reports C1 as ordinal 2, not 1 | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | contold: the retired hand-mapped encoding | `Issue619ContinuityEncodingTests.swift:70 c1.continuity == 2  (+1 more)` | ✅ | MATCH |  |
| Measured continuity encoding after the retirement (#619) | An analytic curve reports CN as ordinal 6, the old encoding's 99 is unreachable | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | contold: the retired hand-mapped encoding | `Issue619ContinuityEncodingTests.swift:38 curve.continuity == 6  (+3 more)` | ✅ | MATCH |  |
| Measured continuity encoding after the retirement (#619) | A raw threshold of 2 now admits a merely-C1 curve; satisfies(.c2) still refuses it | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | contold: the retired hand-mapped encoding | `Issue619ContinuityEncodingTests.swift:94 c1.continuity >= 2  (+2 more)` | ✅ | MATCH |  |
| Measured continuity encoding after the retirement (#619) | Every measured class round-trips through the raw ordinal | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | contold: the retired hand-mapped encoding | `Issue619ContinuityEncodingTests.swift:141 curve.continuity == Int(curve.continuityClass.rawValue)  (+4` | ✅ | MATCH |  |
| Measured continuity encoding after the retirement (#619) | The two vocabularies' constants do not line up, which is why the raw form drifted | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | satall: satisfies() always true | `Issue619ContinuityEncodingTests.swift:126 !ContinuityClass.c1.satisfies(.c2)` | ✅ | N/A |  |
| Measured continuity encoding after the retirement (#619) | The sibling raw-ordinal accessors use the same encoding, so CN is 6 there too | `OCCTCurve3DNearestParameter / OCCTExtremaLocateOnCurve / OCCTCurve3DGetContinuity` | bzcont: bezierContinuity - 2 | `Issue619ContinuityEncodingTests.swift:55 bezier.bezierContinuity == 6` | ✅ | MATCH |  |

...
| v0.113.0 - ProjectionOnCurve | multiResultProjection | `OCCTProjOnCurveCreate` | proj: Init on (10,1,0) instead of (10,0,0) | `ProjectionOnCurveTests.swift:22 point(at: 0) == (5,0,0)` | ✅ | MATCH | REWRITTEN: tolerances were 0.1 and the far extremum was never checked; silent `if let` guards now record an issue. The extremum count is `try #require` (`:21`), not `#expect` then `guard ... else { return }`: red at `:21` with OCCTProjOnCurveNbPoints returning 1 |
| v0.113.0 - ProjectionOnCurve | parameterAccess | `OCCTProjOnCurveParameter` | proj | `ProjectionOnCurveTests.swift:39 parameter(at: 0) == 0` | ✅ | MATCH | REWRITTEN: was `param >= 0`, true of any wrong parameter in [0, 2pi). The extremum count is `try #require` (`:37`): red at `:37` with OCCTProjOnCurveNbPoints returning 1 |
...
| Quasi-Uniform Abscissa Sampling | minCount | `OCCTCurve3DQuasiUniformAbscissa` | qua | `QuasiUniformAbscissaTests.swift:50 params[1] == 10` | ✅ | MATCH | REWRITTEN: was count only. The count is `try #require` (`:48`), not `#expect` then `guard ... else { return }`: red at `:48` with OCCTCurve3DQuasiUniformAbscissa returning 1 |
| Quasi-Uniform Deflection Sampling | sampleCircle | `OCCTCurve3DQuasiUniformDeflection` | qud: sampler given 2x the deflection | `QuasiUniformDeflectionTests.swift:29 points.count == 24` | ✅ | MATCH | REWRITTEN: was count > 4 and radius within 0.2. The first point is `try #require` (`:35`), not `if let`: red at `:35` with OCCTCurve3DQuasiUniformDeflection returning no points |
| Quasi-Uniform Deflection Sampling | tighterDeflection | `OCCTCurve3DQuasiUniformDeflection` | qud | `QuasiUniformDeflectionTests.swift:47 coarse.count == 8` | ✅ | MATCH | REWRITTEN: was fine.count > coarse.count |
| v0.143 Sketch arcs and circles | circleTessellation | none (pure Swift SketchElement.CurveKind.tessellate2D) | sk (Swift): radius scaled by 1.000001 in tessellate2D | `SketchArcCircleTests.swift:20 abs(r - 5.0) < 1e-9` | ✅ | N/A | strengthened: exact count 51 and closure added. The first and last points are `try #require` (`:22`, `:23`), not `if let`: red at `:22` with tessellate2D returning no points (Swift injection) |
...
| Wire.edgePolyline | singleEdge | `OCCTShapeGetEdgePolyline` | wep: z + 1e-3 on every primary-path point | `WireEdgePolylineTests.swift:22 polyline[0] == (-5,-2.5,0)` | ✅ | MATCH | REWRITTEN: was count >= 2. The count is `try #require` (`:21`), not `#expect` then `guard ... else { return }`: red at `:21` with OCCTShapeGetEdgePolyline reporting one point |

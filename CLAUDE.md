...
- The full suite occasionally (~1-in-10 parallel runs) hits a hard crash or a timing-flake under
  parallel execution — see Known OCCT Bugs' `XCAFDoc_ShapeTool::theAutoNaming` entry (#341, mitigated)
  and the "File was not written with this version of the topology" note (uncharacterized, likely
  fixed-temp-file-path collisions in `Tests/`, not a kernel bug). A single domain target rarely trips
  either.

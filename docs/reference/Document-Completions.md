...
**Every evaluator here returns an optional, and `nil` means the call was refused rather than
answered (#1646).** Three things produce a refusal, and none of them can be told from a real answer
in a non-optional return, because the origin is a point these curves legitimately pass through:

1. **An argument the OCCT constructor rejects.** `Geom2dEval_SineWaveCurve` requires amplitude and
   omega above zero, `Geom2dEval_CircleInvoluteCurve` a radius above zero,
   `Geom2dEval_ArchimedeanSpiralCurve` a growth rate above zero and a non-negative initial radius,
   `Geom2dEval_LogarithmicSpiralCurve` a scale and growth exponent above zero. Each raises
   `Standard_ConstructionError`, which was a process abort before it was caught (#1407, #345).
2. **A non-finite argument.** OCCT's own checks are written as `<= 0`, and every comparison against
   NaN is false, so a NaN reaches the evaluation and comes back as a NaN point.
3. **A finite argument whose evaluation overflows**, such as the logarithmic spiral's `exp(b*t)` at
   a large `t`.

Each is measured against the pinned kernel in
`Scripts/repro/1646-evaluator-contract/probe_geom2deval_contract.mm`. The bridge's C functions
return `bool` for the same reason, and zero their out-parameters on a refusal so a caller ignoring
the flag reads a deterministic value rather than a NaN.
...
- **Returns:** 2D point on the spiral, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_ArchimedeanSpiralCurve::EvalD0` (via `OCCTGeom2dEvalArchimedeanSpiralD0`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let pt = Geom2dEval.archimedeanSpiralD0(initialRadius: 1, growthRate: 0.1, u: 2 * .pi) {
      print(pt)
  }
  ```
...
- **Returns:** `(point, d1)`, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_ArchimedeanSpiralCurve::EvalD1` (via `OCCTGeom2dEvalArchimedeanSpiralD1`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let (pt, d1) = Geom2dEval.archimedeanSpiralD1(
...
- **Returns:** 2D point on the spiral, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_LogarithmicSpiralCurve::EvalD0` (via `OCCTGeom2dEvalLogSpiralD0`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let pt = Geom2dEval.logarithmicSpiralD0(scale: 1, growthExponent: 0.2, u: 2 * .pi) {
      print(pt)
  }
  ```
...
- **Returns:** `(point, d1)`, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_LogarithmicSpiralCurve::EvalD1` (via `OCCTGeom2dEvalLogSpiralD1`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let (pt, d1) = Geom2dEval.logarithmicSpiralD1(scale: 1, growthExponent: 0.2, u: .pi) {
      print(pt, d1)
  }
  ```
...
- **Returns:** 2D point on the involute, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_CircleInvoluteCurve::EvalD0` (via `OCCTGeom2dEvalCircleInvoluteD0`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
...
- **Example:**
  ```swift
  if let pt = Geom2dEval.circleInvoluteD0(radius: 10, u: 0.5) {
      print(pt)
  }
  ```
...
- **Returns:** `(point, d1)`, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_CircleInvoluteCurve::EvalD1` (via `OCCTGeom2dEvalCircleInvoluteD1`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let (pt, d1) = Geom2dEval.circleInvoluteD1(radius: 10, u: 0.5) {
      print(pt, d1)
  }
  ```
...
- **Returns:** 2D point on the involute, or `nil` if the call was refused, which includes a `direction` shorter than `1e-12`, a `radius` of 0 or less, and any non-finite argument.
- **OCCT:** `Geom2dEval_CircleInvoluteCurve::EvalD0` on a `gp_Ax2d` built from `origin` and the normalised `direction` (via `OCCTGeom2dEvalCircleInvoluteD0WithPlacement`). Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  // A gear-tooth involute rooted at the pitch point, unwinding along +Y.
  if let pt = Geom2dEval.circleInvoluteD0(
...
- **Returns:** `(point, d1)`, or `nil` if the call was refused, on the same terms as the D0 overload.
- **OCCT:** `Geom2dEval_CircleInvoluteCurve::EvalD1` (via `OCCTGeom2dEvalCircleInvoluteD1WithPlacement`). Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let (pt, tangent) = Geom2dEval.circleInvoluteD1(
      origin: SIMD2(25, 0), direction: SIMD2(0, 1), radius: 10, u: 0.5)
  {
      print(pt, tangent)
  }
...
- **Returns:** 2D point on the wave, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_SineWaveCurve::EvalD0` (via `OCCTGeom2dEvalSineWaveD0`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let pt = Geom2dEval.sineWaveD0(amplitude: 1, omega: 2 * .pi, phase: 0, u: 0.25) {
      print(pt)
  }
  ```
...
- **Returns:** `(point, d1)`, or `nil` if the call was refused.
- **OCCT:** `Geom2dEval_SineWaveCurve::EvalD1` (via `OCCTGeom2dEvalSineWaveD1`). Constructed at the origin on the X axis (`gp_Ax2d`) and evaluated in place; nothing is retained. Returns `nil` on a refused call, per the contract note at the top of this section (#1646).
- **Example:**
  ```swift
  if let (pt, d1) = Geom2dEval.sineWaveD1(amplitude: 1, omega: 2 * .pi, phase: 0, u: 0.25) {
      print(pt, d1)
  }
  ```

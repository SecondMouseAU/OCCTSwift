...
- **OCCT:** `BndLib::Add(gp_Lin, first, last, tolerance, Bnd_Box)` via `OCCTBndLibLine`.
...
- **OCCT:** `BndLib::Add(gp_Sphere, tolerance, Bnd_Box)` via `OCCTBndLibSphere`.
...
- **OCCT:** `GProp_SelGProps` over a `gp_Cylinder` via `OCCTGPropCylinderSurface`.
...
- **OCCT:** `GProp_VelGProps` over a `gp_Cylinder` via `OCCTGPropCylinderVolume`.

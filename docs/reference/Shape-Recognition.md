...
1. One of the pairs is **too close together** to yield a bisector, and the call is refused. The threshold is not machine epsilon: measured, a separation of `1e-9` still gives a crossing and `1e-10` does not. Two different mechanisms refuse, at very different separations. From `1e-10` down it is `GccAna_NoSolution`, raised inside `Bisector_Bisec::Perform` because the bisector construction has no solution to return. Only at `1e-300`, where the squared component underflows, does the perpendicular's own normalisation refuse as well, and an exactly coincident pair reaches that one first. So a caller meeting this in practice meets `GccAna_NoSolution` at around `1e-10`, not a zero-length direction.

...
1. One of the pairs is **too close together** to yield a bisector, and the call is refused. The threshold is not machine epsilon: measured, a separation of `1e-9` still gives a crossing and `1e-10` does not, refused by `GccAna_NoSolution` inside `Bisector_Bisec::Perform` rather than by the perpendicular's normalisation, which itself copes down to `1e-300`.
